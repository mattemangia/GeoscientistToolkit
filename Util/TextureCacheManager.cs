...
public class TextureCacheManager : IDisposable
{
    ...
    public void Dispose()
    {
        lock (_lock)
        {
            foreach (var entry in _cache.Values) entry.TextureManager.Dispose();
            _cache.Clear();
            _currentCacheSize = 0;
        }
    }
    ...
    {
        if (_currentCacheSize + sizeOfNewTexture <= _maxCacheSize) return;
        ...
        foreach (var item in evictable)
        {
            ...
            if (_currentCacheSize + sizeOfNewTexture <= _maxCacheSize) return; // Enough space has been freed
        }
