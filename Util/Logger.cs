...
using GeoscientistToolkit.Settings; // Using the shared LogLevel enum
...
namespace GeoscientistToolkit.Util
{
    ...
    public static class Logger
    {
        ...
        public static void Initialize(LoggingSettings settings)
        {
            lock (_fileLock)
            {
                ...
                if (_settings.EnableFileLogging)
                {
                    try
                    {
                        string logDirectory = _settings.LogFilePath;
                        string logFileName = _settings.LogFilePattern.Replace("{date}", DateTime.Now.ToString("yyyy-MM-dd"));
        ...
        public static void Log(string message, LogLevel level = LogLevel.Information)
        {
            ...
            if (!_isInitialized || level < _settings.MinimumLogLevel)
            {
                return;
            }
            ...
            string logString = BuildLogString(entry);
