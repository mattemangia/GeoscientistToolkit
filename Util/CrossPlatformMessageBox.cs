        ...
        try
        {
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
            {
                ...
                var startInfo = new ProcessStartInfo
                {
                    FileName = "zenity",
                    Arguments =
                        $"--{iconType} --title=\"{EscapeShellArg(title)}\" --text=\"{EscapeShellArg(message)}\"",
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
            {
                ...
                var script =
                    $"display dialog \"{EscapeAppleScript(message)}\" with title \"{EscapeAppleScript(title)}\" buttons {{\"OK\"}} default button \"OK\"";
