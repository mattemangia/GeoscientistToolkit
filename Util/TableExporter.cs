...
namespace GeoscientistToolkit.Data.Table
{
    ...
    public static class TableExporter
    {
        ...
        public static TableDataset ExportMaterialStatistics(CtImageStackDataset ctDataset)
        {
            try
            {
                ...
                foreach (var material in ctDataset.Materials)
                {
                    long voxelCount = materialCounts.GetValueOrDefault(material.ID, 0);
                    double volume = voxelCount * voxelVolumeMm3;
                    double volumePercent = totalVoxels > 0 ? (100.0 * voxelCount / totalVoxels) : 0;
                    double mass = volume * material.Density / 1000.0; // Convert to grams
        ...
        public static TableDataset ExportSliceStatistics(CtImageStackDataset ctDataset)
        {
            try
            {
                ...
                for (int z = 0; z < ctDataset.Depth; z++)
                {
                    double zPosition = 0;
                    if (ctDataset.Unit == "µm")
                    {
                    ...
                    double stdDev = 30;
                    int nonZeroPixels = ctDataset.Width * ctDataset.Height / 2; // Placeholder
                    double fillRatio = 50.0; // Placeholder
        ...
        public static TableDataset CreateHistogramTable(string name, int[] histogram, int binSize = 1)
        {
            try
            {
                ...
                long totalCount = histogram.Sum(h => (long)h);
                ...
                for (int i = 0; i < histogram.Length; i++)
                {
                    ...
                    double frequency = totalCount > 0 ? (100.0 * histogram[i] / totalCount) : 0;
                    double cumulative = totalCount > 0 ? (100.0 * cumulativeCount / totalCount) : 0;
    ...
    public class Measurement
    {
        ...
        public System.Numerics.Vector3 StartPoint { get; set; }
        public System.Numerics.Vector3 EndPoint { get; set; }
