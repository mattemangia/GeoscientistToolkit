...
namespace GeoscientistToolkit.Util
{
    ...
    public class TextureManager : IDisposable
    {
        ...
        private readonly Dictionary<IntPtr, IntPtr> _textureIdsByContext = new Dictionary<IntPtr, IntPtr>();
        ...
        /// </summary>
        public static TextureManager CreateFromPixelData(byte[] pixelData, uint width, uint height, PixelFormat format = PixelFormat.R8_G8_B8_A8_UNorm)
        {
        ...
        public IntPtr GetImGuiTextureId()
        {
            ...
            lock (_lock)
            {
                ...
                if (_textureIdsByContext.TryGetValue(currentContext, out IntPtr existingId))
                {
                    return existingId;
                }
        ...
        public void RemoveContextBinding(IntPtr context)
        {
            lock (_lock)
            {
                if (_textureIdsByContext.TryGetValue(context, out IntPtr textureId))
                {
                    ...
                    var controller = VeldridManager.GetCurrentImGuiController();
                    if (controller != null && _textureView != null)
                    {
                        controller.RemoveImGuiBinding(_textureView);
                    }
    ...
    {
        private static readonly Dictionary<string, TextureManager> _sharedTextures = new Dictionary<string, TextureManager>();
        ...
        public static TextureManager GetOrCreate(string key, Func<TextureManager> factory)
        {
            lock (_lock)
            {
                if (_sharedTextures.TryGetValue(key, out var existing))
                {
                    return existing;
                }
        ...
        public static void DisposeAll()
        {
            lock (_lock)
            {
                foreach (var texture in _sharedTextures.Values)
                {
                    texture.Dispose();
                }
