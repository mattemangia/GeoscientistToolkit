...
public static class ViewerScreenshotUtility
{
    private readonly record struct Request(Vector2 Position, Vector2 Size, string Path,
        ScreenshotUtility.ImageFormat Format, int Quality, Action<bool,string> Callback);
    ...
    {
        lock (Gate) Pending.Enqueue(new Request(viewerScreenPos, viewerSize, filePath, format, 90, callback));
    }
