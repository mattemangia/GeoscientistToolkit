...
public static class ViewerScreenshotUtility
{
    ...
    private static readonly Queue<DeferredCapture> _deferredCaptures = new();
        ...
        int jpegQuality = 90)
    {
        try
        {
            ...
            if (!_resourcesInitialized) InitializeResources(factory);

            // Create or resize composite texture to match viewer size
            var width = (uint)Math.Max(1, (int)viewerSize.X);
            var height = (uint)Math.Max(1, (int)viewerSize.Y);
    ...
    public static void ProcessDeferredCaptures()
    {
        while (_deferredCaptures.Count > 0)
        {
            ...
            var success = CaptureMainWindowRegion(
                capture.ScreenPos,
                capture.Size,
                capture.FilePath,
                capture.Format);
        ...
        int jpegQuality = 90)
    {
        try
        {
            ...
            var srcX = (uint)Math.Max(0, (int)viewerScreenPos.X);
            var srcY = (uint)Math.Max(0, (int)viewerScreenPos.Y);
            var width = (uint)Math.Min(backbuffer.Width - srcX, (int)viewerSize.X);
            var height = (uint)Math.Min(backbuffer.Height - srcY, (int)viewerSize.Y);
            ...
            if (_captureCommandList == null) _captureCommandList = factory.CreateCommandList();
            ...
            var result = ReadAndSaveTexture(stagingTexture, filePath, format, jpegQuality);
        ...
        ScreenshotUtility.ImageFormat format, int jpegQuality)
    {
        ...
        try
        {
            ...
            unsafe
            {
                var srcPtr = (byte*)mappedResource.Data.ToPointer();
                ...
                for (var y = 0; y < height; y++)
                {
                    ...
                    var srcY = y; // or (height - 1 - y) for flipped

                    for (var x = 0; x < width; x++)
                    {
                        var srcIdx = srcY * rowPitch + x * 4;
