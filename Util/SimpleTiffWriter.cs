...
namespace GeoscientistToolkit.Util
{
    ...
    public static class SimpleTiffWriter
    {
        ...
        public static void WriteTiff(Stream stream, byte[] rgbaData, int width, int height, bool isGrayscale = false)
        {
            ...
            {
                int samplesPerPixel = isGrayscale ? 1 : 4;
                int bytesPerRow = width * samplesPerPixel;
                int imageDataSize = height * bytesPerRow;
        ...
        private static void WriteDirectoryEntry(BinaryWriter writer, ushort tag, ushort type, uint count, uint valueOrOffset)
        {
            writer.Write(tag);
            writer.Write(type);
            writer.Write(count);
