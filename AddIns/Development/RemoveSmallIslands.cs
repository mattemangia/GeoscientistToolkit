...
namespace GeoscientistToolkit.AddIns.RemoveSmallIslands
{
    ...
    internal class RemoveSmallIslandsTool : AddInTool, IDisposable
    {
        ...
        private IslandAnalysisProcessor _processor;
        ...
        public void DrawPanel(CtImageStackDataset dataset)
        {
            ...
            {
                ImGui.SetTooltip("Shows a real-time 3D preview of the particles that will be removed (highlighted in red).");
            }
        ...
        private async Task ProcessIslandsAsync(CtImageStackDataset dataset, bool isPreviewOnly)
        {
            ...
            try
            {
                ...
                if (isPreviewOnly)
                {
                    ...
                    byte[] previewMask = await Task.Run(() =>
        ...
        private byte[] GeneratePreviewMask(int[,,] labelVolume, HashSet<int> smallParticleIds)
        {
            ...
            Parallel.For(0, depth, z =>
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    {
                        int label = labelVolume[x, y, z];
                        if (label > 0 && smallParticleIds.Contains(label))
                        {
        ...
        {
            int width = islandLabels.GetLength(0);
            int height = islandLabels.GetLength(1);
            int depth = islandLabels.GetLength(2);
            ...
            Parallel.For(0, depth, z =>
            {
                for (int y = 0; y < height; y++)
                {
                    for (int x = 0; x < width; x++)
                    {
                        int labelId = islandLabels[x, y, z];
                ...
                for (int y = 0; y < height; y++)
                    for (int x = 0; x < width; x++)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                unionFind.Union(minLabel, neighbor);
                            }
                        }
                    }
            ...
            Parallel.For(0, depth, new ParallelOptions { CancellationToken = token }, z =>
            {
                for (int y = 0; y < height; y++)
                    for (int x = 0; x < width; x++)
                    {
                        ...
                        if (label > 0)
                        {
                            particles.AddOrUpdate(label, 1, (key, count) => count + 1);
                        }
                    }
    ...
    internal class ConcurrentUnionFind
    {
        ...
        public void MakeSet(int x) => _parent.TryAdd(x, x);

        public int Find(int x)
        {
            if (!_parent.TryGetValue(x, out int parent))
            {
                MakeSet(x);
                return x;
            }
