...
namespace GeoscientistToolkit.AddIns.ParticleSeparator
{
    public class ParticleSeparatorAddIn : IAddIn
    {
        ...
        private static ParticleSeparatorAddIn _instance;

        public static ParticleSeparatorAddIn Instance => _instance;
        ...
        public static ParticleSeparatorTool GetTool() => _tool;
    }
    ...
    public class ParticleSeparatorTool : AddInTool, IDisposable
    {
        ...
        private AcceleratedProcessor _processor;
        ...
        private int _selectedZSlice = 0; // --- FIX: Added to store the selected slice for 2D processing ---

        // Visualization
        private Dictionary<int, Vector4> _particleColors = new Dictionary<int, Vector4>();
        ...
        public void DrawPanel(CtImageStackDataset dataset)
        {
            ...
            if (ImGui.CollapsingHeader("Processing Options", ImGuiTreeNodeFlags.DefaultOpen))
            {
                ...
                {
                    ImGui.SetTooltip("Process entire volume (3D) or current slice only (2D)");
                }
                ...
                {
                    ImGui.Indent();
                    int maxSlice = (dataset != null && dataset.Depth > 0) ? dataset.Depth - 1 : 0;
                ...
                if (ImGui.IsItemHovered())
                {
                    ImGui.SetTooltip("Conservative mode filters out very small particles");
                }

                if (_conservative)
                {
                    ImGui.DragFloat("Min Size (voxels)", ref _minParticleSize, 1.0f, 1.0f, 1000.0f);
                }

                // Thread count for CPU processing
                if (_selectedAcceleration == AccelerationType.CPU || _selectedAcceleration == AccelerationType.SIMD)
                {
                    int threadCount = _processor.ThreadCount;
            ...
            // Process Button
            bool canProcess = !_isProcessing && dataset.Materials != null && dataset.Materials.Count > 0;

            if (!canProcess) ImGui.BeginDisabled();

            string buttonText = _isProcessing ? "Processing..." : "Separate Particles";
            if (ImGui.Button(buttonText, new Vector2(200, 30)))
            {
        ...
        private void DrawResultsPanel(CtImageStackDataset dataset)
        {
                        ...
                        new Vector2(0, 200)))
                    {
                        ...
                        while (clipper.Step())
                        {
                            for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++)
                            {
                                ...
                                bool isSelected = _selectedParticleId == particle.Id;
            ...
            // Action Buttons
            if (ImGui.Button("Apply as New Materials"))
            {
                CreateMaterialsFromParticles(dataset);
            }
        ...
        private async Task ProcessParticlesAsync(CtImageStackDataset dataset)
        {
            ...
            try
            {
                ...
                if (_lastResult != null && _lastResult.Particles != null)
                {
                    Logger.Log($"[ParticleSeparator] Found {_lastResult.Particles.Count} particles in {_processor.LastProcessingTime:F2}s");
        ...
        private void GenerateParticleColors()
        {
            ...
            {
                float hue = (_random.Next(360)) / 360.0f;
                float saturation = 0.7f + _random.Next(30) / 100.0f;
                float value = 0.7f + _random.Next(30) / 100.0f;
        ...
        private void CreateMaterialsFromParticles(CtImageStackDataset dataset)
        {
            ...
            var sortedParticles = _lastResult.Particles.OrderByDescending(p => p.VoxelCount).ToList();
            int maxMaterials = Math.Min(10, sortedParticles.Count);
        ...
        private void ApplyParticleAsMaterial(CtImageStackDataset dataset, Particle particle, int[,,] labels, byte materialId)
        {
            Parallel.For(particle.Bounds.MinZ, particle.Bounds.MaxZ + 1, z =>
            {
                for (int y = particle.Bounds.MinY; y <= particle.Bounds.MaxY; y++)
                {
                    for (int x = particle.Bounds.MinX; x <= particle.Bounds.MaxX; x++)
                    {
    ...
    public unsafe class AcceleratedProcessor : IDisposable
    {
        ...
        private dynamic _gpuContext = null;
        private dynamic _gpuAccelerator = null;
            ...
            try
            {
                ...
                if (use3D)
                {
                    labels = SelectedAcceleration switch
                    {
                        ...
                        AccelerationType.SIMD when Vector.IsHardwareAccelerated => LabelComponents3DSIMD(mask, cancellationToken),
                ...
                long totalVoxels = (long)dataset.Width * dataset.Height * dataset.Depth;
        ...
        private int[,,] LabelComponents3DParallel(byte[,,] mask, CancellationToken cancellationToken)
        {
            ...
            int slabSize = Math.Max(1, depth / ThreadCount);

            Parallel.For(0, ThreadCount, new ParallelOptions { CancellationToken = cancellationToken },
            threadId =>
            {
                int startZ = threadId * slabSize;
                int endZ = (threadId == ThreadCount - 1) ? depth : (threadId + 1) * slabSize;
        ...
        private int[,,] LabelComponents3DSIMD(byte[,,] mask, CancellationToken cancellationToken)
        {
            ...
            Parallel.For(0, depth, new ParallelOptions { CancellationToken = cancellationToken }, z =>
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    {
                        // Load mask values
                        int[] maskValues = new int[vectorSize];
                        for (int i = 0; i < vectorSize; i++)
                        {
                            maskValues[i] = mask[x + i, y, z];
                        }
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    else
                    {
                        ...
                        if (left != top)
                        {
                            UnionLabels(equivalences, left, top);
                        }
                    }
                }
            }
        ...
        private int FindRoot(Dictionary<int, int> equivalences, int label)
        {
            ...
            int root = FindRoot(equivalences, equivalences[label]);
        ...
        private void ResolveEquivalencesParallel(int[,,] labels, ConcurrentUnionFind unionFind, CancellationToken cancellationToken)
        {
            int width = labels.GetLength(0);
            int height = labels.GetLength(1);
            int depth = labels.GetLength(2);
            ...
            Parallel.For(0, depth, new ParallelOptions { CancellationToken = cancellationToken }, z =>
            {
                ...
                for (int y = 0; y < height; y++)
                {
                    for (int x = 0; x < width; x++)
                    {
                        ...
                        if (!localParticles.ContainsKey(label))
                        {
                            localParticles[label] = new ParticleAccumulator { Id = label };
                        }
    ...
    {
        private readonly ConcurrentDictionary<int, int> _parent = new ConcurrentDictionary<int, int>();
        private readonly ConcurrentDictionary<int, int> _rank = new ConcurrentDictionary<int, int>();
