...
namespace GeoscientistToolkit.AddIns.Development
{
    ...
    public class FilterAddIn : IAddIn
    {
        ...
        public string Description => "Comprehensive CPU and GPU-accelerated image filters (Gaussian, Median, Mean, Non-Local Means, Edge Detection, Unsharp Masking) for CT data.";

        private readonly FilterTool _filterTool = new FilterTool();
        ...
        public IEnumerable<AddInTool> GetTools() => new[] { _filterTool };
    ...
    public class FilterTool : AddInTool, IDisposable
    {
        ...
        private GpuProcessor _gpuProcessor;
        ...
        public void DrawPanel(CtImageStackDataset dataset)
        {
            ...
            {
                ImGui.SetTooltip("If checked, applies the filter in 3D (e.g., 3x3x3 kernel).\nIf unchecked, applies a 2D filter to each Z-slice independently.");
            }
            ...
            if (ImGui.Button("Apply Filter", new Vector2(-1, 0)))
            {
                ...
                _ = System.Threading.Tasks.Task.Run(() => ApplyFilter(dataset));
            }
        ...
        private async System.Threading.Tasks.Task ApplyFilter(CtImageStackDataset dataset)
        {
            ...
            try
            {
                ...
                for (int z = 0; z < depth; z++)
                {
                    ...
                    System.Buffer.BlockCopy(resultData, z * width * height, sliceBuffer, 0, sliceBuffer.Length);
            ...
            catch (Exception ex)
            {
                ...
                Logger.LogError($"[FilterTool] ApplyFilter failed: {ex.ToString()}");
            }
            finally
            {
                _isProcessing = false;
        ...
        private void ApplyMedianCPU(byte[] source, byte[] result, int width, int height, int depth)
        {
            ...
            Parallel.For(0, depth, z =>
            {
                List<byte> neighbors = new List<byte>(_kernelSize * _kernelSize * (_process3D ? _kernelSize : 1));
        ...
        {
            int searchRadius = (int)_nlmSearchRadius;
            int patchRadius = (int)_nlmPatchRadius;
            float h = _sigma;
            float h2 = h * h;
                            ...
                            for (int sy = Math.Max(0, y - searchRadius); sy <= Math.Min(height - 1, y + searchRadius); sy++)
                                for (int sx = Math.Max(0, x - searchRadius); sx <= Math.Min(width - 1, x + searchRadius); sx++)
                                {
                                    ...
                                    for (int pz = pzMin; pz <= pzMax; pz++)
                                        for (int py = Math.Max(-patchRadius, -Math.Min(y, sy));
                                             py <= Math.Min(patchRadius, Math.Min(height - 1 - y, height - 1 - sy)); py++)
                                            for (int px = Math.Max(-patchRadius, -Math.Min(x, sx));
                                                 px <= Math.Min(patchRadius, Math.Min(width - 1 - x, width - 1 - sx)); px++)
                                            {
        ...
        private void ApplyCannyCPU(byte[] source, byte[] result, int width, int height, int depth)
        {
            ...
            byte[] gradients = new byte[source.Length];
            float[,,] gradX = new float[depth, height, width];
            float[,,] gradY = new float[depth, height, width];
            ...
            Parallel.For(0, depth, z =>
            {
                for (int y = 1; y < height - 1; y++)
                {
                    for (int x = 1; x < width - 1; x++)
                    {
                        ...
                        float angle = (float)Math.Atan2(gradY[z, y, x], gradX[z, y, x]);
                        ...
                        else if (val >= _cannyLowThreshold)
                        {
                            ...
                            for (int ky = -1; ky <= 1 && !hasStrongNeighbor; ky++)
                                for (int kx = -1; kx <= 1 && !hasStrongNeighbor; kx++)
                                {
                                    ...
                                    {
                                        long nIdx = (long)z * width * height + (long)ny * width + nx;
