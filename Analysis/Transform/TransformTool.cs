...
public class TransformTool : IDatasetTools, IDisposable
{
    ...
    private static readonly ConfirmationDialog _confirmApplyDialog = new(
        "Confirm Apply",
    ...
    public bool UniformCropFromCenter;
    ...
    public void Draw(Dataset dataset)
    {
        ...
        if (_currentDataset != ctDataset || _newVoxelCount == Vector3.Zero) InitializeForDataset(ctDataset);
        ...
        Overlay = _overlayMode == OverlayMode.Transform ? _transformOverlay : _cropOverlay;
        ...
        {
            if (ImGui.RadioButton("By Voxel Count", _resampleMode == ResampleMode.VoxelCount))
                _resampleMode = ResampleMode.VoxelCount;
            ImGui.SameLine();
            if (ImGui.RadioButton("By Voxel Size", _resampleMode == ResampleMode.VoxelSize))
                _resampleMode = ResampleMode.VoxelSize;
            ...
            if (ImGui.Button("Reset Resampling")) InitializeResampleParameters(_currentDataset);

            ImGui.TextDisabled(
                $"Resulting Voxel Size: {_newVoxelSize.X:F2}x{_newVoxelSize.Y:F2}x{_newVoxelSize.Z:F2} {_currentDataset.Unit}");
            ImGui.TextDisabled(
                $"Resulting Dimensions: {(int)_newVoxelCount.X}x{(int)_newVoxelCount.Y}x{(int)_newVoxelCount.Z}");
        }
        ...
        if (_confirmApplyDialog.Submit()) _ = ProcessAndApplyAsync();
    }

    public void Dispose()
    {
        if (_currentDataset != null) TransformIntegration.UnregisterTool(_currentDataset);
    }

    public (Vector3 min, Vector3 max) GetCropBounds()
    {
        return (_cropMin, _cropMax);
    }
    ...
    private void DrawCropMiniPreviews()
    {
        ...
        var h = 90f;
        DrawMini("XY preview", h, (x, y) => (x, y), _currentDataset.Width, _currentDataset.Height, _cropMin.X,
            _cropMax.X, _cropMin.Y, _cropMax.Y);
        ImGui.SameLine();
        DrawMini("XZ preview", h, (x, z) => (x, z), _currentDataset.Width, _currentDataset.Depth, _cropMin.X,
            _cropMax.X, _cropMin.Z, _cropMax.Z);
        ImGui.SameLine();
        DrawMini("YZ preview", h, (y, z) => (y, z), _currentDataset.Height, _currentDataset.Depth, _cropMin.Y,
            _cropMax.Y, _cropMin.Z, _cropMax.Z);
    }
    ...
    private async Task ProcessAndExportAsync()
    {
        ...
        if (transformedDataset != null) _exportDialog.Open(transformedDataset);
    }
    ...
    private async Task<CtImageStackDataset> CreateTransformedDatasetAsync()
    {
        ...
        // Destination
        var destWidth = (int)_newVoxelCount.X;
        var destHeight = (int)_newVoxelCount.Y;
        var destDepth = (int)_newVoxelCount.Z;

        var newGrayscaleVolume = new ChunkedVolume(destWidth, destHeight, destDepth);
        var newLabelVolume =
            new ChunkedLabelVolume(destWidth, destHeight, destDepth, newGrayscaleVolume.ChunkDim, false);
        ...
        await Task.Run(() =>
        {
            Parallel.For(0, destDepth, z =>
            {
                ...
                for (var x = 0; x < destWidth; x++)
                {
                    ...
                    var insideGray = sourcePos.X >= grayMin.X && sourcePos.X < grayMax.X &&
                                     sourcePos.Y >= grayMin.Y && sourcePos.Y < grayMax.Y &&
                                     sourcePos.Z >= grayMin.Z && sourcePos.Z < grayMax.Z;

                    var insideLab = sourcePos.X >= labMin.X && sourcePos.X < labMax.X &&
        ...
        var volumeDataField =
            newDataset.GetType().GetField("_volumeData", BindingFlags.NonPublic | BindingFlags.Instance);
    ...
    private void InitializeForDataset(CtImageStackDataset ds)
    {
        ...
        _cropLabels = true;
        _invertCrop = false;
        UniformCropFromCenter = false;
        _cropPaddingVox = Vector3.Zero;

        SnapEnabled = false;
        SnapTranslationStep = 1f;
        SnapRotationStep = 5f;
        SnapScaleStep = 0.05f;
    ...
    private void UpdateVoxelSizeFromCount()
    {
        ...
        {
            var originalAspectXY = (float)_currentDataset.Width / _currentDataset.Height;
            var newAspectXY = _newVoxelCount.X / _newVoxelCount.Y;
    ...
    private void UpdateVoxelCountFromSize()
    {
        if (_lockAspectRatio && _currentDataset != null)
        {
            var originalAspectXY = 1f;
            var newAspectXY = _newVoxelSize.X / _newVoxelSize.Y;
            if (Math.Abs(originalAspectXY - newAspectXY) > 0.01f) _newVoxelSize.Y = _newVoxelSize.X / originalAspectXY;
        }
    ...
    // Presets
    private Preset MakePresetFromCurrent()
    {
        return new Preset
        {
            ...
            SnapScale = SnapScaleStep, SnapCropX = SnapCropVoxStepX, SnapCropY = SnapCropVoxStepY,
            SnapCropZ = SnapCropVoxStepZ,
    ...
    private void LoadPreset(Preset p)
    {
        ...
        _cropMin = p.CropMin;
        _cropMax = p.CropMax;
        _cropGrayscale = p.CropGray;
        _cropLabels = p.CropLabels;
        _invertCrop = p.InvertCrop;
        UniformCropFromCenter = p.UniformFromCenter;
        SnapEnabled = p.SnapOn;
        SnapTranslationStep = p.SnapTr;
        SnapRotationStep = p.SnapRot;
        SnapScaleStep = p.SnapScale;
        SnapCropVoxStepX = p.SnapCropX;
        SnapCropVoxStepY = p.SnapCropY;
        SnapCropVoxStepZ = p.SnapCropZ;
        _cropPaddingVox = p.PaddingVox;
        _resampleMode = p.ByCount ? ResampleMode.VoxelCount : ResampleMode.VoxelSize;
        _newVoxelCount = p.NewCount;
        _newVoxelSize = p.NewVoxelSize;
    }
    ...
    private class Preset
    {
        ...
        public Vector3 NewCount;
        public Vector3 NewVoxelSize;
