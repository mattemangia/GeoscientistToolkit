...
public class ReactionGenerator
{
    ...
    public Dictionary<string, int> ParseChemicalFormula(string formula)
    {
        ...
        {
            var waterMoles = string.IsNullOrEmpty(hydrationMatch.Groups[1].Value) ? 1 : int.Parse(hydrationMatch.Groups[1].Value);
    ...
    private string ExpandParentheses(string formula)
    {
        ...
        while (formula.Contains('('))
        {
            ...
            {
                string element = em.Groups[1].Value;
                int count = string.IsNullOrEmpty(em.Groups[2].Value) ? 1 : int.Parse(em.Groups[2].Value);
    ...
    List<ChemicalCompound> possibleProducts)
{
    ...
    try
    {
        ...
        for (int i = 0; i < Math.Min(nElements, nProducts); i++)
        {
        ...
        {
            Logger.LogWarning($"[ReactionGenerator] Large residual {residualNorm:E2} when balancing {mineral.Name}");
        ...
        for (int i = 0; i < stoichiometry.Count; i++)
        {
            if (Math.Abs(stoichiometry[i]) < 1e-9)
            {
                stoichiometry[i] = 0.0;
            }
        ...
        List<ChemicalCompound> products, ChemicalCompound mineral)
    {
        ...
        int primaryIdx = FindPrimaryProduct(products, mineral);
        ...
        var svd = A_reduced.Svd(computeVectors: true);
...
private double RoundToRationalFraction(double value)
{
    ...
    {
        for (int numer = 1; numer < denom * 3; numer++)
        {
            var fraction = (double)numer / denom;
            if (Math.Abs(value - fraction) < 1e-4)
            {
                return fraction;
            }
            if (Math.Abs(value + fraction) < 1e-4)
    ...
    List<ChemicalCompound> products, List<string> elements)
{
    ...
    foreach (var (species, stoich) in reaction.Stoichiometry)
    {
        ...
        if (compound != null)
        {
            totalCharge += stoich * (compound.IonicCharge ?? 0);
        }
    }
    ...
    if (Math.Abs(totalCharge) > 0.01)
    {
        ...
        {
            reaction.Stoichiometry[hPlus.Name] = reaction.Stoichiometry.GetValueOrDefault(hPlus.Name, 0) - totalCharge;
        }
        else if (totalCharge > 0 && ohMinus != null)
        {
            reaction.Stoichiometry[ohMinus.Name] = reaction.Stoichiometry.GetValueOrDefault(ohMinus.Name, 0) + totalCharge;
        }
    }
}
/// <summary>
    ...
    double H_deficit, double O_deficit)
{
    ...
    if (ohMinus != null)
    {
        reaction.Stoichiometry[ohMinus.Name] = OH_count;
    }
}
    ...
    double H_deficit, double O_deficit)
{
    ...
    if (silicicAcid != null && Si_count > 0)
    {
        reaction.Stoichiometry[silicicAcid.Name] = Si_count;
    }
    ...
    if (H_needed > 0 && hPlus != null)
    {
        reaction.Stoichiometry[hPlus.Name] = -H_needed; // Consumed (reactant)
    }
    ...
    double H_deficit, double O_deficit)
{
    ...
    if (Math.Abs(H_deficit) > 0.01 && water != null)
    {
        ...
        if (formula.Contains("·"))
        {
            ...
            {
                var waterMoles = string.IsNullOrEmpty(waterMatch.Groups[1].Value) ? 1 : int.Parse(waterMatch.Groups[1].Value);
    ...
    List<ChemicalCompound> basisSpecies)
{
    ...
    try
    {
        ...
        for (int j = 0; j < nBasis; j++)
        {
            ...
            {
                reaction.Stoichiometry[basisSpecies[j].Name] = -coeff; // Reactant
            }
        }
    }
    catch (Exception ex)
    {
        Logger.LogError($"[ReactionGenerator] Failed to balance complexation for {secondarySpecies.Name}: {ex.Message}");
    }
}
/// <summary>
...
public List<ChemicalReaction> GenerateComplexationReactions(ThermodynamicState state)
{
    ...
    foreach (var secondary in secondarySpecies)
    {
        try
        {
            ...
            if (reaction != null && reaction.Stoichiometry.Count > 1)
            {
                reactions.Add(reaction);
            }
        }
        ...
        {
            Logger.LogWarning($"[ReactionGenerator] Failed to generate reaction for {secondary.Name}: {ex.Message}");
        }
    }
    ...
    List<ChemicalCompound> basisSpecies)
{
    ...
    try
    {
        ...
        for (int j = 0; j < nBasis; j++)
        {
            ...
            if (Math.Abs(coeff) > 1e-9)
            {
                reaction.Stoichiometry[basisSpecies[j].Name] = -coeff;
            }
        }
    ...
    List<ChemicalCompound> basisSpecies, ChemicalCompound secondary)
{
    ...
    if (reducedBasis.Count == 0)
    {
        ...
        {
            result[fixedIndices[i]] = fixedValues[i];
        }
        return result;
    }
...
private bool VerifyReactionBalance(ChemicalReaction reaction)
{
    ...
    public List<ChemicalReaction> GenerateRedoxReactions(ThermodynamicState state)
    {
        ...
        if (electron == null)
        {
            Logger.LogWarning("[ReactionGenerator] Electron (e⁻) not defined in library. Cannot generate redox reactions.");
            return reactions;
        }
        ...
        foreach (var elementSymbol in redoxElements)
        {
            var speciesOfElement = _compoundLibrary.Compounds
                .Where(c => c.Phase == CompoundPhase.Aqueous && c.OxidationState.HasValue && ParseChemicalFormula(c.ChemicalFormula).ContainsKey(elementSymbol))
                .OrderBy(c => c.OxidationState.Value)
                .ToList();

            for (int i = 0; i < speciesOfElement.Count - 1; i++)
            {
                var reduced = speciesOfElement[i];
                var oxidized = speciesOfElement[i + 1];

                int e_diff = oxidized.OxidationState.Value - reduced.OxidationState.Value;
                if (e_diff > 0)
                {
                    var reaction = new ChemicalReaction
                    {
    ...
    {
        var reactions = new List<ChemicalReaction>();
        var gasSpecies = _compoundLibrary.Compounds.Where(c => c.Phase == CompoundPhase.Gas && c.HenrysLawConstant_mol_L_atm.HasValue).ToList();
    ...
    public List<ChemicalReaction> GenerateSurfaceComplexationReactions(ThermodynamicState state)
    {
        ...
        var aqueousIons = _compoundLibrary.Compounds.Where(c => c.Phase == CompoundPhase.Aqueous && c.IonicCharge.HasValue).ToList();
        var hPlus = _compoundLibrary.Find("H⁺");
        ...
        foreach (var site in primarySites)
        {
            ...
            if (protonated != null)
                reactions.Add(CreateSurfaceReaction($"{site.Name} Protonation", new[] { site.Name, hPlus.Name }, new[] { protonated.Name }));

            // >SOH <=> >SO- + H+
            var deprotonated = surfaceSpecies.FirstOrDefault(s => s.ChemicalFormula == site.ChemicalFormula.Replace("OH", "O⁻"));
            if (deprotonated != null)
                reactions.Add(CreateSurfaceReaction($"{site.Name} Deprotonation", new[] { site.Name }, new[] { deprotonated.Name, hPlus.Name }));
            ...
            foreach (var cation in aqueousIons.Where(i => i.IonicCharge > 0))
            {
                 ...
                 if (cationComplex != null)
                     reactions.Add(CreateSurfaceReaction($"{site.Name}-{cation.Name} Binding", new[] { site.Name, cation.Name }, new[] { cationComplex.Name, hPlus.Name }));
            }
        }
        return reactions;
    }
