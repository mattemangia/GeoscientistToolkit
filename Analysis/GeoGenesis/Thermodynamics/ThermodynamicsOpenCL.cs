// ported from GeoscientistToolkit/Business/Thermodynamics/ThermodynamicsOpenCL.cs
//
...
public class ThermodynamicsOpenCL : IDisposable
{
    ...
    private unsafe void InitializeOpenCL()
    {
        try
        {
            ...
            _multiGPUMode = GAIA.GeoGenesis.Compute.OpenCLDeviceManager.IsMultiGPUEnabled();

            if (_multiGPUMode)
            {
                // Multi-GPU mode: get all GPU devices
                var deviceInfos = GAIA.GeoGenesis.Compute.OpenCLDeviceManager.GetAllComputeDevices();
            ...
            {
                // Single GPU mode
                _device = GAIA.GeoGenesis.Compute.OpenCLDeviceManager.GetComputeDevice();
                ...
                var deviceInfo = GAIA.GeoGenesis.Compute.OpenCLDeviceManager.GetDeviceInfo();
