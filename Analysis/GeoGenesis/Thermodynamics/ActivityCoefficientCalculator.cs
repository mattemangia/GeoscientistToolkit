...
public class ActivityCoefficientCalculator
{
    ...
    public void CalculateActivityCoefficients(ThermodynamicState state)
    {
        ...
        if (state.SurfaceSites.Any()) CalculateSurfaceActivities(state);
    }
    ...
    ///
    ///     FIX (GAIA.GeoGenesis): the original first-principles expressions were dimensionally
    ...
    private (double epsilon, double rho_kg_m3) GetWaterPropertiesAccurate(double T_K)
    {
        ...
        //
        // FIX (GAIA.GeoGenesis): the original port carried a mistranscribed Bradley–Pitzer
        ...
        List<(string name, double m, int z)> anions)
    {
        ...
        foreach (var (cation, m_c, _) in cations)
        {
            ...
            if (_lambdaParams.TryGetValue(key, out var lambda)) lnGamma += 2 * lambda * m_c;
        }
        ...
        foreach (var (anion, m_a, _) in anions)
        {
            ...
            if (_lambdaParams.TryGetValue(key, out var lambda)) lnGamma += 2 * lambda * m_a;
        }
    ...
    private void CalculateSurfaceActivities(ThermodynamicState state)
    {
        ...
        if (state.IonicStrength_molkg < 1e-12)
        {
            ...
            {
                var concentration = site.SpeciesMoles[speciesName] / (site.Mass_g / 1000.0);
        ...
        var epsilon = epsilon_r * PERMITTIVITY_VACUUM;

        // --- COMPLETE: Calculate actual surface area from mineral properties ---
        var totalSurfaceArea_m2 = 0.0;
        ...
        var totalCharge_C = state.SurfaceCharge_mol_L.Value * FARADAY_CONSTANT * state.Volume_L;
        var sigma_C_per_m2 = totalCharge_C / totalSurfaceArea_m2;

        // --- Convert ionic strength to volumetric concentration ---
        var I_conc_mol_per_m3 = I * rho_w;

        // --- Solve Gouy-Chapman equation for surface potential ---
        var denominator = Math.Sqrt(8 * R_GAS_CONSTANT * T * epsilon * I_conc_mol_per_m3);
        ...
        {
            var argument = sigma_C_per_m2 / denominator;
        ...
        foreach (var site in state.SurfaceSites)
        {
            ...
            var mineral = _compoundLibrary.Find(site.MineralName);
            var capacitance = mineral?.SurfaceCapacitance_F_m2 ?? 1.0; // F/m²
            ...
            foreach (var speciesName in site.SpeciesMoles.Keys)
            {
                ...
                var charge = compound.IonicCharge ?? 0;
                ...
                var effectivePotential = CalculateEffectivePotential(psi, charge, compound, mineral, state);

                var lnGamma = -charge * FARADAY_CONSTANT * effectivePotential / (R_GAS_CONSTANT * T);
                var gamma = Math.Exp(lnGamma);

                // Activity = concentration × activity coefficient
                var concentration_mol_per_kg_solid = site.SpeciesMoles[speciesName] / (site.Mass_g / 1000.0);
        ...
        ChemicalCompound mineral, ThermodynamicState state)
    {
        ...
        var psi_beta = psi0 * Math.Exp(-kappa * sternThickness * 1e-9);
        return psi_beta;
    }
    ...
    private double CalculateDebyeLength(ThermodynamicState state)
    {
        ...
        var I_mol_per_L = state.IonicStrength_molkg * (rho_w / 1000.0);
        ...
        var I_mol_per_m3 = I_mol_per_L * 1000.0;

        var kappa = Math.Sqrt(2.0 * FARADAY_CONSTANT * FARADAY_CONSTANT * I_mol_per_m3 /
                              (epsilon * R_GAS_CONSTANT * T));

        // Debye length λ_D = 1/κ (typical range: 0.3 nm at 1 M to 30 nm at 0.001 M)
        var debyeLength_nm = 1e9 / kappa;
    ...
    private double EstimateSpecificSurfaceArea(ChemicalCompound mineral)
    {
        ...
        if (formula.Contains("Al") && formula.Contains("O") && !formula.Contains("Si"))
            // Aluminum oxides
            return 100.0; // m²/g (γ-alumina)

        if (formula.Contains("Mn") && formula.Contains("O"))
            // Manganese oxides
            return 200.0; // m²/g (birnessite)

        if (formula.Contains("Si") && formula.Contains("Al"))
            // Clay minerals
