// ported from GeoscientistToolkit/Business/Thermodynamics/KineticsSolver.cs
//
...
public class KineticsSolver : SimulatorNodeSupport
{
        ...
        double totalTime_s, List<ChemicalReaction> reactions)
    {
        ...
        for (var i = 0; i < elements.Count; i++) y[i] = state.ElementalComposition[elements[i]];
        ...
        while (time < totalTime_s)
        {
            ...
            var order = Math.Min(3, bdfHistory.Count);
            ...
            if (bdfHistory.Count > 3) bdfHistory.Dequeue();
            ...
            if (iteration > 5 && iteration % 5 == 0)
            {
                ...
                else if (errorEstimate < 1e-8) dt = Math.Min(dt * 1.5, timeStep_s * 2.0); // Increase step
            }
        ...
        List<ChemicalReaction> reactions, List<string> elements)
    {
        ...
        for (var i = 0; i < alpha.Length && i < historyArray.Length; i++)
        ...
        for (var iter = 0; iter < maxNewtonIter; iter++)
        {
            ...
            // Ensure positivity
            for (var i = 0; i < yNew.Count; i++) yNew[i] = Math.Max(yNew[i], 0);
        }

        if (!converged) Logger.LogWarning("[KineticsSolver] BDF Newton iteration did not converge");
        ...
        foreach (var reaction in reactions)
        {
            ...
            foreach (var (species, stoich) in reaction.Stoichiometry)
            {
                ...
                foreach (var (element, elementStoich) in formula)
                {
                    ...
                    if (idx >= 0) rates[idx] += rate * stoich * elementStoich;
                }
            }
        }
    ...
    {
        for (var i = 0; i < elements.Count; i++) state.ElementalComposition[elements[i]] = Math.Max(0, y[i]);
    }
    ...
    private double CalculateMineralReactionRate(ChemicalReaction reaction, ThermodynamicState state)
    {
        ...
        var saturationIndex =
            _equilibriumSolver.CalculateSaturationIndices(state).GetValueOrDefault(mineral.Name, -999);
        ...
        var isDissolution = reaction.Stoichiometry[mineral.Name] < 0;

        var k0 = isDissolution
            ? mineral.RateConstant_Dissolution_mol_m2_s
            : mineral.RateConstant_Precipitation_mol_m2_s;
        var Ea = isDissolution
            ? mineral.ActivationEnergy_Dissolution_kJ_mol
            : mineral.ActivationEnergy_Precipitation_kJ_mol;
        ...
        Dictionary<ChemicalReaction, double> reactionRates, List<ChemicalReaction> kineticReactions)
    {
        ...
        foreach (var elem in state.ElementalComposition.Keys) elementRates[elem] = 0.0;
    ...
    private ThermodynamicState CloneState(ThermodynamicState state)
    {
        return new ThermodynamicState
        {
            ...
            SurfaceSites = state.SurfaceSites.Select(s => new SurfaceSite
            {
                MineralName = s.MineralName, Mass_g = s.Mass_g,
