...
public partial class GeomechanicalSimulatorCPU
{
    ...
    private void ValidateParameters()
    {
        ...
        {
            Logger.LogWarning($"[GeomechCPU] Tolerance {_params.Tolerance} is quite loose. Recommend 1e-6 for accuracy.");
        }
        ...
        IProgress<float> progress, CancellationToken token)
    {
        ...
        try
        {
            ...
            ApplyBoundaryConditionsAndLoading(labels);  // FIX: Pass labels for surface detection
            token.ThrowIfCancellationRequested();
            ...
            // FIX: Calculate global statistics
    ...
    private void InitializeChunking(byte[,,] labels, float[,,] density)
    {
        ...
        long nodesPerChunk = w * h * 32L;
        long bytesPerNode = 3 * sizeof(float) + 3 * sizeof(int);
        long elementBytesPerChunk = nodesPerChunk * 8 * (2 * sizeof(float) + 8 * sizeof(int));
        long chunkMemory = nodesPerChunk * bytesPerNode + elementBytesPerChunk;
        ...
        Logger.Log($"[GeomechCPU] Created {_chunks.Count} chunks ({slicesPerChunk} slices each with {CHUNK_OVERLAP} overlap)");
    }
    ...
    private float[,] ComputeElasticityMatrix(float E, float nu)
    {
        ...
        var D = new float[6, 6];
        D[0, 0] = lambda_plus_2mu; D[0, 1] = lambda; D[0, 2] = lambda;
        D[1, 0] = lambda; D[1, 1] = lambda_plus_2mu; D[1, 2] = lambda;
        D[2, 0] = lambda; D[2, 1] = lambda; D[2, 2] = lambda_plus_2mu;
        D[3, 3] = mu;
        D[4, 4] = mu;
        D[5, 5] = mu;
    ...
    private void ApplyBoundaryConditionsAndLoading(byte[,,] labels)
    {
        ...
        float GetNodalTributaryArea(int x, int y, int z, int normalDir)
        {
            ...
            for (int dy = -1; dy <= 0; dy++)
            {
                for (int dx = -1; dx <= 0; dx++)
                {
                    ...
                    if (normalDir == 0) { ex = x; ey = y + dy; ez = z + dx; } // YZ face
                    if (normalDir == 1) { ex = x + dx; ey = y; ez = z + dy; } // XZ face
        ...
        for (var y = 0; y < h; y++)
        {
            for (var x = 0; x < w; x++)
            {
                ...
                if (tributaryArea > 0)
                {
                    _force[dofZ] -= sigma1_Pa * tributaryArea;  // FIX: Proper force calculation
                }
            }
        }
    ...
    private bool SolveDisplacements(IProgress<float> progress, CancellationToken token)
    {
        ...
        while (iter < maxIter && !converged)
        {
            ...
            {
                converged = true;
                Logger.Log($"[GeomechCPU] PCG converged at iteration {iter} with relative residual {relativeResidual:E4}");
                break;
            }
    ...
    private void ProcessChunkStrainStressNodal(GeomechanicalChunk chunk, float[,] nodalStress, int[] nodalCount)
    {
        Parallel.For(0, _numElements, e =>
        {
            ...
            int GetLockIndex(int nodeIdx) => nodeIdx % NUM_STRESS_LOCKS;
                        ...
                        case FailureCriterion.Griffith:
                            if (sigma3 < 0)
                                ...
                                failureIndex = tensileStrength_Pa * 8 > 1e-9f ?
                                    (sigma1 - sigma3) / (8 * tensileStrength_Pa) : sigma1 - sigma3;
                            break;
                    }
