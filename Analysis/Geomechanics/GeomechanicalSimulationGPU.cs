...
public unsafe partial class GeomechanicalSimulatorGPU : IDisposable
{
    ...
    private readonly object _chunkAccessLock = new object();
    private Dictionary<int, ChunkGPUBuffers> _chunkBuffers = new();
    ...
    private void DetectGPUMemory()
    {
        ...
        {
            nuint gpuMemSize;
            _cl.GetDeviceInfo(_device, DeviceInfo.GlobalMemSize, (nuint)sizeof(ulong), &gpuMemSize, null);
...
void compute_jacobian(__local const float* dN_dxi, __global const float* nodeX, __global const float* nodeY,
    ...
    for (int i = 0; i < 8; i++) {
        int nodeIdx = nodes[i];
        J[0] += dN_dxi[i*3 + 0] * nodeX[nodeIdx];
        J[1] += dN_dxi[i*3 + 0] * nodeY[nodeIdx];
        J[2] += dN_dxi[i*3 + 0] * nodeZ[nodeIdx];
        J[3] += dN_dxi[i*3 + 1] * nodeX[nodeIdx];
        J[4] += dN_dxi[i*3 + 1] * nodeY[nodeIdx];
        J[5] += dN_dxi[i*3 + 1] * nodeZ[nodeIdx];
        J[6] += dN_dxi[i*3 + 2] * nodeX[nodeIdx];
        J[7] += dN_dxi[i*3 + 2] * nodeY[nodeIdx];
        J[8] += dN_dxi[i*3 + 2] * nodeZ[nodeIdx];
    }
}
...
__kernel void assemble_element_stiffness(__global const int* elementNodes, __global const float* elementE,
    __global const float* elementNu, __global const float* nodeX, __global const float* nodeY,
    __global const float* nodeZ, __global const int* rowPtr, __global const int* colIdx,
    __global float* values, const int numElements)
{
    ...
    for (int i = 0; i < 8; i++) nodes[i] = elementNodes[e*8 + i];
    ...
    for (int gp_idx = 0; gp_idx < 8; gp_idx++) {
        ...
        inverse3x3(J, Jinv);
        for (int i = 0; i < 8; i++) {
            dN_dx[i*3 + 0] = Jinv[0]*dN_dxi[i*3+0] + Jinv[1]*dN_dxi[i*3+1] + Jinv[2]*dN_dxi[i*3+2];
            dN_dx[i*3 + 1] = Jinv[3]*dN_dxi[i*3+0] + Jinv[4]*dN_dxi[i*3+1] + Jinv[5]*dN_dxi[i*3+2];
            dN_dx[i*3 + 2] = Jinv[6]*dN_dxi[i*3+0] + Jinv[7]*dN_dxi[i*3+1] + Jinv[8]*dN_dxi[i*3+2];
        }
        ...
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 24; j++) {
                float sum = 0.0f;
                for (int k = 0; k < 6; k++) sum += D[i*6 + k] * B[k*24 + j];
                DB[i*24 + j] = sum;
            }
        }
        ...
        for (int i = 0; i < 24; i++) {
            for (int j = 0; j < 24; j++) {
                float sum = 0.0f;
                for (int k = 0; k < 6; k++) sum += B[k*24 + i] * DB[k*24 + j];
...
__kernel void apply_boundary_conditions(__global float* force, __global const uchar* isDirichlet,
    __global const float* dirichletValue, const int numDOFs)
{
    int i = get_global_id(0);
    if (i >= numDOFs) return;
    if (isDirichlet[i]) force[i] = dirichletValue[i];
}

__kernel void sparse_matvec(__global const int* rowPtr, __global const int* colIdx,
    __global const float* values, __global const float* x, __global float* y,
    __global const uchar* isDirichlet, const int numRows)
{
    ...
    for (int j = rowStart; j < rowEnd; j++) {
        int col = colIdx[j];
        if (!isDirichlet[col]) sum += values[j] * x[col];
    }
    y[row] = sum;
}

__kernel void dot_product(__global const float* a, __global const float* b,
    __global float* partial_sums, __local float* scratch, const int n)
{
    int gid = get_global_id(0);
    int lid = get_local_id(0);
    int group_size = get_local_size(0);
    float sum = 0.0f;
    if (gid < n) sum = a[gid] * b[gid];
    scratch[lid] = sum;
    ...
    if (lid == 0) partial_sums[get_group_id(0)] = scratch[0];
}

__kernel void vector_ops(__global float* y, __global const float* x,
    __global const uchar* isDirichlet, const float alpha, const int op_type, const int n)
{
    int i = get_global_id(0);
    if (i >= n) return;
    if (isDirichlet[i] && op_type != 2) return;
    switch (op_type) {
        case 0: y[i] += alpha * x[i]; break;
        case 1: y[i] = x[i] + alpha * y[i]; break;
        case 2: y[i] *= alpha; break;
    }
}

__kernel void calculate_strains_stresses(__global const int* elementNodes, __global const float* elementE,
    __global const float* elementNu, __global const float* nodeX, __global const float* nodeY,
    __global const float* nodeZ, __global const float* displacement, __global float* stressXX,
    __global float* stressYY, __global float* stressZZ, __global float* stressXY,
    __global float* stressXZ, __global float* stressYZ, const int numElements,
    const int width, const int height, const float dx)
{
    ...
    for (int i = 0; i < 6; i++) {
        strain[i] = 0.0f;
        for (int j = 0; j < 24; j++) strain[i] += B[i*24 + j] * ue[j];
    }
    ...
    for (int i = 0; i < 6; i++) {
        stress[i] = 0.0f;
        for (int j = 0; j < 6; j++) stress[i] += D[i*6 + j] * strain[j];
    }
...
__kernel void calculate_principal_stresses(__global const float* stressXX, __global const float* stressYY,
    __global const float* stressZZ, __global const float* stressXY, __global const float* stressXZ,
    __global const float* stressYZ, __global float* sigma1, __global float* sigma2,
    __global float* sigma3, __global const uchar* labels, const int numVoxels)
{
    ...
    float I3 = sxx*syy*szz + 2*sxy*sxz*syz - sxx*syz*syz - syy*sxz*sxz - szz*sxy*sxy;
...
__kernel void evaluate_failure(__global const float* sigma1, __global const float* sigma2,
    __global const float* sigma3, __global float* failureIndex, __global uchar* damage,
    __global uchar* fractured, __global const uchar* labels, const float cohesion,
    const float frictionAngle, const float tensileStrength, const int failureCriterion,
    const int numVoxels)
{
    int idx = get_global_id(0);
    if (idx >= numVoxels) return;
    ...
    switch (failureCriterion) {
        case 0: {
            float left = s1 - s3;
            float right = 2*cohesion*cos(phi) + (s1 + s3)*sin(phi);
            fi = (right > 1e-9f) ? left / right : left;
            break;
        }
        case 1: {
            float p = (s1 + s3) / 2;
            float q = (s1 - s3) / 2;
            float alpha = 2*sin(phi) / (3 - sin(phi));
            float k = 6*cohesion*cos(phi) / (3 - sin(phi));
            fi = (k > 1e-9f) ? (q - alpha*p) / k : q - alpha*p;
            break;
        }
        case 3: {
            if (s3 < 0) fi = -s3 / tensileStrength;
            else fi = (s1 - s3) / (8*tensileStrength);
            break;
        }
    }
    ...
    if (fi >= 1.0f) {
        ...
        damage[idx] = (uchar)((fi - 0.8f) / 0.2f * 255);
    }
}
";
    }
        ...
        IProgress<float> progress, CancellationToken token)
    {
        ...
        try
        {
            Logger.Log("[GeomechGPU] Starting GPU-accelerated simulation");
            ...
            Logger.Log($"[GeomechGPU] Simulation complete: {results.ComputationTime.TotalSeconds:F2}s");
            
            return results;
        }
        catch (Exception ex)
        {
            Logger.LogError($"[GeomechGPU] Simulation failed: {ex.Message}");
    ...
    private void AssembleStiffnessMatrixGPU(IProgress<float> progress, CancellationToken token)
    {
        ...
        nuint globalSize = (nuint)((_numElements + _workGroupSize - 1) / _workGroupSize * _workGroupSize);
        nuint localSize = (nuint)_workGroupSize;
        
        var error = _cl.EnqueueNdrangeKernel(_queue, _kernelAssembleElement, 1, null, &globalSize, &localSize, 0, null, null);
    ...
    private void ApplyBoundaryConditionsGPU(byte[,,] labels)
    {
        ...
        nuint globalSize = (nuint)((_numDOFs + _workGroupSize - 1) / _workGroupSize * _workGroupSize);
        nuint localSize = (nuint)_workGroupSize;
    ...
    private void CalculateStrainsAndStressesGPU()
    {
        ...
        error = _cl.EnqueueNdrangeKernel(_queue, _kernelCalculateStrains, 1, null, &globalSize, &localSize, 0, null, null);
    ...
    private void CalculatePrincipalStressesGPU(byte[,,] labels)
    {
        ...
        nuint globalSize = (nuint)((numVoxels + _workGroupSize - 1) / _workGroupSize * _workGroupSize);
        nuint localSize = (nuint)_workGroupSize;
        
        error = _cl.EnqueueNdrangeKernel(_queue, _kernelCalculatePrincipal, 1, null, &globalSize, &localSize, 0, null, null);
    ...
    private float DotProductGPU(nint a, nint b)
    {
        ...
        nuint globalSize = (nuint)(numWorkGroups * _workGroupSize);
        nuint localSize = (nuint)_workGroupSize;
    ...
    {
        _cl.SetKernelArg(kernel, (uint)index, (nuint)sizeof(int), &value);
    }

    private void SetKernelArg(nint kernel, int index, float value)
    {
        _cl.SetKernelArg(kernel, (uint)index, (nuint)sizeof(float), &value);
    }
    ...
    private void Cleanup()
    {
        ...
        {
            try { Directory.Delete(_offloadPath, true); }
            catch { }
        }
    }
