        ...
        IProgress<float> progress, CancellationToken token)
    {
        ...
        for (var step = 0; step < numSteps; step++)
        {
            ...
            if (!breakdownDetected && newFractureCount > 0)
            {
                ...
                Logger.Log($"[GeomechCPU] *** BREAKDOWN at t={breakdownTime:F1}s, P={results.BreakdownPressure:F1} MPa ***");
            }
        ...
        Logger.Log($"[GeomechCPU] Fluid injection complete. Final fracture volume: {results.TotalFractureVolume:F4} m³");
    }

    private void ApplyInjectionSource(int x, int y, int z, float P_inj, float Q_inj, float dt, float dx, byte[,,] labels)
    {
    ...
    private void EnhancedDiffusionThroughFractures(byte[,,] labels, float dx, float dt)
{
    ...
    Parallel.For(1, d - 1, z =>
    {
        ...
        for (var x = 1; x < w - 1; x++)
        {
            ...
            {
                // Check neighbors
                if (x > 0 && _fractureAperture[x-1, y, z] > _params.MinimumFractureAperture) is_in_fracture_zone = true;
                if (x < w-1 && _fractureAperture[x+1, y, z] > _params.MinimumFractureAperture) is_in_fracture_zone = true;
                if (y > 0 && _fractureAperture[x, y-1, z] > _params.MinimumFractureAperture) is_in_fracture_zone = true;
                if (y < h-1 && _fractureAperture[x, y+1, z] > _params.MinimumFractureAperture) is_in_fracture_zone = true;
                if (z > 0 && _fractureAperture[x, y, z-1] > _params.MinimumFractureAperture) is_in_fracture_zone = true;
                if (z < d-1 && _fractureAperture[x, y, z+1] > _params.MinimumFractureAperture) is_in_fracture_zone = true;
            }
            ...
            if (x + 1 < w && labels[x+1, y, z] != 0)
            {
                ...
                var aperture_interface = 2f / (1f / (aperture_c + 1e-12f) + 1f / (aperture_xp + 1e-12f)); // Harmonic mean
            ...
            for (var dir = 0; dir < 3; dir++)
            {
                ...
                var segment = new FractureSegment
                {
                    ...
                    Temperature = _temperature != null ? 
                        (_temperature[x, y, z] + _temperature[nx, ny, nz]) / 2f : 0f,
