...
public class AcousticSimulatorCPU : IAcousticKernel
{
        ...
        float dt, float dx, int w, int h, int d)
    {
        ...
        Parallel.For(1, d - 1, z =>
        {
            ...
            {
                var E_local = E[x, y, z];
                var nu_local = Math.Clamp(nu[x, y, z], 0.01f, 0.49f); // Prevent singularities
                ...
                var dvx_dx = (vx[x + 1, y, z] - vx[x - 1, y, z]) / (2f * dx);
                var dvy_dy = (vy[x, y + 1, z] - vy[x, y - 1, z]) / (2f * dx);
                var dvz_dz = (vz[x, y, z + 1] - vz[x, y, z - 1]) / (2f * dx);

                // Volumetric strain
                var div_v = dvx_dx + dvy_dy + dvz_dz;
                ...
                // Shear strains
                var dvx_dy = (vx[x, y + 1, z] - vx[x, y - 1, z]) / (2f * dx);
                var dvy_dx = (vy[x + 1, y, z] - vy[x - 1, y, z]) / (2f * dx);
                var dvx_dz = (vx[x, y, z + 1] - vx[x, y, z - 1]) / (2f * dx);
                var dvz_dx = (vz[x + 1, y, z] - vz[x - 1, y, z]) / (2f * dx);
                var dvy_dz = (vy[x, y, z + 1] - vy[x, y, z - 1]) / (2f * dx);
                var dvz_dy = (vz[x, y + 1, z] - vz[x, y - 1, z]) / (2f * dx);
        ...
        int w, int h, int d)
    {
        Parallel.For(1, d - 1, z =>
        {
            ...
            for (var x = 1; x < w - 1; x++)
            {
                var rho_local = Math.Max(rho[x, y, z], 1.0f); // Prevent division by zero

                // Stress gradients (central differences)
                var dsxx_dx = (sxx[x + 1, y, z] - sxx[x - 1, y, z]) / (2f * dx);
                var dsxy_dy = (sxy[x, y + 1, z] - sxy[x, y - 1, z]) / (2f * dx);
                var dsxz_dz = (sxz[x, y, z + 1] - sxz[x, y, z - 1]) / (2f * dx);

                var dsxy_dx = (sxy[x + 1, y, z] - sxy[x - 1, y, z]) / (2f * dx);
                var dsyy_dy = (syy[x, y + 1, z] - syy[x, y - 1, z]) / (2f * dx);
                var dsyz_dz = (syz[x, y, z + 1] - syz[x, y, z - 1]) / (2f * dx);

                var dsxz_dx = (sxz[x + 1, y, z] - sxz[x - 1, y, z]) / (2f * dx);
                var dsyz_dy = (syz[x, y + 1, z] - syz[x, y - 1, z]) / (2f * dx);
                var dszz_dz = (szz[x, y, z + 1] - szz[x, y, z - 1]) / (2f * dx);

                // Momentum equation: ρ * dv/dt = ∇·σ
                var ax = (dsxx_dx + dsxy_dy + dsxz_dz) / rho_local;
                var ay = (dsxy_dx + dsyy_dy + dsyz_dz) / rho_local;
                var az = (dsxz_dx + dsyz_dy + dszz_dz) / rho_local;
