...
private void DrawTomographyView()
{
    ...
    availableSize.Y -= 100; // Reserve space for color bar and labels
    ...
    {
        // Center the "No image" text in the reserved area
        ...
        var buttonPos = new Vector2(imageContainerTopLeft.X + (imageContainerSize.X - buttonSize.X) * 0.5f, textPos.Y + textSize.Y + 10);
...
private void DrawColorBar()
{
    ...
    ImGui.Text(title);
    ImGui.SetCursorScreenPos(pos); // Reset cursor for drawing
    ...
    // Position the min and max labels correctly under the color bar
    ...
    ImGui.Text(maxLabel);
    ImGui.Dummy(new Vector2(0, barHeight + maxLabelSize.Y + 5)); // Add final dummy to push cursor down
}
