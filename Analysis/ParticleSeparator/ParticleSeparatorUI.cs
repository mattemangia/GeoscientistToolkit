...
namespace GeoscientistToolkit.Analysis.ParticleSeparator
{
    ...
    public class ParticleSeparatorUI : IDisposable
    {
        ...
        private int _selectedZSlice = 0;

        // Visualization
        private readonly Dictionary<int, Vector4> _particleColors = new Dictionary<int, Vector4>();
        ...
        public void DrawPanel(CtImageStackDataset dataset)
        {
            ...
            if (ImGui.CollapsingHeader("Processing Options", ImGuiTreeNodeFlags.DefaultOpen))
            {
                ...
                {
                    ImGui.SetTooltip("Process entire volume (3D) or a single slice (2D)");
                }

                if (!_use3D)
                {
                    ImGui.Indent();
                    int maxSlice = dataset.Depth > 0 ? dataset.Depth - 1 : 0;
                ...
                if (ImGui.RadioButton("Auto", _selectedAcceleration == AccelerationType.Auto)) _selectedAcceleration = AccelerationType.Auto; ImGui.SameLine();
                if (ImGui.RadioButton("CPU", _selectedAcceleration == AccelerationType.CPU)) _selectedAcceleration = AccelerationType.CPU; ImGui.SameLine();
                if (ImGui.RadioButton("SIMD", _selectedAcceleration == AccelerationType.SIMD)) _selectedAcceleration = AccelerationType.SIMD; ImGui.SameLine();
                if (ImGui.RadioButton("GPU", _selectedAcceleration == AccelerationType.GPU)) _selectedAcceleration = AccelerationType.GPU;
            ...
            // Process Button
            bool canProcess = !_isProcessing && dataset.Materials != null && dataset.Materials.Any();
            ...
            if (_isProcessing)
            {
                ...
                ImGui.ProgressBar(_processor.Progress, new Vector2(-1, 0), $"{_processor.Progress * 100:F1}% - {_processor.CurrentStage}");
            }
        ...
        private void DrawResultsPanel(CtImageStackDataset dataset)
        {
            ...
            {
                if (ImGui.BeginTable("ParticleTable", 4, ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg | ImGuiTableFlags.Resizable | ImGuiTableFlags.ScrollY, new Vector2(0, 200)))
                {
                    ImGui.TableSetupColumn("ID"); ImGui.TableSetupColumn("Voxels"); ImGui.TableSetupColumn("Volume (µm³)"); ImGui.TableSetupColumn("Center (X,Y,Z)");
                    ...
                    foreach(var particle in sortedParticles)
                    {
                        ...
                        ImGui.TableNextColumn(); ImGui.Text($"{particle.VoxelCount:N0}");
                        ImGui.TableNextColumn(); ImGui.Text($"{particle.VolumeMicrometers:N0}");
                        ImGui.TableNextColumn(); ImGui.Text($"({particle.Center.X},{particle.Center.Y},{particle.Center.Z})");
                    }
                    ImGui.EndTable();
                }
            }
        ...
        private async Task ProcessParticlesAsync(CtImageStackDataset dataset)
        {
            ...
            try
            {
                ...
                _lastResult = await Task.Run(() => _processor.SeparateParticles(dataset, material, _use3D, true, _minParticleSize, _selectedZSlice, _cancellationTokenSource.Token), _cancellationTokenSource.Token);
            ...
            catch (OperationCanceledException) { Logger.Log("[ParticleSeparator] Operation cancelled"); }
            catch (Exception ex) { Logger.LogError($"[ParticleSeparator] Error: {ex.Message}"); }
            finally
            {
                _isProcessing = false;
