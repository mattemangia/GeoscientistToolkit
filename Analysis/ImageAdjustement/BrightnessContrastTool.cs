...
namespace GeoscientistToolkit.Analysis.ImageAdjustment
{
    ...
    {
        private float _brightness = 0.0f;     // Range: -100 to +100
        private float _contrast = 1.0f;       // Range: 0.1 to 3.0
        private bool _autoNormalize = false;
        ...
        public void Draw(Dataset dataset)
        {
            ...
            else
            {
                if (_previewEnabled)
                {
                    ImGui.TextColored(new Vector4(0.8f, 0.8f, 0.2f, 1), "Preview Active");
                }
                
                if (ImGui.Button("Apply to Dataset", new Vector2(-1, 30)))
                {
                    _ = ApplyToDatasetAsync(ct);
                }
        ...
        private void UpdateHistogram(CtImageStackDataset ct)
        {
            ...
            int step = Math.Max(1, ct.Depth / 10);
            for (int z = 0; z < ct.Depth; z += step)
            {
            ...
            if (totalPixels > 0)
            {
                ...
                double variance = (sumSquared / totalPixels) - (_meanValue * _meanValue);
        ...
        private void DrawHistogram()
        {
            ...
            if (_histogram != null)
            {
                ...
                for (int i = 0; i < 256; i++)
                {
                    if (_histogram[i] > maxCount) maxCount = _histogram[i];
                }
                
                if (maxCount > 0)
                {
                    // Draw histogram bars
                    float barWidth = size.X / 256.0f;
                    for (int i = 0; i < 256; i++)
                    {
                        float height = (_histogram[i] / (float)maxCount) * size.Y;
                        ...
                        float adjustedValue = ApplyAdjustment(i / 255.0f);
                        uint color = ImGui.GetColorU32(new Vector4(adjustedValue, adjustedValue, adjustedValue, 1));
                        
                        drawList.AddRectFilled(barPos, barEnd, color);
                    }
                    
                    // Draw mean line
                    float meanX = pos.X + (_meanValue / 255.0f) * size.X;
        ...
        private void CalculateAutoLevels(CtImageStackDataset ct)
        {
            ...
            long totalPixels = 0;
            foreach (int count in _histogram)
            {
                totalPixels += count;
            }
            
            long threshold1 = totalPixels / 100;  // 1%
            long threshold99 = totalPixels * 99 / 100;  // 99%
            ...
            Logger.Log($"[BrightnessContrast] Auto-levels: min={min}, max={max}, brightness={_brightness:F1}, contrast={_contrast:F2}");
        }

        private float ApplyAdjustment(float value)
        {
            // Apply contrast and brightness
            float adjusted = (value - 0.5f) * _contrast + 0.5f + (_brightness / 255.0f);
        ...
        public static byte GetAdjustedValue(byte original, float brightness, float contrast)
        {
            ...
            float adjusted = (normalized - 0.5f) * contrast + 0.5f + (brightness / 255.0f);
        ...
        private async Task ApplyToDatasetAsync(CtImageStackDataset ct)
        {
            ...
            try
            {
                Logger.Log($"[BrightnessContrast] Applying adjustments: brightness={_brightness:F1}, contrast={_contrast:F2}");
                ...
                if (ct.VolumeData is Data.VolumeData.ChunkedVolume chunkedVolume)
                {
                    string volumePath = System.IO.Path.Combine(
                        System.IO.Path.GetDirectoryName(ct.FilePath),
                        System.IO.Path.GetFileNameWithoutExtension(ct.FilePath) + ".Volume.bin"
                    );
