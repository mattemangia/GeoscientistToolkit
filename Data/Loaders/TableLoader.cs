        ...
        public DataTable GetPreview(int maxRows = 5)
        {
            ...
            try
            {
                ...
                using (var reader = new StreamReader(FilePath, encoding))
                {
                    ...
                    string[] headers = ParseCsvLine(firstLine, _delimiter[0]);
                    ...
                    if (HasHeaders)
                    {
                        ...
                        {
                            _preview.Columns.Add(header.Trim());
                        }
                    }
                    else
                    {
                        ...
                        {
                            _preview.Columns.Add($"Column{i + 1}");
                        }
                    ...
                    while ((line = reader.ReadLine()) != null && linesRead < maxRows)
                    {
                        ...
                        string[] values = ParseCsvLine(line, _delimiter[0]);
                        
                        // Ensure we have the right number of values
                        if (values.Length != _preview.Columns.Count)
                        {
                            Array.Resize(ref values, _preview.Columns.Count);
                        }
        ...
        public async Task<Dataset> LoadAsync(IProgress<(float progress, string message)> progressReporter)
        {
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    progressReporter?.Report((1.0f, $"Table imported successfully! ({dataset.RowCount} rows, {dataset.ColumnCount} columns)"));
