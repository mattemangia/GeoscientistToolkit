...
public class GISLoader : IDataLoader
{
    ...
    public string Description =>
        "Load GIS data from shapefiles, GeoJSON, KML, or GeoTIFF files, or create an empty map";
    ...
    public async Task<Dataset> LoadAsync(IProgress<(float progress, string message)> progress)
    {
        ...
        if (!File.Exists(FilePath)) throw new FileNotFoundException($"GIS file not found: {FilePath}");
