...
public static class LabeledVolumeLoader
{
            ...
            IProgress<float> progress)
    {
        return await Task.Run(async () =>
        {
            ...
            var labelPath = Path.Combine(outputDir, $"{datasetName}.Labels.bin");
            ...
            {
                grayscaleVolume = new ChunkedVolume(width, height, depth)
                {
                    PixelSize = pixelSize
                };
            ...
            IProgress<float> progress)
    {
        return await Task.Run(async () =>
        {
            ...
            for (var z = 0; z < depth; z++)
            {
                ...
                var progressVal = 0.4f + (float)(z + 1) / depth * 0.5f;
                progress?.Report(progressVal);
            }
        ...
        Action<float> progressCallback)
    {
        ...
        var sampleRate = Math.Max(1, imageFiles.Count / 20);

        // Declare once so the lambda can fill them in
        int w = 0, h = 0, channels = 0;

        for (var i = 0; i < imageFiles.Count; i += sampleRate)
        {
            // Load slice on a worker thread
            var rgbData = await Task.Run(() =>
                ImageLoader.LoadColorImage(imageFiles[i], out w, out h, out channels));

            // Step through the pixel data using the channel count
            for (var j = 0; j < rgbData.Length; j += channels)
            {
                var r = rgbData[j];
                var g = channels > 1 ? rgbData[j + 1] : r;
                var b = channels > 2 ? rgbData[j + 2] : r;

                var colorKey = ((uint)r << 16) | ((uint)g << 8) | b;
        ...
        Action<float> progressCallback)
    {
        ...
        var sampleRate = Math.Max(1, pageCount / 20); // Sample ~20 pages max

        await Task.Run(() =>
        {
            for (var i = 0; i < pageCount; i += sampleRate)
            {
                var rgbData = ImageLoader.LoadTiffPageAsColor(tiffPath, i,
                    out var w, out var h, out var channels);
        ...
        List<Material> materials)
    {
        await Task.Run(() =>
        {
            var rgbData = ImageLoader.LoadColorImage(imagePath, out var width, out var height, out var channels);
            ...
            {
                var pixelIndex = i * channels;
                var r = rgbData[pixelIndex];
                var g = channels > 1 ? rgbData[pixelIndex + 1] : r;
                var b = channels > 2 ? rgbData[pixelIndex + 2] : r;
                ...
                else if (colorToMaterialMap.TryGetValue(colorKey, out var materialId))
                {
        ...
        int chunkDim)
    {
        await Task.Run(() =>
        {
            ...
            var cntZ = (depth + chunkDim - 1) / chunkDim;
            var chunkSize = (long)chunkDim * chunkDim * chunkDim;
            var totalSize = 28 + cntX * cntY * cntZ * chunkSize; // 28 byte header for labels
