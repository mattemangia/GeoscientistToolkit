...
namespace GeoscientistToolkit
{
    public class CtImageStackTools : IDatasetTools
    {
        ...
        private static bool _isThresholdPreviewActive = false;
        private static byte _previewMinThreshold = 0;
        ...
        private bool _show3DThresholdPreview = false;
        private byte[] _thresholdPreviewMask3D;
        private bool _isGenerating3DPreview = false;
        ...
        {
            if (_isThresholdPreviewActive && _previewMaterialRef != null && _previewMaterialRef.TryGetTarget(out var material))
            {
        ...
        public void Draw(Dataset dataset)
        {
            ...
            if (ImGui.DragInt("Min", ref min, 1, 0, 255)) { _minThreshold = (byte)min; thresholdChanged = true; }
            ImGui.SameLine();
            ImGui.SetNextItemWidth(ImGui.GetContentRegionAvail().X);
            if (ImGui.DragInt("Max", ref max, 1, 0, 255)) { _maxThreshold = (byte)max; thresholdChanged = true; }
            ...
            if (ImGui.IsItemActive() || thresholdChanged)
            {
                Set2DPreviewState(true, _selectedMaterialForThresholding);
            }
            ...
            if (ImGui.Button("Add to Material", new Vector2(ImGui.GetContentRegionAvail().X, 0)))
            {
                ...
                _ = MaterialOperations.AddVoxelsByThresholdAsync(ctDataset.VolumeData, ctDataset.LabelData, _selectedMaterialForThresholding.ID, _minThreshold, _maxThreshold, ctDataset);
            }
            ...
            if (ImGui.Button("Remove from Material", new Vector2(ImGui.GetContentRegionAvail().X, 0)))
            {
                Set2DPreviewState(false, null);
                if(_show3DThresholdPreview) Clear3DThresholdPreview();
                _ = MaterialOperations.RemoveVoxelsByThresholdAsync(ctDataset.VolumeData, ctDataset.LabelData, _selectedMaterialForThresholding.ID, _minThreshold, _maxThreshold, ctDataset);
            }
