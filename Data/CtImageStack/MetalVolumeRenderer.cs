...
";
    ...
    public void UpdateMaterialTextures()
    {
        ...
        for (var i = 0; i < 256; i++)
        {
            ...
            if (material != null)
            {
                ...
                var opacity = _viewer.GetMaterialOpacity((byte)i);
        ...
        DeviceBuffer planeVertexBuffer, DeviceBuffer planeIndexBuffer)
    {
        ...
        try
        {
            ...
            var constants = new CtVolume3DViewer.VolumeConstants
            {
                ...
                CutPlaneX = new Vector4(_viewer.CutXEnabled ? 1 : 0, _viewer.CutXForward ? 1 : -1, _viewer.CutXPosition,
                    0),
                CutPlaneY = new Vector4(_viewer.CutYEnabled ? 1 : 0, _viewer.CutYForward ? 1 : -1, _viewer.CutYPosition,
                    0),
                CutPlaneZ = new Vector4(_viewer.CutZEnabled ? 1 : 0, _viewer.CutZForward ? 1 : -1, _viewer.CutZPosition,
                    0),
                ClippingPlanesInfo = new Vector4(0, _viewer.ShowPlaneVisualizations ? 1 : 0, 0, 0),
                PreviewParams = new Vector4(_viewer._showPreview ? 1 : 0, _viewer._previewColor.X,
                    _viewer._previewColor.Y, _viewer._previewColor.Z),
            ...
            {
                var enabledPlanes = 0;
                for (var i = 0; i < Math.Min(_viewer.ClippingPlanes.Count, CtVolume3DViewer.MAX_CLIPPING_PLANES); i++)
                {
    ...
    private void RenderTransducerMarkers(CommandList cl, Matrix4x4 viewMatrix, Matrix4x4 projMatrix)
    {
        ...
        if (distance > 0.01f)
        {
            ...
            var angle = MathF.Acos(Vector3.Dot(Vector3.UnitZ, normalizedDir));
