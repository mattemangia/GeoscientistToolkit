...
public class CtImageStackViewer : IDatasetViewer
{
    ...
    private bool _showSegmentationWindow;
    ...
    public void DrawToolbarControls()
    {
        ...
        var layoutIndex = (int)_layout;
        if (ImGui.Combo("##Layout", ref layoutIndex, layouts, layouts.Length)) _layout = (Layout)layoutIndex;
        ...
        if (ImGui.Button("Segmentation")) _showSegmentationWindow = !_showSegmentationWindow;
    ...
    private void DrawSegmentationToolWindow()
    {
        ...
        {
            var preview = _selectedMaterialForSegmentation?.Name ?? "Select a material...";
    ...
    {
        var viewWidth = (availableSize.X - 4) / 3;
        var viewHeight = availableSize.Y;

        DrawView(0, new Vector2(viewWidth, viewHeight), "XY (Axial)", ref _zoomXY, ref _panXY, ref _needsUpdateXY,
            ref _textureXY);
        ImGui.SameLine(0, 2);
        DrawView(1, new Vector2(viewWidth, viewHeight), "XZ (Coronal)", ref _zoomXZ, ref _panXZ, ref _needsUpdateXZ,
            ref _textureXZ);
        ImGui.SameLine(0, 2);
        DrawView(2, new Vector2(viewWidth, viewHeight), "YZ (Sagittal)", ref _zoomYZ, ref _panYZ, ref _needsUpdateYZ,
            ref _textureYZ);
    }

    private void DrawVerticalLayout(Vector2 availableSize)
    {
        var viewWidth = availableSize.X;
        var viewHeight = (availableSize.Y - 4) / 3;
    ...
    private void Draw3DInfoPanel(Vector2 size)
    {
        ...
        ImGui.Text(
            $"Voxel Size: {_dataset.PixelSize:F2} × {_dataset.PixelSize:F2} × {_dataset.SliceThickness:F2} {_dataset.Unit}");
    ...
    private void UpdateTexture(int viewIndex, ref TextureManager texture)
    {
        ...
        try
        {
            ...
            var segmentationPreviewMask = _segmentationManager?.GetPreviewMask(currentSlice, viewIndex);
            var committedSelectionMask = _segmentationManager?.GetCommittedSelectionMask(currentSlice, viewIndex);
            ...
            for (var i = 0; i < width * height; i++)
            {
                ...
                if (committedSelectionMask != null && committedSelectionMask[i] > 0)
                {
                    ...
                    var opacity = 0.4f;
                    finalColor = Vector4.Lerp(finalColor, new Vector4(selColor.X, selColor.Y, selColor.Z, 1.0f),
                        opacity);
                }
                ...
                if (segmentationPreviewMask != null && segmentationPreviewMask[i] > 0)
                {
                    ...
                    var opacity = 0.6f;
                    finalColor = Vector4.Lerp(finalColor, new Vector4(segColor.X, segColor.Y, segColor.Z, 1.0f),
                        opacity);
                }
