    ...
    public class InterpolationManager
    {
            ...
            int viewIndex, InterpolationType type)
        {
            ...
            return type switch
            {
                ...
                InterpolationType.ShapeInterpolation => InterpolateShapeBased(startMask, startSlice, endMask, endSlice, viewIndex),
                InterpolationType.Morphological3D => InterpolateMorphological3D(startMask, startSlice, endMask, endSlice, viewIndex),
            ...
            List<List<Vector2>> startContours, List<List<Vector2>> endContours)
        {
            ...
            foreach (var startContour in startContours)
            {
                ...
                for (int i = 0; i < endContours.Count; i++)
                {
                    ...
                    if (dist < minDist) { minDist = dist; bestMatch = i; }
                }
        ...
        private List<Vector2> ResampleContour(List<Vector2> contour, int targetPoints)
        {
            ...
            for (int i = 1; i < targetPoints; i++)
            {
                ...
                while (currentDist < targetDist && currentIndex < contour.Count)
                {
                    ...
                    {
                        float lerpT = (targetDist - currentDist) / edgeLength;
        ...
        private void FillContour(byte[] mask, List<Vector2> contour, int width, int height)
        {
            ...
            int minY = Math.Max(0, (int)Math.Floor(contour.Min(p => p.Y)));
            int maxY = Math.Min(height - 1, (int)Math.Ceiling(contour.Max(p => p.Y)));
