...
public class LassoTool : ISegmentationTool
{
    ...
    protected virtual void UpdateSelectionMask()
    {
        ...
        for (var i = 0; i < _points.Count - 1; i++) DrawLine(_points[i], _points[i + 1]);
    }
    ...
    protected void FillPolygon()
    {
        ...
        var minY = Math.Max(0, (int)Math.Ceiling(edges.Min(e => e.YMin)));
        var maxY = Math.Min(_height - 1, (int)Math.Floor(edges.Max(e => e.YMax)));
        ...
        for (var y = minY; y <= maxY; y++)
        {
            ...
            intersections.Clear();
            foreach (var edge in activeEdges) intersections.Add(edge.XAtYMin + (y - edge.YMin) * edge.Slope);
            ...
            for (var i = 0; i < intersections.Count - 1; i += 2)
            {
                ...
                var xEnd = Math.Min(_width - 1, (int)Math.Floor(intersections[i + 1]));
                for (var x = xStart; x <= xEnd; x++) _selectionMask[y * _width + x] = 255;
            }
        }
    }
