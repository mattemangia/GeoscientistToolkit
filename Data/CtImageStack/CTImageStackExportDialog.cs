...
namespace GeoscientistToolkit.Data.CtImageStack
{
    public class CtImageStackExportDialog
    {
        ...
        private int _selectedFormat = 0; // 0=PNG, 1=TIF, 2=BMP, 3=JPEG
        ...
        private bool _exportLabels = false;
        private bool _exportOverlay = false;
        ...
        public void Submit()
        {
            ...
            {
                _selectedFolder = _folderDialog.CurrentDirectory;
            }
        ...
        private void DrawButtons()
        {
            ...
            if (ImGui.Button("Select Folder...", new Vector2(buttonWidth, 0)))
            {
                _folderDialog.Open("");
            }
            ...
            bool canExport = !string.IsNullOrEmpty(_selectedFolder) && Directory.Exists(_selectedFolder);
        ...
        private async Task ExportStackAsync(string outputFolder)
        {
            try
            {
                ...
                string baseFilePath = Path.Combine(outputFolder, _baseFileName);
                string extension = FormatExtensions[_selectedFormat];
                ...
                for (int z = 0; z < totalSlices; z++)
                {
                    ...
                    tasks[z] = Task.Run(async () =>
                    {
                        ...
                        try
                        {
                            byte[] sliceData = new byte[_width * _height];
                            byte[] rgbaData = new byte[_width * _height * 4];
                            byte[] labelData = (_exportLabels || _exportOverlay) ? new byte[_width * _height] : null;
        ...
        private void ConvertToRGBA(byte[] grayscale, byte[] labels, byte[] rgba, bool applyOverlay)
        {
            for (int i = 0; i < grayscale.Length; i++)
            {
                ...
                if (applyOverlay && labels != null && labels[i] > 0)
                {
                    ...
                    {
                        float opacity = 0.5f; // Semi-transparent overlay
        ...
        private async Task SaveImageAsync(string filePath, byte[] rgbaData, int width, int height, int format, int jpegQuality)
        {
            await Task.Run(() =>
            {
                switch (format)
                {
                    ...
                    case 1: // TIF
                        SimpleTiffWriter.WriteTiff(filePath, rgbaData, width, height, false);
                        break;
                    ...
                    case 3: // JPEG
                        ...
                        {
                            var writer = new ImageWriter();
                            writer.WriteJpg(rgbaData, width, height, ColorComponents.RedGreenBlueAlpha, stream, jpegQuality);
                        }
                        break;
                }
            });
