...
public static class LabelStackLoader
{
        ...
        foreach (var imagePath in imageFiles)
        {
            ...
            tasks.Add(Task.Run(async () =>
            {
                try
                {
                    ...
                    lock (colorLock)
                    {
                        foreach (var color in localColors) uniqueColors.Add(color);

                        processedCount++;
                        var currentProgress = 0.1f + 0.3f * processedCount / totalFiles;
                ...
                {
                    Logger.LogError(
                        $"[LabelStackLoader] Error analyzing image {Path.GetFileName(imagePath)}: {ex.Message}");
                }
                finally
                {
                    semaphore.Release();
    ...
    private static (List<Material>, Dictionary<uint, byte>) CreateMaterialsFromColors(HashSet<uint> uniqueColors)
    {
        ...
        foreach (var color in sortedColors)
        {
            ...
            {
                Logger.LogWarning(
                    "[LabelStackLoader] Maximum material count (255) reached. Skipping remaining colors.");
                break;
            }
        ...
        for (var z = 0; z < imageFiles.Count; z++)
        {
            ...
            await Task.Run(() =>
            {
                try
                {
                    ...
                    processedCount++;
                    var currentProgress = 0.4f + 0.6f * processedCount / totalFiles;
                    progress?.Report(currentProgress);
                }
                catch (Exception ex)
                {
                    Logger.LogError(
                        $"[LabelStackLoader] Error loading slice {z} from {Path.GetFileName(imageFiles[z])}: {ex.Message}");
                }
            }, cancellationToken);
        }
        ...
        Dictionary<uint, byte> colorToIdMap)
    {
        using (var stream = File.OpenRead(imagePath))
        {
            ...
            {
                Logger.LogWarning(
                    $"[LabelStackLoader] Image dimensions ({image.Width}×{image.Height}) don't match volume ({labelVolume.Width}×{labelVolume.Height})");
                return;
            }
            ...
            for (var x = 0; x < image.Width; x++)
            {
                ...
                if (colorToIdMap.TryGetValue(color, out var id))
                    materialId = id;
                else
        ...
        string datasetName)
    {
        ...
        try
        {
            ...
            {
                var volumePath = Path.Combine(outputPath, $"{datasetName}.Volume.bin");
                result.GrayscaleVolume.SaveAsBin(volumePath);
                Logger.Log($"[LabelStackLoader] Saved grayscale volume to: {volumePath}");
            }

            // Save label volume
            var labelPath = Path.Combine(outputPath, $"{datasetName}.Labels.bin");
