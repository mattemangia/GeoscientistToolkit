...
namespace GeoscientistToolkit.UI.Borehole
{
    ...
    public static class BoreholeDebugTools
    {
        ...
        private static int _selectedPreset = 0;
        ...
        {
            if (ImGui.Button("Generate Test Borehole"))
            {
            ...
            if (_showDebugWindow)
            {
                ImGui.OpenPopup("Generate Test Borehole");
            }
        ...
        public static void FillEmptyBorehole(BoreholeDataset borehole, string presetName = "default")
        {
            ...
            int presetIndex = 0; // Default to shallow urban
        ...
        private static string GetPresetDescription(int presetIndex)
        {
            ...
            {
                0 => "100m urban borehole with soil, clay, sand, and bedrock layers. Suitable for ground source heat pump testing.",
                1 => "2500m deep borehole through sedimentary basin with multiple aquifers. Good for deep geothermal exploration.",
        ...
        private static List<ParameterPoint> GeneratePorosityProfile(BoreholeDataset borehole)
        {
            ...
            {
                if (unit.Parameters.TryGetValue("Porosity", out float porosity))
                {
                    // Add points at unit boundaries with some variation
                    for (float depth = unit.DepthFrom; depth <= unit.DepthTo; depth += 2.0f)
                    {
                        float variation = (float)(Random.Shared.NextDouble() * 0.1 - 0.05); // ±5%
                        float value = Math.Max(0, Math.Min(0.5f, porosity + variation)) * 100f; // Convert to percentage
        ...
        private static List<ParameterPoint> GeneratePermeabilityProfile(BoreholeDataset borehole)
        {
            ...
            foreach (var unit in borehole.LithologyUnits)
            {
                if (unit.Parameters.TryGetValue("Permeability", out float perm))
                {
                    // Convert from m² to millidarcies (1 m² = 1.013e15 mD)
                    float permMD = perm * 1.013e15f;

                    for (float depth = unit.DepthFrom; depth <= unit.DepthTo; depth += 2.0f)
                    {
                        // Add log-normal variation
                        float logVariation = (float)(Random.Shared.NextDouble() * 0.5 - 0.25);
                        float value = permMD * (float)Math.Exp(logVariation);
        ...
        private static List<ParameterPoint> GenerateThermalProfile(BoreholeDataset borehole)
        {
            ...
            foreach (var unit in borehole.LithologyUnits)
            {
                if (unit.Parameters.TryGetValue("ThermalConductivity", out float thermal))
                {
                    for (float depth = unit.DepthFrom; depth <= unit.DepthTo; depth += 5.0f)
                    {
                        float variation = (float)(Random.Shared.NextDouble() * 0.2 - 0.1);
                        float value = Math.Max(0.5f, Math.Min(4.0f, thermal + variation));
        ...
        private static List<ParameterPoint> GenerateGammaRayProfile(BoreholeDataset borehole)
        {
            ...
            foreach (var unit in borehole.LithologyUnits)
            {
                ...
                float baseGR = unit.LithologyType switch
                {
                    "Shale" => 100f,
                    "Clay" => 90f,
                    "Mudstone" => 85f,
                ...
                for (float depth = unit.DepthFrom; depth <= unit.DepthTo; depth += 1.0f)
                {
                    float variation = (float)(Random.Shared.NextDouble() * 20 - 10);
                    float value = Math.Max(0, Math.Min(150, baseGR + variation));
