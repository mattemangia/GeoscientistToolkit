...
public class SurfaceNetsMesher
{
        ...
        CancellationToken token)
    {
        ...
        {
            Logger.LogWarning(
                "No edge vertices found for the selected material. The material might be empty or not form a surface.");
        ...
        var vertexMap = edgeVertices.Keys.Select((k, i) => new { Key = k, Index = i })
        ...
        ChunkedLabelVolume labels, byte materialId, int ds, CancellationToken token)
    {
        ...
        return (Task<ConcurrentDictionary<long, Vector3>>)Task.Run(() =>
        {
            ...
            {
                for (var y = 0; y < labels.Height - ds; y += ds)
                for (var x = 0; x < labels.Width - ds; x += ds)
                {
                    var p = labels[x, y, z] == materialId;

                    // Check against X+1 neighbor
                    if (p != (labels[x + ds, y, z] == materialId))
                    {
                        var key = GetEdgeKey(x, y, z, X_AXIS);
                    ...
                    {
                        var key = GetEdgeKey(x, y, z, Y_AXIS);
                    ...
                    {
                        var key = GetEdgeKey(x, y, z, Z_AXIS);
        ...
        IProgress<(float progress, string message)> progress, CancellationToken token)
    {
        ...
        return Task.Run(() =>
        {
            ...
            {
                for (var y = ds; y < labels.Height - ds; y += ds)
                for (var x = ds; x < labels.Width - ds; x += ds)
                {
                    ...
                    {
                        var v1 = vertexMap[GetEdgeKey(x - ds, y - ds, z - ds, Y_AXIS)];
                        var v2 = vertexMap[GetEdgeKey(x - ds, y, z - ds, Y_AXIS)];
                        var v3 = vertexMap[GetEdgeKey(x - ds, y, z, Y_AXIS)];
                        var v4 = vertexMap[GetEdgeKey(x - ds, y - ds, z, Y_AXIS)];
                    ...
                    {
                        var v1 = vertexMap[GetEdgeKey(x - ds, y - ds, z - ds, X_AXIS)];
                        var v2 = vertexMap[GetEdgeKey(x, y - ds, z - ds, X_AXIS)];
                        var v3 = vertexMap[GetEdgeKey(x, y - ds, z, X_AXIS)];
                        var v4 = vertexMap[GetEdgeKey(x - ds, y - ds, z, X_AXIS)];
                    ...
                    {
                        var v1 = vertexMap[GetEdgeKey(x - ds, y - ds, z - ds, Z_AXIS)];
                        var v2 = vertexMap[GetEdgeKey(x, y - ds, z - ds, Z_AXIS)];
                        var v3 = vertexMap[GetEdgeKey(x, y, z - ds, Z_AXIS)];
                        var v4 = vertexMap[GetEdgeKey(x - ds, y, z - ds, Z_AXIS)];
                        faces.Add(new[] { v1, v2, v3 });
                        faces.Add(new[] { v1, v3, v4 });
                    }
                }

                var currentSlice = Interlocked.Increment(ref processedSlices);
    ...
    {
        return ((long)x << 42) | ((long)y << 22) | ((long)z << 2) | axis;
    }
}