...
namespace GeoscientistToolkit.Data.Mesh3D
{
    ...
    public class MeshVoxelizer
    {
            ...
            IProgress<(float progress, string message)> progress)
        {
            ...
            int width = (int)Math.Ceiling(size.X / voxelSize) + 2;  // +2 for padding
            int height = (int)Math.Ceiling(size.Y / voxelSize) + 2;
            int depth = (int)Math.Ceiling(size.Z / voxelSize) + 2;

            Logger.Log($"[MeshVoxelizer] Voxelizing mesh to {width}×{height}×{depth} volume");
            Logger.Log($"[MeshVoxelizer] Voxel size: {voxelSize}mm, Bounds: {transformedBounds.min} to {transformedBounds.max}");

            // Create output folder for the stack
            string stackFolder = Path.Combine(outputFolder, $"{mesh.Name}_voxelized_{DateTime.Now:yyyyMMdd_HHmmss}");
        ...
        private Matrix4x4 CreateTransformMatrix(float scale, Vector3 rotation, Vector3 center)
        {
            ...
            float rotX = rotation.X * (float)(Math.PI / 180.0);
            float rotY = rotation.Y * (float)(Math.PI / 180.0);
            float rotZ = rotation.Z * (float)(Math.PI / 180.0);
        ...
        private List<Vector3> TransformVertices(List<Vector3> vertices, float scale, Vector3 rotation, Vector3 center)
        {
            ...
            {
                transformed.Add(Vector3.Transform(vertex, transformMatrix));
            }
            ...
            IProgress<(float progress, string message)> progress)
        {
            ...
            int batchSize = Math.Max(1, totalFaces / 100);
            ...
            await Task.Run(() =>
            {
                Parallel.ForEach(faces, face =>
                {
                    ...
                    int current = System.Threading.Interlocked.Increment(ref processedFaces);
                    if (current % batchSize == 0)
                    {
                        float faceProgress = (float)current / totalFaces;
            ...
            float voxelSize)
        {
            ...
            Vector3 minTriangle = Vector3.Min(Vector3.Min(v0, v1), v2);
            Vector3 maxTriangle = Vector3.Max(Vector3.Max(v0, v1), v2);

            int minX = Math.Max(0, (int)((minTriangle.X - minBounds.X) / voxelSize));
            int minY = Math.Max(0, (int)((minTriangle.Y - minBounds.Y) / voxelSize));
            int minZ = Math.Max(0, (int)((minTriangle.Z - minBounds.Z) / voxelSize));

            int maxX = Math.Min(voxelGrid.GetLength(0) - 1, (int)((maxTriangle.X - minBounds.X) / voxelSize) + 1);
            int maxY = Math.Min(voxelGrid.GetLength(1) - 1, (int)((maxTriangle.Y - minBounds.Y) / voxelSize) + 1);
            int maxZ = Math.Min(voxelGrid.GetLength(2) - 1, (int)((maxTriangle.Z - minBounds.Z) / voxelSize) + 1);
            ...
            for (int z = minZ; z <= maxZ; z++)
            {
                for (int y = minY; y <= maxY; y++)
                {
                    for (int x = minX; x <= maxX; x++)
                    {
                        ...
                        if (VoxelIntersectsTriangle(voxelCenter, voxelSize * 0.5f, v0, v1, v2))
                        {
                            voxelGrid[x, y, z] = 255;
                        }
                    }
        ...
        private bool VoxelIntersectsTriangle(Vector3 voxelCenter, float halfSize, Vector3 v0, Vector3 v1, Vector3 v2)
        {
            ...
            float dist = Math.Abs(Vector3.Dot(normal, voxelCenter) + d);

            // Project half-size onto normal (maximum distance from center to corner along normal)
            float projectedHalfSize = halfSize * (Math.Abs(normal.X) + Math.Abs(normal.Y) + Math.Abs(normal.Z));
                ...
                for (int y = 0; y < height; y++)
                {
                    ...
                    for (int x = 0; x < width; x++)
                    {
                        if (voxelGrid[x, y, z] == 255)
                        {
                            ...
                            {
                                // Fill the gap
                                for (int fillX = lastBoundary + 1; fillX < x; fillX++)
                                {
            ...
            IProgress<(float progress, string message)> progress)
        {
            ...
            await Task.Run(() =>
            {
                for (int z = 0; z < depth; z++)
                {
                    ...
                    string filename = Path.Combine(outputFolder, $"slice_{z:D4}.bmp");
                    SaveAsBitmap(filename, sliceData, width, height);

                    if (z % 10 == 0)
                    {
                        float saveProgress = (float)z / depth;
        ...
        private void SaveAsBitmap(string filename, byte[] data, int width, int height)
        {
            ...
            int fileSize = 14 + 40 + 1024 + (rowSize * height); // headers + palette + padded data
            int dataOffset = 14 + 40 + 1024; // after headers and palette
