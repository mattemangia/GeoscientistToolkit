...
";
    ...
    public void Initialize(Mesh3DDataset dataset)
    {
        ...
        var vsMain = factory.CreateShader(new ShaderDescription(
            ShaderStages.Vertex,
            Encoding.UTF8.GetBytes(GlslVertexShaderMesh),
            "main"));

        var fsMain = factory.CreateShader(new ShaderDescription(
            ShaderStages.Fragment,
            Encoding.UTF8.GetBytes(GlslFragmentShaderMesh),
            "main"));

        var vsLine = factory.CreateShader(new ShaderDescription(
            ShaderStages.Vertex,
            Encoding.UTF8.GetBytes(GlslVertexShaderLine),
            "main"));

        var fsLine = factory.CreateShader(new ShaderDescription(
            ShaderStages.Fragment,
