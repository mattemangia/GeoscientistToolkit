...
public class Mesh3DTools : IDatasetTools
{
    ...
    public void Draw(Dataset dataset)
    {
        ...
        var rot = _rotationByDataset[mesh];
        ...
        if (ImGui.CollapsingHeader("Scan to Volume", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ...
            if (!mesh.IsLoaded)
            {
                ...
                if (ImGui.Button("Load Model")) mesh.Load();
            }
            ...
            else
            {
                ...
                var estimatedWidth = (int)Math.Ceiling(scaledSize.X / scanState.VoxelSize);
                var estimatedHeight = (int)Math.Ceiling(scaledSize.Y / scanState.VoxelSize);
                var estimatedDepth = (int)Math.Ceiling(scaledSize.Z / scanState.VoxelSize);

                ImGui.Separator();
                ImGui.Text("Estimated Volume Size:");
                ImGui.Text($"  {estimatedWidth} × {estimatedHeight} × {estimatedDepth} voxels");
                var estimatedBytes = (long)estimatedWidth * estimatedHeight * estimatedDepth;
                ...
                ImGui.Separator();
                var canStart = !string.IsNullOrEmpty(scanState.OutputFolder) &&
                               Directory.Exists(scanState.OutputFolder);
    ...
    private async Task StartScanToVolumeAsync(Mesh3DDataset mesh, ScanToVolumeState state)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(outputPath))
            {
                ...
                var loader = new CTStackLoaderWrapper
                {
                    Mode = CTStackLoaderWrapper.LoadMode.LegacyFor2D,
                    SourcePath = outputPath,
                    IsMultiPageTiff = false,
                    PixelSize = state.VoxelSize,
                    Unit = PixelSizeUnit.Millimeters,
                    BinningFactor = 1
                };
