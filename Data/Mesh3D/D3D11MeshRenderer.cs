...
";
    ...
    public void Initialize(Mesh3DDataset dataset)
    {
        ...
        var depthFormat = PixelFormat.D24_UNorm_S8_UInt;
        ...
        var vertexShaderMain = factory.CreateShader(new ShaderDescription(
            ShaderStages.Vertex, Encoding.UTF8.GetBytes(HlslShaderSource), "mesh_vs"));
        var fragmentShaderMain = factory.CreateShader(new ShaderDescription(
            ShaderStages.Fragment, Encoding.UTF8.GetBytes(HlslShaderSource), "mesh_fs"));
        var vertexShaderLine = factory.CreateShader(new ShaderDescription(
            ShaderStages.Vertex, Encoding.UTF8.GetBytes(HlslShaderSource), "line_vs"));
        var fragmentShaderLine = factory.CreateShader(new ShaderDescription(
        ...
        var triPipelineDesc = new GraphicsPipelineDescription
        {
            ...
            ShaderSet = new ShaderSetDescription(new[] { vertexLayoutMain },
                new[] { vertexShaderMain, fragmentShaderMain }),
        ...
        var linePipelineDesc = new GraphicsPipelineDescription
        {
            ...
            ShaderSet = new ShaderSetDescription(new[] { vertexLayoutLine },
                new[] { vertexShaderLine, fragmentShaderLine }),
            ResourceLayouts = new[] { _resourceLayout },
            Outputs = _framebuffer.OutputDescription
        };
    ...
    private void UpdateGridBuffers(Mesh3DDataset dataset, Vector3 cameraTarget)
    {
        ...
        var vertexBufferSize = (uint)(vertices.Count * sizeof(float));
        if (_vertexBufferGrid == null || _vertexBufferGrid.SizeInBytes < vertexBufferSize)
        {
            _vertexBufferGrid?.Dispose();
            _vertexBufferGrid =
                VeldridManager.Factory.CreateBuffer(new BufferDescription(vertexBufferSize, BufferUsage.VertexBuffer));
        }

        VeldridManager.GraphicsDevice.UpdateBuffer(_vertexBufferGrid, 0, vertices.ToArray());
        var indexBufferSize = (uint)(indices.Count * sizeof(uint));
        if (_indexBufferGrid == null || _indexBufferGrid.SizeInBytes < indexBufferSize)
        {
            _indexBufferGrid?.Dispose();
            _indexBufferGrid =
                VeldridManager.Factory.CreateBuffer(new BufferDescription(indexBufferSize, BufferUsage.IndexBuffer));
        }
