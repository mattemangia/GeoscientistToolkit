        ...
        public void Draw(Dataset dataset)
        {
            ...
            if (ImGui.CollapsingHeader("Mesh Summary", ImGuiTreeNodeFlags.DefaultOpen))
            {
                ...
                if (mesh.IsLoaded)
                {
                    ...
                    ImGui.Text($"  Min: ({mesh.BoundingBoxMin.X:F2}, {mesh.BoundingBoxMin.Y:F2}, {mesh.BoundingBoxMin.Z:F2})");
                    ImGui.Text($"  Max: ({mesh.BoundingBoxMax.X:F2}, {mesh.BoundingBoxMax.Y:F2}, {mesh.BoundingBoxMax.Z:F2})");
