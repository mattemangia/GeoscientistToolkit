...
namespace GeoscientistToolkit.Data.Mesh3D
{
    ...
    public class MetalMeshRenderer : IDisposable
    {
        ...
        public Texture ColorTarget => _colorTarget;  // expose for ImGui
...
";
        ...
        public void Initialize(Mesh3DDataset dataset)
        {
            ...
            Shader vsMain = factory.CreateShader(new ShaderDescription(ShaderStages.Vertex, Encoding.UTF8.GetBytes(MetalShaderSource), "mesh_vertex_main"));
            Shader fsMain = factory.CreateShader(new ShaderDescription(ShaderStages.Fragment, Encoding.UTF8.GetBytes(MetalShaderSource), "mesh_fragment_main"));
            Shader vsLine = factory.CreateShader(new ShaderDescription(ShaderStages.Vertex, Encoding.UTF8.GetBytes(MetalShaderSource), "line_vertex_main"));
            Shader fsLine = factory.CreateShader(new ShaderDescription(ShaderStages.Fragment, Encoding.UTF8.GetBytes(MetalShaderSource), "line_fragment_main"));
        ...
        public void Render(Mesh3DDataset dataset, Matrix4x4 viewMatrix, Matrix4x4 projMatrix, Vector3 cameraTarget, bool showGrid)
        {
            ...
            uint indexCount = (uint)(dataset.Faces.Count * 3);
