...
public static class GeologicalMapping
{
    ...
    public static class GeologicalSymbols
    {
        ...
        /// </summary>
        public static List<Vector2[]> GenerateStrikeDipSymbol(Vector2 position, float strike, float dip,
            float scale = 20f)
        {
            ...
            float scale = 10f, string movementSense = null)
        {
            ...
            for (var i = 0; i < faultLine.Length - 1; i++)
            {
            ...
            float scale = 15f, float? plunge = null)
        {
            ...
            for (var i = 0; i < axisLine.Length - 1; i++)
            {
    ...
    public static class ProfileGenerator
    {
            ...
            List<GeologicalFeature> geologicalFeatures = null)
        {
            ...
            for (var i = 0; i < numSamples; i++)
            {
                ...
                var elevation = SampleDEM(demData, demBounds, samplePos);
        ...
        {
            var width = demData.GetLength(0);
            var height = demData.GetLength(1);

            // Convert world coordinates to pixel coordinates
            var nx = (position.X - bounds.Min.X) / (bounds.Max.X - bounds.Min.X);
            var ny = (position.Y - bounds.Min.Y) / (bounds.Max.Y - bounds.Min.Y);
            ...
            var v00 = demData[x0, y0];
            var v10 = demData[x1, y0];
            var v01 = demData[x0, y1];
            var v11 = demData[x1, y1];
    ...
    public static class CrossSectionGenerator
    {
        ...
        private static ProjectedFormation ProjectFormation(GeologicalFeature formation,
            ProfileGenerator.TopographicProfile profile, float defaultThickness)
        {
            ...
            var projected = new ProjectedFormation
            {
                ...
                Color = LithologyPatterns.StandardColors.GetValueOrDefault(formation.LithologyCode ?? "",
        ...
        private static ProjectedFault ProjectFault(GeologicalFeature fault, ProfileGenerator.TopographicProfile profile)
        {
            ...
            for (var i = 0; i < profile.Points.Count; i++)
            for (var j = 0; j < fault.Coordinates.Count - 1; j++)
            {
                var dist = ProfileGenerator.DistanceToLineSegment(profile.Points[i].Position, fault.Coordinates[j],
                    fault.Coordinates[j + 1]);
                if (dist < minDistance)
                {
                    minDistance = dist;
            ...
            if (minDistance < 50f && profileIndex != -1) // 50 units tolerance
            {
                ...
                var sectionDepth = profile.MaxElevation - profile.MinElevation + 1000f;
                var end = new Vector2(
