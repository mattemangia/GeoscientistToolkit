...
namespace GeoscientistToolkit.UI.GIS
{
    public class GISTools : IDatasetTools
    {
        ...
        public void Draw(Dataset dataset)
        {
            ...
            {
                DrawLayerManager(gisDataset);
            }
            
            if (ImGui.CollapsingHeader("Create from Metadata"))
            {
                DrawCreateFromMetadata(gisDataset);
            }
            
            if (ImGui.CollapsingHeader("Export"))
            {
                DrawExportOptions(gisDataset);
            }
            
            if (ImGui.CollapsingHeader("Projection"))
            {
                DrawProjectionInfo(gisDataset);
            }
            
            // Handle export dialogs
            if (_shpExportDialog.Submit())
            {
                gisDataset.SaveAsShapefile(_shpExportDialog.SelectedPath);
            }
            
            if (_geoJsonExportDialog.Submit())
            {
                gisDataset.SaveAsGeoJSON(_geoJsonExportDialog.SelectedPath);
            }
        }
        ...
        private void DrawLayerManager(GISDataset dataset)
        {
            ...
            // Layer list
            for (int i = 0; i < dataset.Layers.Count; i++)
            {
                ...
                if (ImGui.TreeNode(layer.Name))
                {
                    ...
                    if (layer.Type == LayerType.Vector)
                    {
                        ...
                        // Line width
                        float lineWidth = layer.LineWidth;
                        ...
                        // Point size
                        float pointSize = layer.PointSize;
                        ...
                        // Editable
                        bool editable = layer.IsEditable;
