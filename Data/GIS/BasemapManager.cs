...
namespace GeoscientistToolkit.UI.GIS
{
    public class BasemapManager
    {
        ...
        public async Task<TileData> GetTileAsync(int x, int y, int z)
        {
            ...
            string cacheKey = $"{CurrentProvider.Id}_{z}_{x}_{y}";
            string cachePath = Path.Combine(_cacheDirectory, CurrentProvider.Id, $"{z}", $"{x}_{y}.png");
        ...
        public GeoTiffData LoadGeoTiff(string path)
        {
            ...
            try
            {
                ...
                int bands = Math.Min(3, geoTiff.BandCount);
                ...
                for (int band = 1; band <= bands; band++)
                {
                    ...
                    {
                        geoTiff.Data[i * 4 + (band - 1)] = buffer[i];
                    }
                }
                
                // Set alpha channel
                for (int i = 0; i < geoTiff.Width * geoTiff.Height; i++)
                {
                    geoTiff.Data[i * 4 + 3] = 255;
                }
