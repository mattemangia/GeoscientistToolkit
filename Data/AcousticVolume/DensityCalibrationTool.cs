...
namespace GeoscientistToolkit.Data.AcousticVolume
{
    public class DensityCalibrationTool
    {
        ...
        private enum CalibrationMode { GrayscaleMapping, MeanDensity, ManualSelection }
        ...
        private List<CalibrationRegion> _regions = new List<CalibrationRegion>();
        ...
        private void DrawManualSelection()
        {
            ...
            {
                _regions.Add(new CalibrationRegion());
            }
            ...
            for (int i = 0; i < _regions.Count; i++)
            {
                ...
                string name = _regions[i].Name;
                if (ImGui.InputText("Name", ref name, 64))
                {
                    _regions[i].Name = name;
                }
                
                float density = _regions[i].Density;
                if (ImGui.InputFloat("Density", ref density, 10, 100, "%.0f"))
                {
                    _regions[i].Density = density;
                }
                
                Vector4 color = _regions[i].Color;
                if (ImGui.ColorEdit4("Color", ref color, ImGuiColorEditFlags.NoInputs))
                {
                    _regions[i].Color = color;
                }
                
                if (ImGui.Button("Select"))
                {
                    _regions[i].IsSelecting = true;
                }
                ImGui.SameLine();
        ...
        private void DrawPreview()
        {
            ...
            if (_showPreview)
            {
                ImGui.Image(IntPtr.Zero, new Vector2(256, 256));
            }
        }
        ...
        {
            Parallel.For(0, _ctDataset.Depth, (int z) =>
            {
                var graySlice = new byte[_ctDataset.Width * _ctDataset.Height];
                _ctDataset.VolumeData.ReadSliceZ(z, graySlice);
                ...
                for (int i = 0; i < graySlice.Length; i++)
                {
                    float normalized = graySlice[i] / 255f;
                    float density = _meanDensity * (1 + _densityVariation * (normalized - 0.5f) * 2);
            
                    int x = i % _ctDataset.Width;
                    int y = i / _ctDataset.Width;
        ...
        public DensityVolume GetDensityVolume() => _densityVolume;
    }
