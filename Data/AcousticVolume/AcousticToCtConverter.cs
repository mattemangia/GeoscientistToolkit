...
public class AcousticToCtConverterDialog
{
    ...
    public void Draw()
    {
        ...
        if (ImGui.Begin("Convert Acoustic Volume to Greyscale", ref _isOpen,
                ImGuiWindowFlags.Modal | ImGuiWindowFlags.NoCollapse))
        {
            ...
            if (_isConverting) ImGui.BeginDisabled();
            ...
            if (_sourceDataset.DamageField == null) ImGui.BeginDisabled();
            ImGui.Checkbox("Export Damage Field as a material", ref _exportDamageAsMaterial);
            if (_sourceDataset.DamageField == null)
            {
                ImGui.EndDisabled();
                if (ImGui.IsItemHovered()) ImGui.SetTooltip("Damage field is not available in this dataset.");
            }
            ...
            if (_exportDialog.Submit())
            {
                ...
                var datasetName = Path.GetFileName(path);
            ...
            if (_isConverting) ImGui.EndDisabled();
        ...
        else
        {
            ...
            if (_isConverting) _conversionCts?.Cancel();
        }
    }
    ...
    private async Task PerformConversion(string outputDirectory, string datasetName, CancellationToken token)
    {
        try
        {
            ...
            var width = sourceGrayscaleVolume.Width;
            var height = sourceGrayscaleVolume.Height;
            var depth = sourceGrayscaleVolume.Depth;

            var datasetFolderPath = Path.Combine(outputDirectory, datasetName);
            ...
            var volumePath = Path.Combine(datasetFolderPath, $"{datasetName}.Volume.bin");
            ...
            if (_exportDamageAsMaterial && _sourceDataset.DamageField != null)
            {
                ...
                var labelVolumePath = Path.Combine(datasetFolderPath, $"{datasetName}.Labels.bin");
                var newLabelVolume = new ChunkedLabelVolume(width, height, depth, ChunkedVolume.DEFAULT_CHUNK_DIM,
                    false, labelVolumePath);
                var materials = new List<Material>
                {
                    new(0, "Exterior", new Vector4(0, 0, 0, 0)),
                    new(1, "Damage", new Vector4(1, 0, 0, 1)) // Red for damage
                };
                ...
                for (var z = 0; z < depth; z++)
                {
                    ...
                    for (var i = 0; i < damageSliceBuffer.Length; i++)
                        labelSliceBuffer[i] = damageSliceBuffer[i] > 10 ? (byte)1 : (byte)0;
                    newLabelVolume.WriteSliceZ(z, labelSliceBuffer);
                    _progressDialog.Update(0.7f + 0.2f * ((float)z / depth),
                        $"Processing damage slice {z + 1}/{depth}");
                }

                newLabelVolume.SaveAsBin(newLabelVolume.FilePath);
                newLabelVolume.Dispose();

                var ctDatasetForMaterials = new CtImageStackDataset(datasetName, datasetFolderPath)
                    { Materials = materials };
