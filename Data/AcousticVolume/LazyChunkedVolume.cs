...
public class LazyChunkedVolume : IGrayscaleVolumeData
{
    ...
    public void ReadSliceZ(int z, byte[] buffer)
    {
        ...
        Parallel.ForEach(chunksNeeded, chunkIndex => { GetOrLoadChunk(chunkIndex); });
        ...
        for (var cx = 0; cx < _chunkCountX; cx++)
        {
            ...
            {
                var ly = y % ChunkDim;
                var srcOffset = lz * ChunkDim * ChunkDim + ly * ChunkDim;
    ...
    private async Task InitializeAsync(string filePath)
    {
        ...
        await Task.Run(() =>
        {
            ...
            var bitsPerPixel = _reader.ReadInt32();
        ...
        Logger.Log(
            $"[LazyChunkedVolume] Initialized lazy volume from {Path.GetFileName(filePath)}: {Width}x{Height}x{Depth}");
    }
    ...
    private byte[] GetOrLoadChunk(int chunkIndex)
    {
        lock (_cacheLock)
        {
            ...
            var chunk = LoadChunkFromFile(chunkIndex);
    ...
    private byte[] LoadChunkFromFile(int chunkIndex)
    {
        try
        {
            var offset = _headerSize + (long)chunkIndex * _chunkSize;
    ...
    /// </summary>
    public async Task PreloadRegionAsync(int xMin, int xMax, int yMin, int yMax, int zMin, int zMax,
        IProgress<float> progress = null)
    {
        await Task.Run(() =>
        {
            var cxMin = xMin / ChunkDim;
            var cxMax = xMax / ChunkDim;
            var cyMin = yMin / ChunkDim;
            var cyMax = yMax / ChunkDim;
            var czMin = zMin / ChunkDim;
            var czMax = zMax / ChunkDim;

            var chunks = new List<int>();
            for (var cz = czMin; cz <= czMax && cz < _chunkCountZ; cz++)
            for (var cy = cyMin; cy <= cyMax && cy < _chunkCountY; cy++)
            for (var cx = cxMin; cx <= cxMax && cx < _chunkCountX; cx++)
