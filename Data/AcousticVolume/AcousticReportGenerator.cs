...
{
    // User-defined data
    private static bool _isVelocityLineSet;
    private static bool _isWaveformPointSet;
    ...
    public void Draw(Dataset dataset)
    {
        ...
        ImGui.TextWrapped(
            "Generate a comprehensive analysis report including metadata, statistics, and visualizations.");
        ImGui.Separator();
        ...
        var canAnalyzeDamage = ad.DamageField != null;
        ...
        var canDoVelocity = ad.DensityData != null;
        ...
        // Waveform Section
        var canDoWaveform = ad.TimeSeriesSnapshots != null && ad.TimeSeriesSnapshots.Count > 0;
        ...
        if (_includeVelocityProfile && canDoVelocity)
        {
            ...
            ImGui.SameLine();
            if (ImGui.Button("Select Line in Viewer...##Vel")) AcousticInteractionManager.StartLineDrawing();
        }
        ...
        if (_includeWaveform && canDoWaveform)
        {
            ...
            ImGui.TextColored(color, status);
            ImGui.SameLine();
            if (ImGui.Button("Select Point in Viewer...##Wf")) AcousticInteractionManager.StartPointSelection();
        }
        ...
        // --- Generation Button ---
        var canGenerate = IsReadyToGenerate();
        if (!canGenerate) ImGui.BeginDisabled();
        if (ImGui.Button("Generate and Export Report...", new Vector2(-1, 0)))
        {
            var extension = (ReportFormat)_format == ReportFormat.PlainText ? ".txt" : ".md";
            var desc = (ReportFormat)_format == ReportFormat.PlainText ? "Plain Text" : "Markdown Document";
    ...
    private string GetGenerationTooltip()
    {
        ...
        if (_includeVelocityProfile && !_isVelocityLineSet)
            return "A line for the velocity profile must be selected in the viewer.";
        if (_includeWaveform && !_isWaveformPointSet)
            return "A point for the waveform analysis must be selected in the viewer.";
    ...
    private void HandleDialogs(AcousticVolumeDataset dataset)
    {
        ...
        if (_isGenerating) _progressDialog.Submit();
    }
    ...
    private async Task GenerateReportAsync(AcousticVolumeDataset ad, string exportPath)
    {
        try
        {
            ...
            var imageSubFolder = $"{Path.GetFileNameWithoutExtension(exportPath)}_images";
            var imageDir = Path.Combine(Path.GetDirectoryName(exportPath), imageSubFolder);
            var useImages = (ReportFormat)_format == ReportFormat.Markdown && _renderImages;
            ...
            if (_includeSummary)
            {
                ...
                if (useImages)
                {
                    ...
                    var imagePath = await renderer.RenderSliceViewAsync(sliceDef, _progressDialog.CancellationToken);
                    sb.AppendLine(
                        $"![Combined Field Slice]({Path.Combine(imageSubFolder, Path.GetFileName(imagePath))})");
                    sb.AppendLine("*Fig 1: Axial slice (XY view) of the combined wave field at Z = " +
                                  sliceDef.SliceIndex + "*");
                    sb.AppendLine();
                }
            }
            ...
            if (_includeDamageAnalysis && ad.DamageField != null)
            {
                ...
                if (useImages)
                {
                    ...
                    var imagePath = await renderer.RenderSliceViewAsync(sliceDef, _progressDialog.CancellationToken);
                    sb.AppendLine(
                        $"![Damage Field Slice]({Path.Combine(imageSubFolder, Path.GetFileName(imagePath))})");
            ...
            if (_includeVelocityProfile && ad.DensityData != null)
            {
                ...
                {
                    var imagePath =
                        await renderer.RenderVelocityProfileAsync(vpData, vsData, _progressDialog.CancellationToken);
                    sb.AppendLine(
                        $"![Velocity Profile Plot]({Path.Combine(imageSubFolder, Path.GetFileName(imagePath))})");
            ...
            if (_includeWaveform && ad.TimeSeriesSnapshots?.Count > 0)
            {
                ...
                {
                    var imagePath = await renderer.RenderWaveformAsync(waveformData,
                        ad.TimeSeriesSnapshots.Last().SimulationTime, _progressDialog.CancellationToken);
    ...
    {
        // Plot Styling
        private static readonly Color PlotBgColor = new(20, 20, 25, 255);
        private static readonly Color PlotGridColor = new(70, 70, 70, 255);
        private static readonly Color PlotTextColor = new(220, 220, 220, 255);
        private static readonly Color VpPlotColor = new(60, 180, 255, 255);
        private static readonly Color VsPlotColor = new(255, 180, 60, 255);
        private static readonly Color WaveformPlotColor = new(100, 255, 100, 255);
        ...
        public async Task<string> RenderSliceViewAsync(SliceDefinition def, CancellationToken token)
        {
            return await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    {
                        var val = sliceData[i] / 255f;
                        var colorVec = def.FieldName == "Damage" ? GetHotColor(val) : GetJetColor(val);
                    ...
                    var path = Path.Combine(_outputDirectory,
                        $"slice_{def.FieldName}_{def.SliceAxis}{def.SliceIndex}.png");
        ...
        public async Task<string> RenderVelocityProfileAsync(List<float> vp, List<float> vs, CancellationToken token)
        {
            return await Task.Run(() =>
            {
                ...
                DrawFilledRect(rgbaBuffer, _config.ImageWidth, 0, 0, _config.ImageWidth, _config.PlotHeight,
                    PlotBgColor);
                ...
                if (vp == null || vp.Count == 0 || vs == null || vs.Count == 0)
                {
                    ...
                    var errorPath = Path.Combine(_outputDirectory, "velocity_profile_error.png");
                    SavePng(errorPath, _config.ImageWidth, _config.PlotHeight, rgbaBuffer);
                    return errorPath;
                }

                var yMin = Math.Min(vs.Min(), 0);
                var yMax = Math.Max(vp.Max(), vs.Max());
                ...
                DrawText(rgbaBuffer, _config.ImageWidth,
                    _config.PaddingLeft + 55, _config.PaddingTop + 20, "VP", PlotTextColor);
                DrawLine(rgbaBuffer, _config.ImageWidth,
                    _config.PaddingLeft + 30, _config.PaddingTop + 40,
                    _config.PaddingLeft + 50, _config.PaddingTop + 40, VsPlotColor, 2);
                DrawText(rgbaBuffer, _config.ImageWidth,
                    _config.PaddingLeft + 55, _config.PaddingTop + 35, "VS", PlotTextColor);

                var path = Path.Combine(_outputDirectory, "velocity_profile.png");
        ...
        public async Task<string> RenderWaveformAsync(float[] waveform, float duration, CancellationToken token)
        {
            return await Task.Run(() =>
            {
                ...
                if (waveform == null || waveform.Length == 0)
                {
                    DrawText(rgbaBuffer, _config.ImageWidth, _config.ImageWidth / 2 - 100, _config.PlotHeight / 2,
                        "NO DATA TO DISPLAY", PlotTextColor, 2);
                    var errorPath = Path.Combine(_outputDirectory, "waveform_error.png");
                    SavePng(errorPath, _config.ImageWidth, _config.PlotHeight, rgbaBuffer);
                    return errorPath;
                }

                var yMin = waveform.Min();
                var yMax = waveform.Max();
                if (Math.Abs(yMax - yMin) < 1e-9f) yMax = yMin + 1e-9f;
                ...
                var path = Path.Combine(_outputDirectory, "waveform.png");
                SavePng(path, _config.ImageWidth, _config.PlotHeight, rgbaBuffer);
                return path;
            }, token);
        }
        ...
        private static class SimpleFont
        {
            ...
            public static readonly Dictionary<char, bool[,]> FontMap = new()
            {
                // Uppercase letters
                {
                    'A',
            ...
            static SimpleFont()
            {
                ...
                for (var c = 'a'; c <= 'z'; c++)
                {
                    ...
                    if (FontMap.ContainsKey(upperC)) FontMap[c] = FontMap[upperC];
                }
            }
        }
        ...
        {
            for (var j = y; j < y + h && j < _config.PlotHeight; j++)
            for (var i = x; i < x + w && i < imgWidth; i++)
        ...
        private void DrawText(byte[] rgbaBuffer, int imgWidth, int x, int y, string text, Color color, int scale = 1)
        {
            ...
            foreach (var c in text)
            {
                ...
                {
                    for (var py = 0; py < SimpleFont.CharHeight; py++)
                    for (var px = 0; px < SimpleFont.CharWidth; px++)
                        if (pattern[py, px])
                        {
                            if (scale == 1)
                                SetPixel(rgbaBuffer, imgWidth, currentX + px, y + py, color);
                            else
                                DrawFilledRect(rgbaBuffer, imgWidth, currentX + px * scale, y + py * scale, scale,
                                    scale, color);
                        }
        ...
        private void DrawTextVertical(byte[] rgbaBuffer, int imgWidth, int x, int y, string text, Color color,
            int scale = 1)
        {
            var currentY = y;
            foreach (var c in text)
                    ...
                    for (var px = 0; px < SimpleFont.CharWidth; px++)
                        if (pattern[py, px])
                        {
                            if (scale == 1)
                                ...
                                DrawFilledRect(rgbaBuffer, imgWidth, x + px * scale, currentY + py * scale, scale,
                                    scale, color);
                        }
        ...
        {
            var plotWidth = imgWidth - _config.PaddingLeft - _config.PaddingRight;
            var plotHeight = imgHeight - _config.PaddingTop - _config.PaddingBottom;

            // Title
            var titleX = imgWidth / 2 - title.Length * 6 * 2 / 2;
            ...
            DrawTextVertical(rgbaBuffer, imgWidth, 10, imgHeight / 2 - yLabel.Length * 7 / 2, yLabel, PlotTextColor);

            for (var i = 0; i <= 5; i++)
            {
                var val = yMin + (yMax - yMin) * (i / 5.0f);
                var y = _config.PaddingTop + plotHeight - (int)(i / 5.0f * plotHeight);
                ...
                var label = $"{val:G3}";
                DrawText(rgbaBuffer, imgWidth, _config.PaddingLeft - 60, y - 4, label, PlotTextColor);
            }
            ...
            var xLabelX = imgWidth / 2 - xLabel.Length * 6 / 2;
            DrawText(rgbaBuffer, imgWidth, xLabelX, imgHeight - 20, xLabel, PlotTextColor);

            for (var i = 0; i <= 5; i++)
            {
                var x = _config.PaddingLeft + (int)(i / 5.0f * plotWidth);
                var val = xDataCount * (i / 5.0f);
                ...
                var label = $"{val:F0}";
                DrawText(rgbaBuffer, imgWidth, x - 10, _config.PaddingTop + plotHeight + 10, label, PlotTextColor);
            }
        }
            ...
            float yMin, float yMax, Color color)
        {
            ...
            {
                var x_norm = (float)i / Math.Max(1, data.Count - 1);
                var y_norm = (data[i] - yMin) / yRange;
                y_norm = Math.Max(0, Math.Min(1, y_norm)); // Clamp

                var x = _config.PaddingLeft + (int)(x_norm * plotWidth);
                var y = _config.PaddingTop + plotHeight - (int)(y_norm * plotHeight);

                if (lastX != -1) DrawLine(rgbaBuffer, imgWidth, lastX, lastY, x, y, color, _config.LineThickness);
                lastX = x;
                lastY = y;
            }
        }
        ...
        private byte[] ExtractSlice(ChunkedVolume vol, char axis, int index, int w, int h)
        {
            ...
            catch (Exception ex)
            {
                ...
                for (var i = 0; i < data.Length; i++) data[i] = 128;
            }
        ...
        private string CreateErrorImage(string message)
        {
            ...
            DrawText(rgbaData, w, 10, h / 2 - 5, message, new Color(255, 100, 100, 255));
            var path = Path.Combine(_outputDirectory, $"error_{DateTime.Now.Ticks}.png");
...
public static class AcousticAnalysisLogic
{
    ...
    public static string GetDamageStatisticsReport(AcousticVolumeDataset ad)
    {
        ...
        var damagedVolume = (float)count.countDamaged / count.totalVoxels;
    ...
    public static async Task<string> GetFractureOrientationReport(AcousticVolumeDataset ad, int threshold = 128,
        int minClusterSize = 100)
    {
        ...
        if (planes == null || planes.Count == 0)
            return
                "No significant fracture clusters were identified with the current settings (Threshold > 128, Min. Size > 100 voxels).\n";

        var sb = new StringBuilder();
        sb.AppendLine("### Major Fracture Planes");
        sb.AppendLine(
            "The following table lists the largest identified fracture clusters and their principal orientation, calculated via PCA.");
        sb.AppendLine();
        ...
        foreach (var plane in planes.Take(10)) // Report top 10
            sb.AppendLine(
                $"| {plane.Size} | ({plane.Centroid.X:F0}, {plane.Centroid.Y:F0}, {plane.Centroid.Z:F0}) | {plane.Dip:F1} | {plane.Azimuth:F1} |");
        sb.AppendLine();
        return sb.ToString();
    }

    public static async Task<(string report, List<float> vpData, List<float> vsData)> GetVelocityProfileReport(
        AcousticVolumeDataset ad)
    {
        ...
        var avgVp = vpData.Average();
        var avgVs = vsData.Average();
    ...
    {
        var waveform = await Task.Run(() => ExtractWaveform_Internal(ad));
        if (waveform == null || waveform.Length == 0)
        ...
        {
            var absVal = Math.Abs(waveform[i]);
            if (absVal > maxAmp) maxAmp = absVal;
            sumOfSquares += waveform[i] * waveform[i];
        }

        var rms = (float)Math.Sqrt(sumOfSquares / waveform.Length);
        var duration = ad.TimeSeriesSnapshots.Last().SimulationTime;
    ...
    private static (byte min, byte max, double mean, double stdDev, (long totalVoxels, long countDamaged) counts)
        CalculateVolumeStats(ChunkedVolume volume)
    {
        long count = 0;
        double sum = 0;
        double sumOfSquares = 0;
    ...
    public static float[] ExtractWaveform_Internal(AcousticVolumeDataset ad)
    {
        ...
        var numTimeSteps = ad.TimeSeriesSnapshots.Count;
        var waveform = new float[numTimeSteps];
