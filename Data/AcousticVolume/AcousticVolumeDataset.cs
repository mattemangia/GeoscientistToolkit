...
public class AcousticVolumeDataset : Dataset, ISerializableDataset
{
    ...
    public override long GetSizeInBytes()
    {
        ...
        foreach (var snapshot in TimeSeriesSnapshots) size += snapshot.GetSizeInBytes();
    ...
    public override void Load()
    {
        ...
        try
        {
            ...
            var pWavePath = Path.Combine(FilePath, "PWaveField.bin");
            ...
            var sWavePath = Path.Combine(FilePath, "SWaveField.bin");
            if (File.Exists(sWavePath)) SWaveField = ChunkedVolume.LoadFromBinAsync(sWavePath, false).Result;

            var combinedPath = Path.Combine(FilePath, "CombinedField.bin");
            ...
            // Load damage field
            var damagePath = Path.Combine(FilePath, "DamageField.bin");
            if (File.Exists(damagePath)) DamageField = ChunkedVolume.LoadFromBinAsync(damagePath, false).Result;

            // Load metadata
            var metadataPath = Path.Combine(FilePath, "metadata.json");
            if (File.Exists(metadataPath)) LoadMetadata(metadataPath);

            // Load calibration data
            var calibrationPath = Path.Combine(FilePath, "calibration.json");
            if (File.Exists(calibrationPath)) LoadCalibration(calibrationPath);
    ...
    {
        var densityPath = Path.Combine(FilePath, "Density.bin");
        var youngsPath = Path.Combine(FilePath, "YoungsModulus.bin");
        var poissonPath = Path.Combine(FilePath, "PoissonRatio.bin");
    ...
    private void SaveMetadata()
    {
        try
        {
            ...
            var path = Path.Combine(FilePath, "metadata.json");
    ...
    private void SaveCalibration()
    {
        ...
        try
        {
            ...
            var path = Path.Combine(FilePath, "calibration.json");
            File.WriteAllText(path, json);
        }
        catch (Exception ex)
        {
    ...
    {
        var timeSeriesDir = Path.Combine(FilePath, "TimeSeries");
        ...
        foreach (var file in files)
            try
            {
                ...
                if (snapshot != null) TimeSeriesSnapshots.Add(snapshot);
            }
            catch (Exception ex)
            {
    ...
    private void SaveTimeSeries()
    {
        ...
        for (var i = 0; i < TimeSeriesSnapshots.Count; i++)
        {
...
public class WaveFieldSnapshot
{
    ...
    private byte[] CompressField(float[,,] field)
    {
        ...
        for (var x = 0; x < Width; x++)
        {
            var normalized = (field[x, y, z] - _minValue) / range;
    ...
    {
        var compressed = GetCompressedVelocityField(component);
    ...
    private float[,,] DecompressField(byte[] compressed)
    {
        ...
        for (var x = 0; x < Width; x++)
        {
            var normalized = compressed[idx++] / 255f;
