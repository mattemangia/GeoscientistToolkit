...
namespace GeoscientistToolkit.UI.AcousticVolume
{
    ...
    public class WaveformViewer : IDisposable
    {
        ...
        private float _profileLengthMm = 0;
        private float _timeRangeMs = 0;
        ...
        private bool _useLogarithmicScale = false;
        ...
        private bool _showFrequencyAnalysis = false;
    ...
    if (ImGui.Begin("Frequency Analysis", ref _showFrequencyAnalysis))
    {
        ...
        if (_frequencySpectrum != null && _frequencyBins != null)
        {
            ...
            if (_frequencySpectrum.Length > 0)
            {
                ...
                float[] spectrumDb = new float[_frequencySpectrum.Length];
                float epsilon = 1e-12f;
                for (int i = 0; i < _frequencySpectrum.Length; i++)
                {
            ...
            ImGui.Separator();
            if (ImGui.Button("Export Spectrum as CSV"))
            {
                ExportFrequencySpectrumCsv();
            }
        }
        else
        {
        ...
        private void DrawSetupTab()
        {
            ...
            {
                ImGui.SetTooltip("Select which velocity component to visualize");
            }
        ...
        private void DrawDisplayTab()
        {
            ...
            if (ImGui.SliderFloat("Scale", ref _amplitudeScale, 0.1f, 10.0f, "%.1fx"))
            {
                UpdateProfileTexture();
            }
            ...
            if (ImGui.IsItemHovered())
            {
                ImGui.SetTooltip("Apply logarithmic scaling to enhance weak signals");
            }
        ...
        private void DrawAnalysisTab()
        {
            ...
            if (_selectedTimeIndex >= 0 && _waveformAtTime != null)
            {
                ...
                // Statistics
                float rms = CalculateRMS(_waveformAtTime);
            ...
            if (_selectedDistanceIndex >= 0 && _timeSeriesAtDistance != null)
            {
                ...
                if (ImGui.Button("Analyze Frequency"))
                {
                    AnalyzeFrequency(_timeSeriesAtDistance);
                }
            }
            else
            {
        ...
        private void DrawExportTab()
        {
            ...
            {
                string filename = $"waveform_{DateTime.Now:yyyyMMdd_HHmmss}";
            ...
            {
                string filename = $"bscan_{DateTime.Now:yyyyMMdd_HHmmss}";
            ...
            // Quick stats export
            if (ImGui.Button("Copy Statistics to Clipboard"))
            {
        ...
        private void DrawProfileDisplay()
        {
            ...
            else if (_profileTexture != null && _profileTexture.IsValid && _profileData != null)
            {
                ...
                if (_showCrosshairs)
                {
                    DrawProfileOverlays(drawList, canvasPos, canvasSize);
                }
        ...
        {
            // Time axis (vertical)
            string timeLabel = $"Time (0 - {_timeRangeMs:F1} ms)";
            ...
            string distLabel = $"Distance (0 - {_profileLengthMm:F1} mm)";
        ...
        private void HandleProfileInteraction(Vector2 canvasPos, Vector2 canvasSize)
        {
            ...
            if (ImGui.IsItemHovered())
            {
                ...
                Vector2 relPos = mousePos / canvasSize;
                
                // Show tooltip with coordinates
                int timeIdx = (int)(relPos.Y * _profileData.GetLength(0));
                int distIdx = (int)(relPos.X * _profileData.GetLength(1));
                
                if (timeIdx >= 0 && timeIdx < _profileData.GetLength(0) &&
                    distIdx >= 0 && distIdx < _profileData.GetLength(1))
                {
                    float time = timeIdx * _timeRangeMs / _profileData.GetLength(0);
                    float dist = distIdx * _profileLengthMm / _profileData.GetLength(1);
                    float value = _profileData[timeIdx, distIdx];
        ...
        private void ExtractProfileData()
        {
            ...
            for (int t = 0; t < numTimeSteps; t++)
            {
                ...
                for (int p = 0; p < numLinePoints; p++)
                {
                    ...
                    float value = GetVelocityComponent(vx, vy, vz, point);
        ...
        private void CalculateProfileMetadata()
        {
            ...
            if (_linePoints.Count > 1)
            {
                ...
                if (volume != null && volume.PixelSize > 0)
                {
                    ...
                    float pixels = (float)Math.Sqrt(dx * dx + dy * dy + dz * dz);
            ...
            while (true)
            {
                ...
                switch (viewIndex)
                {
                    ...
                    case 1: // XZ View
                        ...
                        break;
                    case 2: // YZ View
                        x_vol = slice_coord; y_vol = x1_2d; z_vol = y1_2d;
                        break;
                    default:
                        return points;
                }
        ...
        private void ExtractWaveformAtTime()
        {
            ...
            float time = _selectedTimeIndex * _timeRangeMs / _profileData.GetLength(0);
        ...
        private void ExtractTimeSeriesAtDistance()
        {
            ...
            float dist = _selectedDistanceIndex * _profileLengthMm / _profileData.GetLength(1);
        ...
        private void UpdateProfileTexture()
        {
            ...
            int height = _profileData.GetLength(0);
            int width = _profileData.GetLength(1);
            byte[] rgbaData = new byte[width * height * 4];
            
            float displayMax = _autoNormalize ? _maxAbsAmplitude : _manualMax;
            float displayMin = _autoNormalize ? -_maxAbsAmplitude : _manualMin;
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                {
                    float value = _profileData[y, x] * _amplitudeScale;
                    ...
                    float normalized = (value - displayMin) / range;
        ...
        {
            float r = 0.267f + t * (2.053f - t * (29.256f - t * (127.357f - t * (214.534f - t * 128.389f))));
            float g = 0.005f + t * (1.107f + t * (4.295f - t * (4.936f + t * (-7.422f + t * 4.025f))));
            float b = 0.329f + t * (0.460f - t * (5.581f + t * (27.207f - t * (50.113f + t * 28.189f))));
        ...
        {
            // WAV export
            if (_exportWavDialog.Submit())
            {
                _ = ExportAsWavAsync(_exportWavDialog.SelectedPath);
            }
            
            // CSV export
            if (_exportCsvDialog.Submit())
            {
                _ = ExportAsCsvAsync(_exportCsvDialog.SelectedPath);
            }
            
            // Image export
            if (_exportImageDialog.Submit())
            {
                _ = ExportAsImageAsync(_exportImageDialog.SelectedPath);
            }
        }
        ...
        private async Task ExportAsWavAsync(string path)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    int originalSamples = _timeSeriesAtDistance.Length;
                    float duration = _timeRangeMs / 1000.0f / _playbackSpeedMultiplier;
                    int targetSamples = (int)(duration * _sampleRate);
                    
                    float[] resampled = new float[targetSamples];
                    for (int i = 0; i < targetSamples; i++)
                    {
                        float srcIndex = (float)i / targetSamples * originalSamples;
                    ...
                    if (_normalizeAudio)
                    {
                        float max = resampled.Max(Math.Abs);
                        if (max > 0)
                        {
        ...
        private void WriteWavFile(string path, float[] data, int sampleRate)
        {
            ...
            using (var writer = new BinaryWriter(fs))
            {
                ...
                {
                    short pcm = (short)(Math.Clamp(sample, -1, 1) * 32767);
                    writer.Write(pcm);
                }
            }
        }
            ...
            try
            {
                await Task.Run(() =>
                {
                    using (var writer = new StreamWriter(path))
                    {
                        ...
                        for (int d = 0; d < _profileData.GetLength(1); d++)
                        {
                            float dist = d * _profileLengthMm / _profileData.GetLength(1);
                            writer.Write($",Dist_{dist:F2}mm");
                        }
                        writer.WriteLine();
                        
                        // Data
                        for (int t = 0; t < _profileData.GetLength(0); t++)
                        {
                            float time = t * _timeRangeMs / _profileData.GetLength(0);
                            writer.Write($"{time:F4}");
                            
                            for (int d = 0; d < _profileData.GetLength(1); d++)
                            {
                                writer.Write($",{_profileData[t, d]:E6}");
                            }
                            writer.WriteLine();
        ...
        private async Task ExportAsImageAsync(string path)
{
    ...
    finally
    {
        ...
        private void AnalyzeFrequency(float[] timeSeries)
        {
            ...
            // Perform the DFT
            for (int k = 0; k < n / 2; k++) // Only compute first half due to symmetry
            {
            ...
            float samplingRate = 1000.0f / (_timeRangeMs / timeSeries.Length); // Hz
        ...
        private void ExportFrequencySpectrumCsv()
        {
            ...
            string fileName = $"frequency_spectrum_{DateTime.Now:yyyyMMdd_HHmmss}.csv";
            ...
            try
            {
                using (var writer = new StreamWriter(path))
                {
                    ...
                    for (int i = 0; i < _frequencySpectrum.Length; i++)
                    {
                        float powerDb = 20.0f * (float)Math.Log10(Math.Max(_frequencySpectrum[i], 1e-12f));
