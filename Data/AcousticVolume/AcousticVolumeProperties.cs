...
public class AcousticVolumeProperties : IDatasetPropertiesRenderer
{
    public void Draw(Dataset dataset)
    {
        ...
        if (ImGui.CollapsingHeader("Wave Properties", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ...
            {
                var quality = acousticDataset.PWaveVelocity / acousticDataset.SWaveVelocity;
            ...
            if (acousticDataset.PWaveVelocity > 0 && acousticDataset.SWaveVelocity > 0)
            {
                ImGui.Spacing();
                ImGui.TextDisabled(
                    $"P-wave is {acousticDataset.PWaveVelocity / acousticDataset.SWaveVelocity:F2}x faster than S-wave");
            }
        }
        ...
        if (ImGui.CollapsingHeader("Source Parameters"))
        {
            ...
            {
                var pWavelength =
                    acousticDataset.PWaveVelocity / (acousticDataset.SourceFrequencyKHz * 1000); // in meters
                ImGui.Text($"P-Wave: {pWavelength * 1000:F2} mm");

                // Resolution info
                if (acousticDataset.PWaveField != null && acousticDataset.VoxelSize > 0)
                {
                    var resolution = pWavelength / acousticDataset.VoxelSize; // meters / meters
            ...
            {
                var sWavelength =
                    acousticDataset.SWaveVelocity / (acousticDataset.SourceFrequencyKHz * 1000); // in meters
                ImGui.Text($"S-Wave: {sWavelength * 1000:F2} mm");

                // Resolution info
                if (acousticDataset.SWaveField != null && acousticDataset.VoxelSize > 0)
                {
                    var resolution = sWavelength / acousticDataset.VoxelSize; // meters / meters
                    ImGui.TextDisabled($"  ({resolution:F1} voxels per wavelength)");
                }
            }
        ...
        if (ImGui.CollapsingHeader("Simulation Info"))
        {
            ...
            {
                var stepsPerSecond = acousticDataset.TimeSteps / acousticDataset.ComputationTime.TotalSeconds;
                ImGui.Text($"Performance: {stepsPerSecond:F0} steps/sec");

                // Estimate time per voxel
                if (acousticDataset.CombinedWaveField != null)
                {
                    var totalVoxels = (long)acousticDataset.CombinedWaveField.Width *
                                      acousticDataset.CombinedWaveField.Height *
                                      acousticDataset.CombinedWaveField.Depth;
                    var timePerVoxel = acousticDataset.ComputationTime.TotalMilliseconds /
                                       (totalVoxels * acousticDataset.TimeSteps);
        ...
        if (ImGui.CollapsingHeader("Volume Data"))
        {
            ...
            if (!hasData) ImGui.TextDisabled("No volume data loaded");
        ...
        {
            var totalSize = acousticDataset.GetSizeInBytes();
            ...
            if (acousticDataset.PWaveField != null)
            {
                var size = (long)acousticDataset.PWaveField.Width *
            ...
            if (acousticDataset.SWaveField != null)
            {
                var size = (long)acousticDataset.SWaveField.Width *
            ...
            if (acousticDataset.CombinedWaveField != null)
            {
                var size = (long)acousticDataset.CombinedWaveField.Width *
                           acousticDataset.CombinedWaveField.Height *
                           acousticDataset.CombinedWaveField.Depth;
            ...
            if (acousticDataset.IsMissing)
                ImGui.TextColored(new Vector4(1, 0.5f, 0, 1), "Warning: Dataset files are missing!");
        }
    }

    private void DrawVolumeInfo(string name, ChunkedVolume volume, AcousticVolumeDataset acousticDataset)
    {
        ImGui.Text($"{name}:");
        ImGui.Indent();
        ImGui.Text($"Dimensions: {volume.Width} × {volume.Height} × {volume.Depth}");

        var voxelCount = (long)volume.Width * volume.Height * volume.Depth;
        ImGui.Text($"Voxels: {voxelCount:N0}");

        // Calculate and display physical size if voxel size is known
        if (acousticDataset.VoxelSize > 0)
        {
            var sizeX = volume.Width * acousticDataset.VoxelSize * 1000; // in mm
            var sizeY = volume.Height * acousticDataset.VoxelSize * 1000; // in mm
            var sizeZ = volume.Depth * acousticDataset.VoxelSize * 1000; // in mm
