...
public class ImageTools : IDatasetTools
{
    ...
    private readonly ImGuiExportFileDialog _exportClassDialog = new("ExportClassification", "Export Classification");
    ...
    private readonly ImageSegmentationToolsUI _segmentationTools;
    ...
    private (int[] labels, Vector3[] centers, int iterations, float convergence) KMeansClustering(List<Vector3> data,
        int k)
    {
        var rand = new Random();
        var n = data.Count;
    ...
    private void CreateNDVIVisualization(ImageDataset imageDataset, float[] ndviData)
    {
        ...
        for (var i = 0; i < ndviData.Length; i++)
        {
            ...
            {
                var normalized = Math.Min(1.0f, ndvi);
    ...
    private void ExportParticleAnalysisResults(ImageDataset imageDataset)
    {
        ...
        using (var writer = new StreamWriter(outputPath))
        {
            ...
            foreach (var p in _detectedParticles)
                writer.WriteLine(
                    $"{p.Id},{p.Area},{p.EquivalentDiameter},{p.Circularity},{p.AspectRatio},{p.CenterX},{p.CenterY}");
        }
...
public class TagManager
{
    ...
    {
        var filename = dataset.Name.ToLower();
