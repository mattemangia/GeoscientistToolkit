...
namespace GeoscientistToolkit.Data.Image
{
    public class ImageTools : IDatasetTools
    {
        ...
        public void Draw(Dataset dataset)
        {
            ...
            // Get available tools based on tags
            ...
            if (ImGui.BeginTabBar("ImageToolsTabs"))
            {
                ...
                // Basic adjustments (always available)
        ...
        private void DrawAdjustments(ImageDataset imageDataset)
        {
            ...
            if (ImGui.Button("Apply Adjustments", new Vector2(-1, 0)))
            {
                ApplyImageAdjustments(imageDataset);
                Logger.Log($"Applied adjustments to {imageDataset.Name}");
            }
        }
        ...
        private void ApplyImageAdjustments(ImageDataset imageDataset)
        {
            ...
            byte[] adjustedData = new byte[imageDataset.ImageData.Length];
            
            for (int i = 0; i < imageDataset.ImageData.Length; i += 4)
            {
                for (int c = 0; c < 3; c++)
                {
                    float value = imageDataset.ImageData[i + c];
                    value = (value - 128) * _contrast + 128 + _brightness * 255;
                    adjustedData[i + c] = (byte)Math.Clamp(value, 0, 255);
                }
                adjustedData[i + 3] = imageDataset.ImageData[i + 3]; // Keep alpha
            }
            
            Array.Copy(adjustedData, imageDataset.ImageData, adjustedData.Length);
        }
                ...
                if (ImGui.CollapsingHeader("Particle Details"))
                {
                    if (ImGui.BeginTable("ParticleTable", 5))
                    {
                        ...
                        for (int i = 0; i < Math.Min(_detectedParticles.Count, 100); i++)
                        {
                            ...
                            ImGui.Text($"{p.Id}");
                            ImGui.TableNextColumn();
                            ImGui.Text($"{p.Area:F1}");
                            ImGui.TableNextColumn();
                            ImGui.Text($"{p.EquivalentDiameter:F1}");
                            ImGui.TableNextColumn();
                            ImGui.Text($"{p.Circularity:F3}");
                            ImGui.TableNextColumn();
                            ImGui.Text($"({p.CenterX:F0},{p.CenterY:F0})");
                        }
        ...
        private void PerformParticleAnalysis(ImageDataset imageDataset)
        {
            ...
            foreach (var component in components)
            {
                ...
                // Filter by size
                if (area < _particleMinSize || area > _particleMaxSize)
                    continue;
                ...
                if (particle.Circularity < _circularityThreshold)
                    continue;
        ...
        private ParticleData AnalyzeParticle(List<int> pixels, int id, int imageWidth)
        {
            ...
            particle.Circularity = Math.Min(1.0f, particle.Circularity); // Cap at 1
        ...
        private float CalculatePerimeter(List<int> pixels, int width)
        {
            ...
            foreach (int pixel in pixels)
            {
                ...
                int neighbors = 0;
                if (pixelSet.Contains(pixel - 1)) neighbors++; // left
                if (pixelSet.Contains(pixel + 1)) neighbors++; // right
                if (pixelSet.Contains(pixel - width)) neighbors++; // top
                if (pixelSet.Contains(pixel + width)) neighbors++; // bottom
                
                // Edge pixels contribute to perimeter
                if (neighbors < 4)
                {
        ...
        private void StartPointCounting(ImageDataset imageDataset)
        {
            ...
            // Initialize counts for all minerals
            foreach (var mineral in _mineralTypes)
            {
                _mineralCounts[mineral] = 0;
            }
        }
        ...
        public void HandlePointCountClick(ImageDataset imageDataset, Vector2 clickPosition)
        {
            ...
            // Check if this point was already counted
            var existingPoint = _countedPoints.FirstOrDefault(p => 
        ...
        private void CalculateNDVI(ImageDataset imageDataset)
        {
            ...
            // Create visualization (map NDVI to color)
        ...
        private void CalculateEVI(ImageDataset imageDataset)
        {
            ...
            for (int i = 0; i < pixelCount; i++)
            {
                ...
                {
                    eviData[i] = G * ((nir - red) / denominator);
                }
                else
                {
                    eviData[i] = 0;
        ...
        private void PerformUnsupervisedClassification(ImageDataset imageDataset, int numClasses)
        {
            ...
            // Prepare feature vectors (RGB values for simplicity)
            List<Vector3> pixels = new List<Vector3>(pixelCount);
            for (int i = 0; i < pixelCount; i++)
            {
                int idx = i * 4;
                pixels.Add(new Vector3(
                    imageDataset.ImageData[idx],
                    imageDataset.ImageData[idx + 1],
            ...
            List<Vector3> data, int k)
        {
            ...
            while (iteration < _maxIterations && convergence > _convergenceThreshold)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    currentError += Vector3.DistanceSquared(data[i], newCenters[labels[i]]);
                }
        ...
        private byte[] ConvertToGrayscale(byte[] imageData, int width, int height)
        {
            ...
            {
                int idx = i * 4;
                grayscale[i] = (byte)(0.299f * imageData[idx] + 
        ...
        private byte[] ApplyThreshold(byte[] grayscale, float threshold)
        {
            ...
            {
                binary[i] = (byte)(grayscale[i] > threshold ? 255 : 0);
            }
            return binary;
        }
        ...
        {
            // Create a new image dataset for NDVI visualization
            ...
            for (int i = 0; i < ndviData.Length; i++)
            {
                ...
                // Map NDVI to color gradient (red to green)
                if (ndvi < 0)
                {
                    // Water/bare soil (blue)
                    visualData[idx] = 0;
                    visualData[idx + 1] = 0;
                ...
                {
                    // Vegetation (red to yellow to green)
                    float normalized = Math.Min(1.0f, ndvi);
            ...
            // Store as separate visualization dataset
        ...
        private Vector3[] GenerateDistinctColors(int count)
        {
            ...
            {
                float hue = i * hueStep;
                colors[i] = HsvToRgb(hue, 0.8f, 0.9f);
            }
        ...
        private void ExportParticleAnalysisResults(ImageDataset imageDataset)
        {
            ...
            using (StreamWriter writer = new StreamWriter(outputPath))
            {
                ...
                {
                    writer.WriteLine($"{p.Id},{p.Area},{p.EquivalentDiameter},{p.Circularity}," +
                                   $"{p.AspectRatio},{p.CenterX},{p.CenterY}");
                }
            }
            
            Logger.Log($"Exported particle analysis results to {outputPath}");
        }
        private readonly ImGuiExportFileDialog _exportClassDialog =
            new ImGuiExportFileDialog("ExportClassification", "Export Classification");
...
{
    // If not already active, open the dialog with sensible defaults.
    if (!_exportClassDialogActive)
    {
        _exportClassDialog.SetExtensions(
            (".tiff", "TIFF (RGBA)"),
            (".png",  "PNG"),
            (".jpg",  "JPEG")
        );
        ...
        string startDir    = string.IsNullOrEmpty(imageDataset.FilePath)
    ...
    // Draw & process the dialog every frame until the user confirms/cancels.
    bool confirmed = _exportClassDialog.Submit();
    if (!confirmed)
        return; // keep the dialog open; call this method again next frame
    ...
    try
    {
        ...
        // Save the RGBA image with the chosen extension.
        SaveRgbaImage(imagePath, rgba, imageDataset.Width, imageDataset.Height);

        // Save the statistics CSV next to the image (same basename).
    ...
    {
        _exportClassDialogActive = false; // close state
    }
}

/// <summary>
/// Saves an RGBA8888 buffer to TIFF (via LibTiff) or PNG/JPEG/BMP (via Skia),
/// based on the file extension in <paramref name="path"/>.
/// </summary>
private void SaveRgbaImage(string path, byte[] rgba, int width, int height)
{
    ...
    // If unknown extension, fall back to .png
    if (fmt == SKEncodedImageFormat.Png && ext != ".png")
    ...
    // Helper class for tag management
    public class TagManager
    {
        public void SuggestTags(ImageDataset dataset)
        {
            // Analyze filename and metadata to suggest appropriate tags
            ...
            // Check for multispectral based on file size or metadata
