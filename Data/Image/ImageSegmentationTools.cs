...
public static class ImageSegmentationTools
{
        ...
        byte materialId, int minValue, int maxValue, bool addMode = true)
    {
        ...
        {
            for (var x = 0; x < segmentation.Width; x++)
            {
                var idx = y * segmentation.Width + x;
                var pixelIdx = idx * 4;
                var gray = (byte)(0.299f * imageData[pixelIdx] + 0.587f * imageData[pixelIdx + 1] +
        ...
        bool addMode = true)
    {
        ...
        var maxX = Math.Min(w - 1, centerX + radius);
    ...
    {
        segmentation.SaveUndoState();
        var grayscale = new byte[segmentation.Width * segmentation.Height];
        for (var i = 0; i < grayscale.Length; i++)
        {
            var pixelIdx = i * 4;
            grayscale[i] = (byte)(0.299f * imageData[pixelIdx] + 0.587f * imageData[pixelIdx + 1] +
                                  0.114f * imageData[pixelIdx + 2]);
        }

        var opened = MorphologicalOpening(grayscale, segmentation.Width, segmentation.Height, kernelSize);
        var tophat = new byte[grayscale.Length];
        for (var i = 0; i < grayscale.Length; i++)
        {
            var diff = grayscale[i] - opened[i];
            tophat[i] = (byte)Math.Max(0, diff);
            if (tophat[i] >= threshold) segmentation.LabelData[i] = materialId;
        }
    }

    private static byte[] MorphologicalOpening(byte[] image, int width, int height, int kernelSize)
    {
        var eroded = Erode(image, width, height, kernelSize);
        ...
        List<(int x, int y, byte materialId)> seeds)
    {
        ...
        segmentation.SaveUndoState();
        var gradient = ComputeGradientMagnitude(imageData, segmentation.Width, segmentation.Height);
        ...
        var inQueue = new bool[segmentation.Width, segmentation.Height];
        foreach (var seed in seeds)
            if (seed.x >= 0 && seed.x < segmentation.Width && seed.y >= 0 && seed.y < segmentation.Height)
            {
                var idx = seed.y * segmentation.Width + seed.x;
                segmentation.LabelData[idx] = seed.materialId;
                inQueue[seed.x, seed.y] = true;
                AddWatershedNeighbors(pq, inQueue, gradient, seed.x, seed.y, seed.materialId, segmentation.Width,
                    segmentation.Height);
            }
        ...
        while (pq.Count > 0)
        {
            var current = pq.Min;
            pq.Remove(current);
            var idx = current.y * segmentation.Width + current.x;
            if (segmentation.LabelData[idx] == 0)
            {
                segmentation.LabelData[idx] = current.label;
                AddWatershedNeighbors(pq, inQueue, gradient, current.x, current.y, current.label, segmentation.Width,
                    segmentation.Height);
            }
        }
    }

    private static void AddWatershedNeighbors(SortedSet<(float, int, int, byte)> pq, bool[,] inQueue, float[] gradient,
        int x, int y, byte label, int width, int height)
    {
        int[] dx = { -1, 1, 0, 0 };
        int[] dy = { 0, 0, -1, 1 };
        for (var i = 0; i < 4; i++)
    ...
    private static float[] ComputeGradientMagnitude(byte[] imageData, int width, int height)
    {
        ...
        for (var x = 1; x < width - 1; x++)
        {
            var idx = y * width + x;
            var pixelIdx = idx * 4;
            var center = 0.299f * imageData[pixelIdx] + 0.587f * imageData[pixelIdx + 1] +
                         0.114f * imageData[pixelIdx + 2];
            var leftIdx = (y * width + (x - 1)) * 4;
            var rightIdx = (y * width + x + 1) * 4;
            var left = 0.299f * imageData[leftIdx] + 0.587f * imageData[leftIdx + 1] + 0.114f * imageData[leftIdx + 2];
            var right = 0.299f * imageData[rightIdx] + 0.587f * imageData[rightIdx + 1] +
                        0.114f * imageData[rightIdx + 2];
            var gx = (right - left) / 2;
            var topIdx = ((y - 1) * width + x) * 4;
            var bottomIdx = ((y + 1) * width + x) * 4;
            var top = 0.299f * imageData[topIdx] + 0.587f * imageData[topIdx + 1] + 0.114f * imageData[topIdx + 2];
            var bottom = 0.299f * imageData[bottomIdx] + 0.587f * imageData[bottomIdx + 1] +
                         0.114f * imageData[bottomIdx + 2];
            var gy = (bottom - top) / 2;
...
public class ImageSegmentationToolsUI : IDatasetTools
{
    ...
    private Vector4 _newMaterialColor = new(1, 0, 0, 1);
    private string _newMaterialName = "New Material";
    private byte[] _previewBackupLabelData;
    private byte _selectedMaterialId;
    private int _selectedTool;
    private bool _showThresholdPreview;
    ...
    private void DrawThresholdTool()
    {
        ...
        var minChanged = ImGui.SliderInt("Min Value", ref _thresholdMin, 0, 255);
        var maxChanged = ImGui.SliderInt("Max Value", ref _thresholdMax, 0, 255);

        var previewStateChanged = ImGui.Checkbox("Show Preview", ref _showThresholdPreview);
    ...
    private void DrawTopHatTool()
    {
        ...
        if (ImGui.Button("Apply Top-Hat", new Vector2(-1, 0)))
        {
            if (_dataset == null) return;
            _dataset.Load();
            ImageSegmentationTools.ApplyTopHat(_dataset.ImageData, _dataset.GetOrCreateSegmentation(),
    ...
    private void DrawWatershedTool()
    {
        ...
        if (ImGui.Button("Apply Watershed", new Vector2(-1, 0)))
            ...
            {
                _dataset.Load();
                ImageSegmentationTools.ApplyWatershed(_dataset.ImageData, _dataset.GetOrCreateSegmentation(),
                    _watershedSeeds);
    ...
    private void DrawGeneralOperations(ImageSegmentationData segmentation)
    {
        ...
        if (_exportDialog.Submit())
            ImageSegmentationExporter.ExportLabeledImage(segmentation, _exportDialog.SelectedPath);
    ...
    public void HandleMouseClick(int x, int y)
    {
        ...
        // ImGui mouse state
        var lDown = ImGui.IsMouseDown(ImGuiMouseButton.Left);
        var rDown = ImGui.IsMouseDown(ImGuiMouseButton.Right);
        var lClicked = ImGui.IsMouseClicked(ImGuiMouseButton.Left);
        var rClicked = ImGui.IsMouseClicked(ImGuiMouseButton.Right);
        ...
        if (changed)
        {
            ...
            try
            {
                ...
                {
                    var key = (_dataset.FilePath ?? string.Empty) + "_segmentation";
    ...
    private void ApplyThresholdPreview()
    {
        ...
        Parallel.For(0, segmentation.Height, y =>
        {
            for (var x = 0; x < segmentation.Width; x++)
            {
                ...
                var gray = (byte)(0.299f * imageData[pixelIdx] +
