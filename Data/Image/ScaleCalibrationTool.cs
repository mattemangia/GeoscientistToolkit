...
public class ScaleCalibrationTool
{
    ...
    public bool HandleMouseClick(Vector2 imagePos, Vector2 displaySize, int imageWidth, int imageHeight,
        Vector2 clickPos)
    {
        if (!IsActive) return false;

        // Convert click position to image coordinates
        var relativePos = clickPos - imagePos;
        if (relativePos.X < 0 || relativePos.Y < 0 ||
            relativePos.X > displaySize.X || relativePos.Y > displaySize.Y)
            return false;

        var imageCoords = new Vector2(
    ...
    public void DrawOverlay(ImDrawListPtr dl, Vector2 imagePos, Vector2 displaySize, int imageWidth, int imageHeight)
    {
        ...
        if (_hasFirstPoint)
        {
            ...
            var screenFirst = imagePos + new Vector2(
                ...
                io.MousePos.Y >= imagePos.Y && io.MousePos.Y <= imagePos.Y + displaySize.Y)
            {
                ...
                var currentImageCoords = new Vector2(
                    relativePos.X / displaySize.X * imageWidth,
                    relativePos.Y / displaySize.Y * imageHeight
                );

                var pixelDistance = Vector2.Distance(_firstPoint, currentImageCoords);
                var distanceText = $"{pixelDistance:F1} pixels";

                var textPos = io.MousePos + new Vector2(10, -20);
    ...
    public bool DrawCalibrationDialog(out float pixelSize, out string unit)
    {
        ...
        {
            var pixelDistance = Vector2.Distance(_firstPoint, _secondPoint);
            ...
            if (ImGui.Combo("##Unit", ref _unitIndex, _units, _units.Length)) _unit = _units[_unitIndex];

            ImGui.Separator();

            // Calculate pixel size in micrometers
            var knownDistanceInMicrometers = _knownDistance * _unitConversions[_unitIndex];
            var calculatedPixelSize = knownDistanceInMicrometers / pixelDistance;
