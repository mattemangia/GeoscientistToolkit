        ...
        if (ImGui.CollapsingHeader("Histogram", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ...
            if (ImGui.BeginTabBar("HistogramTabs"))
            {
                if (ImGui.BeginTabItem("Luminance"))
                {
                    ...
                    ImGui.PlotHistogram("##Luminance", ref image.HistogramLuminance[0], image.HistogramLuminance.Length,
                        0, null, 0.0f, float.MaxValue, plotSize);
                    ImGui.PopStyleColor();
                    ImGui.EndTabItem();
                }
                ...
                if (image.HistogramR != null)
                    if (ImGui.BeginTabItem("Red"))
                    {
                        ...
                        ImGui.PlotHistogram("##Red", ref image.HistogramR[0], image.HistogramR.Length, 0, null, 0.0f,
                            float.MaxValue, plotSize);
                        ImGui.PopStyleColor();
                        ImGui.EndTabItem();
                    }
                ...
                if (image.HistogramG != null)
                    if (ImGui.BeginTabItem("Green"))
                    {
                        ...
                        ImGui.PlotHistogram("##Green", ref image.HistogramG[0], image.HistogramG.Length, 0, null, 0.0f,
                            float.MaxValue, plotSize);
                        ImGui.PopStyleColor();
                        ImGui.EndTabItem();
                    }
                ...
                if (image.HistogramB != null)
                    if (ImGui.BeginTabItem("Blue"))
                    {
                        ...
                        ImGui.PlotHistogram("##Blue", ref image.HistogramB[0], image.HistogramB.Length, 0, null, 0.0f,
                            float.MaxValue, plotSize);
                        ImGui.PopStyleColor();
                        ImGui.EndTabItem();
                    }
            ...
            if (image.HistogramLuminance != null)
            {
                ...
                if (ImGui.TreeNode("Histogram Statistics"))
                {
                    ...
                    for (var i = 0; i < image.HistogramLuminance.Length; i++)
                    {
                        var count = image.HistogramLuminance[i];
                        if (count > 0)
                        {
                            if (i < min) min = i;
                            if (i > max) max = i;
                    ...
                    var mean = totalPixels > 0 ? sum / totalPixels : 0;
