...
public class TwoDGeologyViewer : IDisposable
{
    ...
    private bool _showOriginalAsGhost = true; // Show original geology as ghost when restoration active
    ...
    public void StartInteractiveRestoration()
    {
        ...
        // Create a new restoration processor with the current cross-section
        ...
        Logger.Log("Interactive restoration started - use slider to adjust percentage");
    }
    ...
    private void RenderToolbar()
    {
        ...
        // RESTORATION CONTROLS - Enhanced UI
        ImGui.Separator();

        if (_restorationInitialized)
        {
            // Show active restoration controls
            ...
            // Ghost original toggle
            ImGui.Checkbox("Ghost", ref _showOriginalAsGhost);
            if (ImGui.IsItemHovered())
                ImGui.SetTooltip("Show original geology as ghost overlay");

            ImGui.SameLine();

            if (ImGui.Button("Apply"))
            {
                ApplyCurrentRestoration();
            }
            if (ImGui.IsItemHovered())
                ImGui.SetTooltip("Apply current restoration to cross-section\n(replaces original data)");
            ...
            if (ImGui.IsItemHovered())
                ImGui.SetTooltip("Cancel restoration and return to original");
        }
        else
        {
            // Show start restoration button
            if (ImGui.Button("Start Restoration"))
            {
                StartInteractiveRestoration();
            }

            if (ImGui.IsItemHovered())
                ImGui.SetTooltip("Start interactive structural restoration\nUnfold and unfault the geology");
        }
    ...
    private void RenderViewport()
    {
        ...
        // Draw geology based on restoration state
        if (_showRestorationOverlay && _restorationData != null)
        {
            // When restoration is active:
            // 1. Optionally draw original as ghost
            if (_showOriginalAsGhost)
            {
                DrawFormationsAsGhost(drawList, screenPos, availSize, _crossSection, _ghostOpacity);
            }

            // 2. Draw the restored geology (main view)
            DrawRestorationOverlay(drawList, screenPos, availSize);
        }
        else
        {
            // Normal view - draw original geology
            if (_showFormations)
            {
    ...
    private void DrawFormations(ImDrawListPtr drawList, Vector2 screenPos, Vector2 availSize)
    {
        foreach (var formation in _crossSection.Formations)
        {
            ...
            {
                DrawFilledFormation(drawList, formation, screenPos, availSize, formation.Color);
            }
    ...
    private void DrawFilledFormation(ImDrawListPtr drawList, ProjectedFormation formation, Vector2 screenPos, Vector2 availSize, Vector4 color)
    {
        if (formation.TopBoundary.Count < 2 || formation.BottomBoundary.Count < 2) return;

        var fillColor = ImGui.ColorConvertFloat4ToU32(color);

        // Draw as series of quads (two triangles each) for correct non-convex filling
        int numSegments = Math.Min(formation.TopBoundary.Count, formation.BottomBoundary.Count) - 1;

        for (int i = 0; i < numSegments; i++)
        {
            var topLeft = WorldToScreen(formation.TopBoundary[i], screenPos, availSize);
            var topRight = WorldToScreen(formation.TopBoundary[i + 1], screenPos, availSize);
            var bottomRight = WorldToScreen(formation.BottomBoundary[i + 1], screenPos, availSize);
            var bottomLeft = WorldToScreen(formation.BottomBoundary[i], screenPos, availSize);

            // Draw two triangles to form a quad
            drawList.AddTriangleFilled(topLeft, topRight, bottomRight, fillColor);
            drawList.AddTriangleFilled(topLeft, bottomRight, bottomLeft, fillColor);
        }
    }

    private void DrawFormationsAsGhost(ImDrawListPtr drawList, Vector2 screenPos, Vector2 availSize, CrossSection section, float opacity)
    {
        foreach (var formation in section.Formations)
        {
            if (!formation.GetIsVisible()) continue;

            var ghostColor = new Vector4(formation.Color.X, formation.Color.Y, formation.Color.Z, opacity);

            if (formation.TopBoundary.Count > 1 && formation.BottomBoundary.Count > 1)
            {
                DrawFilledFormation(drawList, formation, screenPos, availSize, ghostColor);
            }

            // Draw ghost boundaries
            var boundaryColor = ImGui.ColorConvertFloat4ToU32(new Vector4(0.5f, 0.5f, 0.5f, opacity));
            DrawPolyline(drawList, formation.TopBoundary, screenPos, availSize, boundaryColor, 1f);
            DrawPolyline(drawList, formation.BottomBoundary, screenPos, availSize, boundaryColor, 1f);
        }
    }
    ...
    private void DrawTopography(ImDrawListPtr drawList, Vector2 screenPos, Vector2 availSize)
    {
        ...
        {
            drawList.AddLine(points[i], points[i + 1], _topographyColor, 3f);
        }
    }
    ...
    private void DrawRestorationOverlay(ImDrawListPtr drawList, Vector2 screenPos, Vector2 availSize)
    {
        ...
        // Draw restored formations with full opacity
        foreach (var formation in _restorationData.Formations)
        {
            if (!formation.GetIsVisible()) continue;

            var color = formation.Color;
            color.W = 0.9f; // High opacity for restored view

            if (formation.TopBoundary.Count > 1 && formation.BottomBoundary.Count > 1)
            {
                DrawFilledFormation(drawList, formation, screenPos, availSize, color);
            }

            // Draw boundaries
            var boundaryColor = ImGui.ColorConvertFloat4ToU32(new Vector4(0.1f, 0.1f, 0.1f, 1f));
        ...
        // Draw restored faults
        var restoredFaultColor = ImGui.ColorConvertFloat4ToU32(new Vector4(0.8f, 0.2f, 0.2f, 1f));
        foreach (var fault in _restorationData.Faults)
        {
            DrawPolyline(drawList, fault.FaultTrace, screenPos, availSize, restoredFaultColor, 2.5f);
        }

        // Add restoration indicator
        var indicatorPos = screenPos + new Vector2(10, 10);
        var indicatorBg = ImGui.ColorConvertFloat4ToU32(new Vector4(0f, 0.3f, 0f, 0.8f));
        var indicatorText = ImGui.ColorConvertFloat4ToU32(new Vector4(1f, 1f, 1f, 1f));

        var text = $"RESTORED: {_restorationPercentage:F0}%";
        ...
        // Show what 0% and 100% mean
        var helpText = _restorationPercentage < 50f
            ? "Slide right to flatten layers"
            : "Slide left to see original";
        var helpPos = indicatorPos + new Vector2(0, textSize.Y + padding.Y * 2);
        var helpColor = ImGui.ColorConvertFloat4ToU32(new Vector4(0.7f, 0.7f, 0.7f, 1f));
        drawList.AddText(helpPos, helpColor, helpText);
    }
    ...
    private void RenderOverlapFixPopup()
    {
        if (ImGui.BeginPopupModal("OverlapDetected", ImGuiWindowFlags.AlwaysAutoResize))
        {
            ...
            foreach (var (f1, f2) in overlaps)
            {
                ImGui.BulletText($"'{f1.Name}' and '{f2.Name}'");
            }
