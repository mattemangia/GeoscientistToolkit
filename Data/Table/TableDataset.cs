...
public class TableDataset : Dataset, ISerializableDataset
{
    ...
    public List<Type> ColumnTypes { get; } = new();
                ...
                switch (extension)
                {
                    ...
                    default:
                        throw new NotSupportedException(
                            $"File format '{extension}' is not supported for table datasets.");
                }
            }
    ...
    private void LoadCsvFile()
    {
        ...
        using (var reader = new StreamReader(FilePath, System.Text.Encoding.GetEncoding(Encoding)))
        {
            ...
            if (string.IsNullOrEmpty(headerLine)) throw new InvalidDataException("CSV file is empty");
            ...
            var headers = ParseCsvLine(headerLine, Delimiter[0]);

            // Create columns
            if (HasHeaders)
            {
                foreach (var header in headers) _dataTable.Columns.Add(header.Trim());
            }
            else
            {
                for (var i = 0; i < headers.Length; i++) _dataTable.Columns.Add($"Column{i + 1}");
    ...
    private void LoadExcelFile()
    {
        ...
        using (var workbook = new XLWorkbook(FilePath))
        {
            ...
            foreach (var row in rows)
                ...
                {
                    foreach (var cell in row.Cells()) _dataTable.Columns.Add(cell.Value.ToString());
                    firstRow = false;
                }
                else
                {
                    ...
                    {
                        if (colIndex < _dataTable.Columns.Count) dataRow[colIndex] = cell.Value.ToString();
                        colIndex++;
                    }
    ...
    private void InferColumnTypes()
    {
        ...
        var samplesToCheck = Math.Min(100, _dataTable.Rows.Count);
        ...
        for (var col = 0; col < _dataTable.Columns.Count; col++)
        {
            ...
            {
                var value = _dataTable.Rows[row][col]?.ToString()?.Trim();
