...
namespace GeoscientistToolkit.UI
{
    public sealed class PNMFilterTools : IDatasetTools
    {
        ...
        private float _minPoreRad = 0f, _maxPoreRad = 0f;
        private float _minArea = 0f, _maxArea = 0f;
        private float _minVolVox = 0f, _maxVolVox = 0f;
        private float _minVolPhys = 0f, _maxVolPhys = 0f;
        private int   _minConn = 0, _maxConn = 0;
        private float _minThroatRad = 0f, _maxThroatRad = 0f;
        ...
        public void Draw(Dataset dataset)
        {
            ...
            if (_exportPoresCsvDialog.Submit())
            {
                try
                {
                    pnm.ExportPoresCsv(_exportPoresCsvDialog.SelectedPath, includeHeaders: true, includePhysicalUnits: true);
            ...
            if (_exportThroatsCsvDialog.Submit())
            {
                try
                {
                    pnm.ExportThroatsCsv(_exportThroatsCsvDialog.SelectedPath, includeHeaders: true, includePhysicalUnits: true);
        ...
        private void DrawFilterSection(PNMDataset pnm)
        {
            if (ImGui.CollapsingHeader("Filter Visualised Pores/Throats", ImGuiTreeNodeFlags.DefaultOpen))
            {
                ...
                float voxelSize = Math.Max(pnm.VoxelSize, 1e-6f); // prevent divide-by-zero

                // Suggested ranges from current visible stats
                float radMin = pnm.MinPoreRadius;
                float radMax = pnm.MaxPoreRadius;
                float throatMin = pnm.MinThroatRadius;
                float throatMax = pnm.MaxThroatRadius;

                // Convert to µm for sliders if requested
                float radMinDisp = _usePhysicalUnits ? radMin * voxelSize : radMin;
                float radMaxDisp = _usePhysicalUnits ? radMax * voxelSize : radMax;

                float throatMinDisp = _usePhysicalUnits ? throatMin * voxelSize : throatMin;
                float throatMaxDisp = _usePhysicalUnits ? throatMax * voxelSize : throatMax;

                // Initialize UI ranges lazily
                if (_maxPoreRad <= 0f) { _minPoreRad = radMinDisp; _maxPoreRad = radMaxDisp; }
                if (_maxThroatRad <= 0f) { _minThroatRad = throatMinDisp; _maxThroatRad = throatMaxDisp; }
                ...
                if (ImGui.Button("Apply Filter", new Vector2(150, 0)))
                {
                    ...
                    // Throat radius
                    if (_minThroatRad > 0) c.MinThroatRadius = _usePhysicalUnits ? _minThroatRad / voxelSize : _minThroatRad;
                    if (_maxThroatRad > 0) c.MaxThroatRadius = _usePhysicalUnits ? _maxThroatRad / voxelSize : _maxThroatRad;
