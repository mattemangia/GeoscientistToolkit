    ...
    public class PNMPropertiesRenderer : IDatasetPropertiesRenderer
    {
        public void Draw(Dataset dataset)
        {
            ...
            if (ImGui.CollapsingHeader("Pore Network Properties", ImGuiTreeNodeFlags.DefaultOpen))
            {
                ...
                {
                    float avgConnectivity = pnm.Throats.Count * 2.0f / pnm.Pores.Count;
            ...
            if (ImGui.CollapsingHeader("Pore Size Distribution"))
            {
                ...
                if (pnm.Pores.Count > 0)
                {
                    ...
                    float meanRadius = pnm.Pores.Sum(p => p.Radius) / pnm.Pores.Count;
                    float variance = pnm.Pores.Sum(p => (p.Radius - meanRadius) * (p.Radius - meanRadius)) / pnm.Pores.Count;
                    float stdDev = System.MathF.Sqrt(variance);
            ...
            if (ImGui.CollapsingHeader("Throat Size Distribution"))
            {
                ...
                if (pnm.Throats.Count > 0)
                {
                    ...
                    // Calculate mean and std dev
                    float meanRadius = pnm.Throats.Sum(t => t.Radius) / pnm.Throats.Count;
                    float variance = pnm.Throats.Sum(t => (t.Radius - meanRadius) * (t.Radius - meanRadius)) / pnm.Throats.Count;
                    float stdDev = System.MathF.Sqrt(variance);
