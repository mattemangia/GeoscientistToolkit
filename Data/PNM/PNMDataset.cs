...
namespace GeoscientistToolkit.Data.Pnm
{
    ...
    public class PNMDataset : Dataset, ISerializableDataset
    {
        ...
        public void ApplyFilter(PoreFilterCriteria criteria)
        {
            ...
            if (criteria.MinPoreVolumeVox.HasValue) pores = pores.Where(p => p.VolumeVoxels >= criteria.MinPoreVolumeVox.Value);
            if (criteria.MaxPoreVolumeVox.HasValue) pores = pores.Where(p => p.VolumeVoxels <= criteria.MaxPoreVolumeVox.Value);

            if (criteria.MinPoreVolumePhys.HasValue) pores = pores.Where(p => p.VolumePhysical >= criteria.MinPoreVolumePhys.Value);
            if (criteria.MaxPoreVolumePhys.HasValue) pores = pores.Where(p => p.VolumePhysical <= criteria.MaxPoreVolumePhys.Value);
            ...
            IEnumerable<Throat> throats = _throatsOriginal.Where(t => visibleIds.Contains(t.Pore1ID) && visibleIds.Contains(t.Pore2ID));
        ...
        public Data.Table.TableDataset BuildPoresTableDataset(string datasetName = null, bool includePhysicalUnits = true)
        {
            ...
            return new Data.Table.TableDataset(table.TableName, table);
        }

        public Data.Table.TableDataset BuildThroatsTableDataset(string datasetName = null, bool includePhysicalUnits = true)
        {
        ...
        public void ExportPoresCsv(string path, bool includeHeaders = true, bool includePhysicalUnits = true)
        {
            ...
            var headers = new List<string> { "PoreID", "X", "Y", "Z", "Radius_vox", "Area_vox2", "Volume_vox3", "Connections" };
