...
public sealed class MaterialLibraryWindow
{
    ...
    private void DrawToolbar()
    {
        ...
        ImGui.SameLine();
        if (ImGui.Button("Export CSV")) ExportCsv("Materials.export.csv");
    ...
    private void DrawList()
    {
        ...
        ImGui.SameLine();
        var canDup = _selectedIndex >= 0 && _selectedIndex < MaterialLibrary.Instance.Materials.Count;
        ...
        if (ImGui.BeginTable("matlist", 1, ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg | ImGuiTableFlags.ScrollY))
        {
            ...
            foreach (var view in mats)
            {
                ...
                var realIdx = FindIndexByName(view.Name);
                var selected = realIdx == _selectedIndex;
    ...
    private void DrawEditor()
    {
        ...
        Numeric("Compressive Strength (MPa)", ref _buf.CompressiveMPa, m.CompressiveStrength_MPa,
            v => m.CompressiveStrength_MPa = v);
        ...
        Numeric("Thermal Conductivity (W/m·K)", ref _buf.ThermalK, m.ThermalConductivity_W_mK,
            v => m.ThermalConductivity_W_mK = v);
        Numeric("Specific Heat Capacity (J/kg·K)", ref _buf.SpecificHeat, m.SpecificHeatCapacity_J_kgK,
            v => m.SpecificHeatCapacity_J_kgK = v);
        Numeric("Thermal Diffusivity (m²/s)", ref _buf.ThermalDiff, m.ThermalDiffusivity_m2_s,
            v => m.ThermalDiffusivity_m2_s = v);

        ImGui.Separator();
        ImGui.Text("Wettability / Porosity");
        Numeric("Contact Angle (°)", ref _buf.ContactAngle, m.TypicalWettability_contactAngle_deg,
            v => m.TypicalWettability_contactAngle_deg = v);
        Numeric("Porosity (fraction 0–1)", ref _buf.Porosity, m.TypicalPorosity_fraction,
            v => m.TypicalPorosity_fraction = v);
        ...
        Numeric("Acoustic Impedance (MRayl)", ref _buf.AcousticZ, m.AcousticImpedance_MRayl,
            v => m.AcousticImpedance_MRayl = v);

        ImGui.Separator();
        ImGui.Text("Electrical & Magnetic Properties");
        Numeric("Electrical Resistivity (Ω·m)", ref _buf.Resistivity, m.ElectricalResistivity_Ohm_m,
            v => m.ElectricalResistivity_Ohm_m = v);
        Numeric("Magnetic Susceptibility (SI)", ref _buf.MagSus, m.MagneticSusceptibility_SI,
            v => m.MagneticSusceptibility_SI = v);
        ...
        if (m.Extra.Count > 0 && ImGui.BeginTable("extra", 3,
                ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg | ImGuiTableFlags.SizingStretchProp))
        {
            ...
            foreach (var kv in m.Extra.ToList())
            {
                ...
                ImGui.Text(kv.Key);
                ImGui.TableNextColumn();
                var s = kv.Value.ToString(CultureInfo.InvariantCulture);
        ...
        if (ImGui.Button("Save Changes"))
        {
            ...
            if (!m.ThermalDiffusivity_m2_s.HasValue && m.ThermalConductivity_W_mK.HasValue &&
                m.Density_kg_m3.HasValue && m.SpecificHeatCapacity_J_kgK.HasValue &&
                m.Density_kg_m3 * m.SpecificHeatCapacity_J_kgK > 1e-6)
                m.ThermalDiffusivity_m2_s = m.ThermalConductivity_W_mK.Value /
                                            (m.Density_kg_m3.Value * m.SpecificHeatCapacity_J_kgK.Value);
        ...
        ImGui.SameLine();
        var canDelete = _selectedIndex >= 0 && _edit.IsUserMaterial;
        ...
        if (!canDelete && ImGui.IsItemHovered()) ImGui.SetTooltip("Default materials cannot be deleted.");
    }
    ...
    private string UniqueName(string baseName)
    {
        ...
        var i = 1;
        var mats = MaterialLibrary.Instance.Materials;
        while (mats.Any(m => m.Name.Equals(n, StringComparison.OrdinalIgnoreCase))) n = $"{baseName} {i++}";
        return n;
    }
    ...
    private void Numeric(string label, ref string buf, double? current, Action<double?> assign)
    {
        ...
        {
            buf = local;
            if (TryParseNullable(local, out var val)) assign(val);
        }
    }
    ...
    private void ExportCsv(string path)
    {
        try
        {
            var cols = new[]
            {
                ...
                "YoungModulus_GPa", "PoissonRatio", "FrictionAngle_deg", "CompressiveStrength_MPa",
                "TensileStrength_MPa", "YieldStrength_MPa",
            ...
            foreach (var m in mats)
            {
                string[] vals =
                {
                    ...
                    F(m.YoungModulus_GPa), F(m.PoissonRatio), F(m.FrictionAngle_deg), F(m.CompressiveStrength_MPa),
                    F(m.TensileStrength_MPa), F(m.YieldStrength_MPa),
        ...
        {
            return v.HasValue ? v.Value.ToString("G", CultureInfo.InvariantCulture) : "";
        }
    }
    ...
    private sealed class NumericBuffers
    {
        ...
        public void Clear()
        {
            Viscosity = Mohs = Density =
                ThermalK = Nu = FrictionAngle = YoungGPa = CompressiveMPa = TensileMPa = YieldMPa = "";
