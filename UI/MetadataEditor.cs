...
namespace GeoscientistToolkit.UI
{
    public class MetadataEditor
    {
        ...
        private void SaveMetadata()
        {
            ...
            // Parse latitude
            if (double.TryParse(_latitudeStr, out double lat) && lat >= -90 && lat <= 90)
                _tempMetadata.Latitude = lat;
            else
                _tempMetadata.Latitude = null;
            
            // Parse longitude
            if (double.TryParse(_longitudeStr, out double lon) && lon >= -180 && lon <= 180)
                _tempMetadata.Longitude = lon;
            else
                _tempMetadata.Longitude = null;
            
            // Parse depth
            if (double.TryParse(_depthStr, out double depth))
            ...
            if (DateTime.TryParse(_collectionDateStr, out DateTime collDate))
            ...
            GeoscientistToolkit.Business.ProjectManager.Instance.HasUnsavedChanges = true;
