...
internal class Volume3DDebugWindow
{
    ...
    private bool _generateLabels;
    private volatile bool _isCreating;
    ...
    public void Submit()
    {
        ...
        if (ImGui.Begin("3D Volume Debugger", ref _isOpen))
        {
            ...
            if (ImGui.RadioButton("Sphere", _selectedPattern == TestPattern.Sphere))
                _selectedPattern = TestPattern.Sphere;
            ImGui.SameLine();
            if (ImGui.RadioButton("Cube", _selectedPattern == TestPattern.Cube)) _selectedPattern = TestPattern.Cube;
            ImGui.SameLine();
            if (ImGui.RadioButton("Checkerboard", _selectedPattern == TestPattern.Checkerboard))
                _selectedPattern = TestPattern.Checkerboard;

            if (ImGui.RadioButton("Density Ramp (X-axis)", _selectedPattern == TestPattern.DensityRamp))
                _selectedPattern = TestPattern.DensityRamp;
            ImGui.SameLine();
            ...
            else
            {
                ...
                {
                    _isCreating = true;
                    Task.Run(async () => { await CreateDebugViewer(); });
                }
            }

            ImGui.SameLine();
            if (ImGui.Button("Export Log...")) _logExportDialog.Open($"debug_log_{DateTime.Now:yyyyMMdd_HHmmss}");
            ...
            lock (_logs)
            {
                foreach (var log in _logs) ImGui.TextUnformatted(log);
            }
        ...
        if (_logExportDialog.Submit()) ExportLogToFile(_logExportDialog.SelectedPath);
    ...
    private void AddLog(string message)
    {
        ...
        Debug.WriteLine($"[Volume3DDebugWindow] {message}");
    }
    ...
    private void GenerateTestVolume(ChunkedVolume volume, ChunkedLabelVolume labels, TestPattern pattern,
        bool generateLabels)
    {
        ...
        switch (pattern)
        {
            case TestPattern.Sphere:
                ...
                {
                    for (var y = 0; y < _volumeSize; y++)
                    for (var x = 0; x < _volumeSize; x++)
                    {
                        var dist = Vector3.Distance(new Vector3(x, y, z), center);
                        var value = dist < radius ? (byte)(255 * (1.0f - dist / radius)) : (byte)0;
                        volume[x, y, z] = value;
                        if (generateLabels)
                        {
            ...
            case TestPattern.Cube:
                var start = _volumeSize / 4;
                var end = _volumeSize * 3 / 4;
                var innerStart = _volumeSize / 3;
                var innerEnd = _volumeSize * 2 / 3;
                for (var z = 0; z < _volumeSize; z++)
                for (var y = 0; y < _volumeSize; y++)
                for (var x = 0; x < _volumeSize; x++)
                {
                    var isInCube = x > start && x < end && y > start && y < end && z > start && z < end;
                    volume[x, y, z] = isInCube ? (byte)200 : (byte)0;

                    if (generateLabels && isInCube)
                    {
                        var isInInnerCube = x > innerStart && x < innerEnd && y > innerStart && y < innerEnd &&
                                            z > innerStart && z < innerEnd;
            ...
            case TestPattern.Checkerboard:
                var boardSize = _volumeSize / 8;
                for (var z = 0; z < _volumeSize; z++)
                for (var y = 0; y < _volumeSize; y++)
                for (var x = 0; x < _volumeSize; x++)
                {
                    var cell = x / boardSize + y / boardSize + z / boardSize;
                    var isHigh = cell % 2 == 0;
                    volume[x, y, z] = isHigh ? (byte)220 : (byte)80;
                    if (generateLabels) labels[x, y, z] = isHigh ? (byte)1 : (byte)2;
                }
            ...
            case TestPattern.DensityRamp:
                ...
                for (var x = 0; x < _volumeSize; x++)
                {
                    ...
                    if (generateLabels && x > _volumeSize / 2) labels[x, y, z] = 1;
                }

                break;

            case TestPattern.Noise:
                var noiseData = new byte[_volumeSize * _volumeSize * _volumeSize];
                ...
                for (var x = 0; x < _volumeSize; x++)
                {
                    var val = noiseData[i++];
                    volume[x, y, z] = val;
                    if (generateLabels && val > 128) labels[x, y, z] = (byte)(val > 192 ? 1 : 2);
                }
    ...
    private async Task CreateDebugViewer()
    {
        ...
        try
        {
            ...
            var dummyVolume = new ChunkedVolume(_volumeSize, _volumeSize, _volumeSize);

            var tempDir = Path.Combine(Path.GetTempPath(), "GeoscientistDebug");
            Directory.CreateDirectory(tempDir);
            var datasetName = "DebugDataset";
            ...
            newEditableDataset = new CtImageStackDataset(datasetName, tempDir)
            {
                ...
                Height = _volumeSize,
                Depth = _volumeSize,
                LabelData = new ChunkedLabelVolume(_volumeSize, _volumeSize, _volumeSize,
                    ChunkedVolume.DEFAULT_CHUNK_DIM, false)
            };
            ...
            var gvtPath = Path.Combine(tempDir, $"{datasetName}.gvt");
            AddLog($"BG Task: Converting volume to GVT file at: {gvtPath}");
            await CtStackConverter.ConvertToStreamableFormat(dummyVolume, gvtPath, (p, m) =>
            {
                /* quiet log */
            });
