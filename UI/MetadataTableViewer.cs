...
namespace GeoscientistToolkit.UI
{
    public class MetadataTableViewer
    {
        ...
        private readonly MetadataEditor _metadataEditor = new MetadataEditor();
        ...
        public void Submit()
        {
            ...
            {
                ExportToCSV(_csvExportDialog.SelectedPath);
            }
            
            if (_xlsxExportDialog.Submit())
            {
                ExportToExcel(_xlsxExportDialog.SelectedPath);
            }
        ...
        private void DrawToolbar()
        {
            ...
            {
                _csvExportDialog.Open("metadata_export");
            }
            
            ImGui.SameLine();
            if (ImGui.Button("Export Excel..."))
            {
                _xlsxExportDialog.Open("metadata_export");
            }
            ...
            // GIS Map creation
            if (ImGui.Button("Create GIS Map"))
            {
                CreateGISFromMetadata();
            }
        ...
        private void DrawTable()
        {
            ...
            if (ImGui.BeginTable("MetadataTable", _metadataTable.Columns.Count, tableFlags))
            {
                ...
                // Draw rows
                for (int row = 0; row < _metadataTable.Rows.Count; row++)
                {
                    ...
                    for (int col = 0; col < _metadataTable.Columns.Count; col++)
                    {
                            ...
                            _metadataTable.Columns[col].ColumnName == "Longitude")
                        {
                            ...
                            {
                                double val = Convert.ToDouble(dataRow[col]);
                        ...
                        else if (_metadataTable.Columns[col].DataType == typeof(DateTime))
                        {
                            if (dataRow[col] != DBNull.Value)
                            {
                                DateTime dt = (DateTime)dataRow[col];
                        ...
                        if (col == 0)
                        {
                            if (ImGui.Selectable(cellValue, _selectedRow == row))
                            {
                                ...
                                {
                                    string datasetName = dataRow["Dataset Name"].ToString();
                            ...
                            if (ImGui.IsItemHovered())
                            {
                                ImGui.SetTooltip("Double-click to edit metadata");
                            }
                        }
                        else
                        {
                        ...
                        if (ImGui.IsItemHovered() && cellValue.Length > 30)
                        {
        ...
        private void ExportToExcel(string path)
        {
            ...
            try
            {
                using (var workbook = new XLWorkbook())
                {
                    ...
                    var latColumn = worksheet.ColumnsUsed().FirstOrDefault(c => c.FirstCell().Value.ToString() == "Latitude");
                    var lonColumn = worksheet.ColumnsUsed().FirstOrDefault(c => c.FirstCell().Value.ToString() == "Longitude");
