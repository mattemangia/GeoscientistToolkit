...
{
    private readonly Dictionary<string, ThumbnailInfo> _thumbnailMetadatas = new();
    ...
    protected override void DrawContent()
    {
        ...
        const int padding = 8;
        var cellSize = _thumbnailSize + padding * 2;

        var columns = Math.Max(1, (int)(contentSize.X / cellSize));
    ...
    private void DrawToolbar()
    {
        ...
        ImGui.SameLine();
        if (ImGui.Button("Refresh")) RefreshThumbnails();
    }
    ...
    private void DrawThumbnail(ImageDataset dataset, int index)
    {
        ...
        var thumbKey = dataset.FilePath + "_thumb";
        var textureManager = GlobalPerformanceManager.Instance.TextureCache.GetTexture(thumbKey, () =>
        {
            var thumbInfo = ImageLoader.CreateThumbnail(dataset.FilePath, 256, 256);
            var manager =
                TextureManager.CreateFromPixelData(thumbInfo.Data, (uint)thumbInfo.Width, (uint)thumbInfo.Height);
            var size = (long)thumbInfo.Width * thumbInfo.Height * 4;

            // Store metadata separately
            _thumbnailMetadatas[dataset.FilePath] = new ThumbnailInfo
                { Width = thumbInfo.Width, Height = thumbInfo.Height };
        ...
        if (textureManager != null && _thumbnailMetadatas.TryGetValue(dataset.FilePath, out var meta))
        {
            ...
            {
                var aspectRatio = (float)meta.Width / meta.Height;
                var size = aspectRatio > 1.0f
                    ? new Vector2(_thumbnailSize, _thumbnailSize / aspectRatio)
                    : new Vector2(_thumbnailSize * aspectRatio, _thumbnailSize);

                var offset = (new Vector2(_thumbnailSize) - size) * 0.5f;
