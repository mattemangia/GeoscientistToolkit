...
namespace GeoscientistToolkit.UI
{
    public class TableTools : IDatasetTools
    {
        ...
        private int _selectedAggregateColumn = 0;
        private int _selectedGroupByColumn = 0;
        private int _selectedAggregateFunction = 0;
        private readonly string[] _aggregateFunctions = { "Count", "Sum", "Average", "Min", "Max", "StdDev" };
        
        // Join operation fields
        private int _selectedJoinDataset = 0;
        private int _selectedJoinColumn1 = 0;
        private int _selectedJoinColumn2 = 0;
        private int _selectedJoinType = 0;
        private readonly string[] _joinTypes = { "Inner Join", "Left Join", "Right Join", "Full Outer Join" };
        
        // Sort fields
        private int _selectedSortColumn = 0;
        ...
        private bool[] _selectedColumns = null;
        
        // Rename fields
        private int _renameColumnIndex = 0;
        ...
        public void Draw(Dataset dataset)
        {
            ...
            {
                ExportToCsvFile(tableDataset, _csvExportDialog.SelectedPath);
            }
            
            if (_tsvExportDialog.Submit())
            {
                ExportToTsvFile(tableDataset, _tsvExportDialog.SelectedPath);
            }
            ...
            if (ImGui.CollapsingHeader("Aggregation"))
            {
                DrawAggregationTools(tableDataset);
            }
            
            if (ImGui.CollapsingHeader("Join Tables"))
            {
                DrawJoinTools(tableDataset);
            }
            
            if (ImGui.CollapsingHeader("Column Operations"))
            {
                DrawColumnOperations(tableDataset);
            }
            
            if (ImGui.CollapsingHeader("Export Options"))
            {
                DrawExportOptions(tableDataset);
            }
        }
        ...
        private void DrawTransformationTools(TableDataset tableDataset)
        {
            ...
            ImGui.InputTextWithHint("##FilterExpression", "e.g., Column1 > 100 AND Column2 = 'Value'", ref _filterExpression, 256);
            
            if (ImGui.Button("Apply Filter"))
            {
                ApplyFilter(tableDataset);
            }
            ...
            if (columnNames.Length > 0)
            {
                ...
                {
                    SortData(tableDataset, _selectedSortColumn, _sortAscending);
                }
            }
            ...
            {
                CreateSample(tableDataset, _sampleSize, _randomSample);
            }
        }
        ...
        private void DrawAggregationTools(TableDataset tableDataset)
        {
            ...
            if (ImGui.Button("Create Aggregated Table"))
            {
                CreateAggregatedTable(tableDataset);
            }
            
            ImGui.Separator();
            
            if (ImGui.Button("Create Summary Statistics"))
            {
                CreateSummaryStatistics(tableDataset);
            }
        }
        ...
        private void DrawJoinTools(TableDataset tableDataset)
        {
            ...
            if (_selectedJoinDataset < otherTables.Count)
            {
                ...
                if (ImGui.Button("Perform Join"))
                {
                    PerformJoin(tableDataset, otherTable);
                }
            }
        }
        ...
        private void DrawColumnOperations(TableDataset tableDataset)
        {
            ...
            {
                AddCalculatedColumn(tableDataset);
            }
            ...
            {
                ImGui.Checkbox(columnNames[i], ref _selectedColumns[i]);
            }
        ...
        private void DrawExportOptions(TableDataset tableDataset)
        {
            ...
            {
                _csvExportDialog.Open($"{tableDataset.Name}_export");
            }
            
            if (ImGui.Button("Export to Tab-Separated..."))
            {
                _tsvExportDialog.Open($"{tableDataset.Name}_export");
            }
            
            if (ImGui.Button("Copy All to Clipboard"))
            {
                CopyTableToClipboard(tableDataset);
            }
            
            if (ImGui.Button("Export to New Table Dataset"))
            {
                ExportToNewDataset(tableDataset);
            }
            
            ImGui.Separator();
            
            if (ImGui.Button("Generate Material Statistics"))
            {
                GenerateMaterialStatistics(tableDataset);
            }
            
            if (ImGui.Button("Generate Histogram Table"))
            {
                GenerateHistogramTable(tableDataset);
            }
        }
        ...
        private void RemoveDuplicates(TableDataset tableDataset)
        {
            try
            {
                ...
                {
                    string rowKey = string.Join("|", row.ItemArray.Select(item => item?.ToString() ?? ""));
                ...
                int duplicatesRemoved = dataTable.Rows.Count - newTable.Rows.Count;
        ...
        private void CreateSample(TableDataset tableDataset, int sampleSize, bool random)
        {
            try
            {
                ...
                if (random)
                {
                    ...
                    {
                        newTable.ImportRow(dataTable.Rows[index]);
                    }
                }
                else
                {
                    for (int i = 0; i < Math.Min(sampleSize, dataTable.Rows.Count); i++)
                    {
        ...
        private void CreateAggregatedTable(TableDataset tableDataset)
        {
            try
            {
                ...
                foreach (var group in groups)
                {
                    ...
                    {
                        if (double.TryParse(row[valueColumn]?.ToString(), out double val))
                            return val;
                        return 0;
                    }).ToList();
        ...
        private void PerformJoin(TableDataset table1, TableDataset table2)
        {
            try
            {
                ...
                {
                    resultTable.Columns.Add($"T1_{col.ColumnName}", col.DataType);
                }
                
                // Add columns from second table
                foreach (DataColumn col in dt2.Columns)
                {
                    resultTable.Columns.Add($"T2_{col.ColumnName}", col.DataType);
                }
                ...
                if (_selectedJoinType == 0) // Inner Join
                {
                    ...
                    foreach (var item in query)
                    {
                        ...
                        int col = 0;
                        foreach (var value in item.Row1.ItemArray)
                        {
        ...
        private void AddCalculatedColumn(TableDataset tableDataset)
        {
            try
            {
                ...
                string expression = _columnFormula;
        ...
        private void GenerateHistogramTable(TableDataset tableDataset)
        {
            try
            {
                ...
                foreach (DataRow row in dataTable.Rows)
                {
                    if (row[numericColumn] != DBNull.Value && double.TryParse(row[numericColumn].ToString(), out double val))
                    {
                        values.Add(val);
                    }
                }
                ...
                int binCount = Math.Min(10, values.Count);
                ...
                for (int i = 0; i < binCount; i++)
                {
                    ...
                    double frequency = (double)histogram[i] / values.Count * 100;
