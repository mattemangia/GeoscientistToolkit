...
public class TableProperties : IDatasetPropertiesRenderer
{
    public void Draw(Dataset dataset)
    {
        ...
        if (ImGui.CollapsingHeader("Column Details"))
        {
            ...
            if (ImGui.BeginTable("ColumnDetailsTable", 3,
                    ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg | ImGuiTableFlags.SizingStretchProp))
            {
                ...
                for (var i = 0; i < tableDataset.ColumnNames.Count && i < tableDataset.ColumnTypes.Count; i++)
                {
                    ImGui.TableNextRow();
                    ...
                    ImGui.TableNextColumn();
                    var typeName = tableDataset.ColumnTypes[i].Name;
        ...
        if (ImGui.CollapsingHeader("Memory Usage"))
        {
            ...
            var estimatedMemory = tableDataset.GetSizeInBytes();
            ...
            if (tableDataset.SourceFormat == "Generated")
                ImGui.TextWrapped(
                    "This is a generated table without a file backing. Size shown is estimated memory usage.");
    ...
    private void DrawQuickStatistics(TableDataset tableDataset)
    {
        ...
        if (numericColumns > 0) ImGui.BulletText($"Numeric: {numericColumns}");
        if (textColumns > 0) ImGui.BulletText($"Text: {textColumns}");
        if (dateColumns > 0) ImGui.BulletText($"Date/Time: {dateColumns}");
        if (boolColumns > 0) ImGui.BulletText($"Boolean: {boolColumns}");

        ImGui.Unindent();

        // Show total cells
        var totalCells = (long)tableDataset.RowCount * tableDataset.ColumnCount;
