...
namespace GeoscientistToolkit.UI
{
    public class SystemInfoWindow
    {
        ...
        private string GetCpuNameWindows()
        {
            ...
            catch { return "N/A (Requires WMI permissions)"; }
        }
        ...
        private string GetTotalRam()
        {
            try
            {
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                {
                    ...
                    var totalRamBytes = Convert.ToUInt64(searcher.Get().Cast<ManagementObject>().FirstOrDefault()?["TotalPhysicalMemory"]);
                    return $"{totalRamBytes / (1024.0 * 1024.0 * 1024.0):F2} GB";
                }
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
                {
                    string meminfo = File.ReadAllLines("/proc/meminfo").FirstOrDefault(line => line.StartsWith("MemTotal:"));
                    var parts = meminfo?.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                    if (parts?.Length >= 2 && long.TryParse(parts[1], out long ramKiB))
                    {
                ...
                else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
                {
                    ...
                    if (long.TryParse(output.Trim(), out long ramBytes))
                    {
        ...
        private void ExportInfoToFile()
        {
            try
            {
                string content = BuildStringForExport();
