...
namespace GeoscientistToolkit.Analysis.RemoveSmallIslands
{
    ...
    public class RemoveSmallIslandsUI : IDisposable
    {
        ...
        public void DrawPanel(CtImageStackDataset dataset)
        {
            ...
            if (ImGui.Checkbox("Show Preview", ref _showPreview))
            {
                if (_showPreview)
                {
                    _ = ProcessIslandsAsync(dataset, isPreviewOnly: true);
                }
                else
                {
            ...
            {
                ImGui.SetTooltip("Shows a 3D preview of particles to be removed (in red).");
            }

            ImGui.Spacing();
            if (_isProcessing)
            {
                ImGui.ProgressBar(_processor.Progress, new Vector2(-1, 0), $"{_processor.CurrentStage} ({_processor.Progress * 100:F1}%)");
            ...
            {
                if (ImGui.Button("Clean Material", new Vector2(-1, 0)))
                {
                    _ = ProcessIslandsAsync(dataset, isPreviewOnly: false);
                }
            }
        }
        ...
        private async Task ProcessIslandsAsync(CtImageStackDataset dataset, bool isPreviewOnly)
        {
            ...
            try
            {
                ...
                long voxelThreshold = ConvertThresholdToVoxels(dataset);
                ...
                var smallParticleIds = new HashSet<int>(_lastAnalysisResult.Particles.Where(p => p.VoxelCount < voxelThreshold).Select(p => p.Id));
                token.ThrowIfCancellationRequested();
                ...
                if (isPreviewOnly)
                {
                    ...
                    byte[] previewMask = await Task.Run(() => GeneratePreviewMask(_lastAnalysisResult.LabelVolume, smallParticleIds), token);
                ...
                else
                {
                    ...
                    _processor.Progress = 0.9f;
                    await Task.Run(() => ApplyCleaning(dataset.LabelData, _lastAnalysisResult.LabelVolume, smallParticleIds, token), token);
                    ProjectManager.Instance.NotifyDatasetDataChanged(dataset);
                    ProjectManager.Instance.HasUnsavedChanges = true;
                    Logger.Log($"[RemoveSmallIslands] Cleaned {smallParticleIds.Count} particles from material '{targetMaterial.Name}'.");
                }
            }
        ...
        private long ConvertThresholdToVoxels(CtImageStackDataset dataset)
        {
            ...
            double thresholdUm3 = _sizeThreshold * (_selectedUnitIndex switch { 0 => 1e12, 1 => 1e9, _ => 1.0 });
        ...
        private void ApplyCleaning(ChunkedLabelVolume datasetLabels, int[,,] islandLabels, HashSet<int> smallParticleIds, CancellationToken token)
        {
            int d = islandLabels.GetLength(2);
            Parallel.For(0, d, new ParallelOptions{ CancellationToken = token }, z =>
            {
                for (int y = 0; y < islandLabels.GetLength(1); y++)
                for (int x = 0; x < islandLabels.GetLength(0); x++)
                {
    ...
    internal class IslandAnalysisResult { public int[,,] LabelVolume; public List<IslandParticle> Particles; }
    internal class IslandParticle { public int Id; public int VoxelCount; }
    internal class IslandAnalysisProcessor : IDisposable
    {
        ...
        {
            CurrentStage = "Extracting mask"; Progress = 0;
            var mask = ExtractMaterialMask(dataset, material, token); token.ThrowIfCancellationRequested();
            CurrentStage = "Labeling components"; Progress = 0.2f;
            var labels = LabelComponents3DParallel(mask, token); token.ThrowIfCancellationRequested();
            CurrentStage = "Analyzing particles"; Progress = 0.6f;
            var particles = AnalyzeParticles(labels, token); token.ThrowIfCancellationRequested();
            CurrentStage = "Complete"; Progress = 1.0f;
        ...
        private byte[,,] ExtractMaterialMask(CtImageStackDataset d, Material m, CancellationToken t)
        {
            ...
            Parallel.For(0, d.Depth, new ParallelOptions { CancellationToken = t }, z => {
                for(int y=0;y<d.Height;y++) for(int x=0;x<d.Width;x++) if(d.LabelData[x,y,z]==m.ID) mask[x,y,z]=1;
        ...
        private int[,,] LabelComponents3DParallel(byte[,,] mask, CancellationToken token)
        {
            ...
            var labels = new int[w,h,d]; var uf = new ConcurrentUnionFind(); int next=1;
            Parallel.For(0,d, new ParallelOptions{CancellationToken=token}, z=>{
                for(int y=0;y<h;y++) for(int x=0;x<w;x++) {
                    ...
                    if(n.Count==0){int l=Interlocked.Increment(ref next)-1; labels[x,y,z]=l; uf.MakeSet(l);}
                    else{int min=n.Min(); labels[x,y,z]=min; foreach(var neighbor in n) uf.Union(min,neighbor);}
                }
                Progress = 0.2f + (0.3f * (z + 1) / d);
            });
            token.ThrowIfCancellationRequested();
            var final=new ConcurrentDictionary<int,int>(); int counter=0;
            Parallel.For(0,d, new ParallelOptions{CancellationToken=token}, z=>{
                for(int y=0;y<h;y++) for(int x=0;x<w;x++) {
                    if(labels[x,y,z]>0){
                        int root=uf.Find(labels[x,y,z]);
        ...
        private List<IslandParticle> AnalyzeParticles(int[,,] labels, CancellationToken token)
        {
            ...
            int d=labels.GetLength(2);
            Parallel.For(0,d,new ParallelOptions{CancellationToken=token}, z=>{
                for(int y=0;y<labels.GetLength(1);y++) for(int x=0;x<labels.GetLength(0);x++) {
                    int l=labels[x,y,z]; if(l>0) particles.AddOrUpdate(l,1,(k,c)=>c+1);
                }
        ...
        public void Dispose() {}
        private class ConcurrentUnionFind { readonly ConcurrentDictionary<int,int> p=new ConcurrentDictionary<int,int>(); public void MakeSet(int x)=>p.TryAdd(x,x); public int Find(int x){if(!p.TryGetValue(x,out int parent)){MakeSet(x);return x;} if(parent==x)return x; return p[x]=Find(parent);} public void Union(int x,int y){int rX=Find(x),rY=Find(y); if(rX!=rY) p[Math.Max(rX,rY)]=Math.Min(rX,rY);} }
    }
}