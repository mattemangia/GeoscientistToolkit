...
The verification tests report per-case **inputs**, **expected values**, **observed outputs**, and **error/variance**. See `VerificationReport.md` for full narratives and citations.
...
Directional sensitivities are summarized from the governing equations used by each test (e.g., Mohr–Coulomb, travel-time, Rayleigh fractionation, EOS, heat conduction). These show how outputs change when key inputs vary.
...
## Results snapshot (inputs vs. expected vs. observed)

The snapshot below restates the per-test inputs and observed outputs so validation data is visible in one place alongside the case-study references. All values are asserted in the verification tests and documented in `VerificationReport.md`.
